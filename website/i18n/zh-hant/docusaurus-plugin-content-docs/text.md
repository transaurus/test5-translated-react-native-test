...
你還失去了為整個子樹設定預設字體的能力。同時，`fontFamily` 只接受單一字體名稱，這與 CSS 中的 `font-family` 不同。建議在應用程式中使用一致的字體和大小的方式是創建一個包含這些樣式的元件 `MyAppText`，並在整個應用程式中使用該元件。你還可以使用此元件來創建更具體的元件，例如 `MyAppHeaderText` 用於其他類型的文字。
...
- （開發者）React 元件設計時考慮了強隔離性：你應該能夠將元件放置在應用程式的任何位置，並相信只要屬性相同，它的外觀和行為就會相同。可以從屬性外部繼承的文字屬性將破壞這種隔離性。
...
輔助功能提示能幫助使用者理解當他們對輔助功能元素執行操作時會發生什麼結果，尤其是當該結果無法從輔助功能標籤中明確得知時。
...
輔助功能操作允許輔助技術以程式化的方式呼叫元件的操作。`accessibilityActions` 屬性應包含一個操作物件列表。每個操作物件應包含名稱和標籤欄位。
...
當使用者執行輔助功能操作時呼叫。此函數的唯一參數是一個包含要執行操作名稱的事件物件。
...
在 Android API Level 23+ 上設定自動斷字的頻率，用於決定單詞斷行。
...
- `head` - 行尾顯示完整，行首缺失文字以省略號表示。例："...wxyz"
- `middle` - 行首行尾顯示完整，中間缺失文字以省略號表示。例："ab...yz"
- `tail` - 行首顯示完整，行尾缺失文字以省略號表示。例："abcd..."
- `clip` - 不繪製超出文字容器邊界的部分。

> 在Android上，當`numberOfLines`設定為大於`1`的值時，只有`tail`能正常運作。
...
在 Android 上，從此回調返回 true 可阻止其他原生元件成為回應者，直到此回應者終止。
...
回應者權限已從此 `View` 移除。可能因呼叫 `onResponderTerminationRequest` 後被其他視圖取得，也可能被作業系統未經詢問直接取走（例如 iOS 上的控制中心/通知中心）。
...
當其他 `View` 試圖成為觸控回應者並要求此 `View` 釋放回應權時，會觸發此函數。返回 `true` 表示允許釋放。
...
若父層 `View` 需阻止子層 `View` 在觸碰開始時成為回應者，應在此處理函數中返回 `true`。
...
當滾動視圖被禁用時，此屬性定義觸碰點可偏離按鈕多遠才會停用按鈕。停用後若將觸碰點移回範圍內，按鈕會重新啟用。在滾動視圖禁用狀態下反覆測試此行為時，請傳入常數以減少記憶體分配。
...
在 Android API Level 23+ 上設定文字斷行策略，可選值為 `simple`、`highQuality`、`balanced`。
...
`TextLayoutEvent` 物件會在元件佈局變更時作為回調結果返回。它包含一個名為 `lines` 的鍵，其值是一個陣列，包含與每個渲染文字行相對應的 [`TextLayout`](text#textlayout) 物件。
