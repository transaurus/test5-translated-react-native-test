...
為 `key` 設定數值並於完成時觸發回呼函式。回傳 `Promise` 物件。
...
移除指定 `key` 的項目並於完成時觸發回呼函式。回傳 `Promise` 物件。
...
清除 _所有_ 用戶端、函式庫等的 `AsyncStorage` 資料。通常不建議直接呼叫此方法，應改用 `removeItem` 或 `multiRemove` 僅清除應用程式自身的鍵值。回傳 `Promise` 物件。
...
取得應用程式 _所有_ 已知的鍵值（包含所有呼叫者與函式庫）。回傳 `Promise` 物件。
...
使用此方法批次儲存多個鍵值對。操作完成時將收到單一回調函數並傳入任何錯誤：
...
批次操作以合併指定鍵集的現有值與新值。此操作假設值為字串化的 JSON。返回一個 `Promise` 物件。
