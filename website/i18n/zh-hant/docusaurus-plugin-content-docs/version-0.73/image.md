...
類似於 `source`，此屬性代表用於渲染圖片載入指示器的資源。載入指示器會一直顯示，直到圖片準備好顯示（通常是圖片下載完成後）。
...
- `resize`：一種軟體操作，會在解碼前改變記憶體中的編碼圖片。當圖片遠大於視圖時，應使用此方法而非 `scale`。

- `scale`：圖片會被縮小或放大繪製。與 `resize` 相比，`scale` 速度更快（通常由硬體加速）且能產生更高品質的圖片。如果圖片小於視圖，或僅略大於視圖，應使用此方法。
...
- `repeat`: 重複圖片以覆蓋視圖的框架。圖片將保持其大小和寬高比，除非它比視圖大，在這種情況下它將被均勻縮小以適應視圖。
...
此屬性還可以包含多個遠程 URL，並指定它們的寬度和高度，以及可能的縮放比例/其他 URI 參數。原生端將根據圖片容器的測量大小選擇最佳的 `uri` 來顯示。可以添加 `cache` 屬性來控制網絡請求如何與本地緩存交互。（更多信息請參見[圖片的緩存控制（僅限 iOS）](images#cache-control-ios-only)）。
...
一個表示圖片遠程 URL 的字串。此屬性優先於 `source` 屬性。
...
在顯示圖片之前，獲取其寬度和高度（以像素為單位）。如果圖片無法找到或下載失敗，此方法可能會失敗。
...
在顯示圖片之前，獲取其寬度和高度（以像素為單位），並能夠提供請求的標頭。如果圖片無法找到或下載失敗，此方法可能會失敗。此方法不適用於靜態圖片資源。
...
執行緩存查詢。返回一個解析為URL到緩存狀態映射的Promise，例如"disk"、"memory"或"disk/memory"。如果請求的URL不在映射中，則表示它不在緩存中。
...
- `only-if-cached`：無論快取資料的年齡或過期日期為何，都將使用現有的快取資料來滿足請求。如果快取中沒有對應 URL 載入請求的資料，則不會嘗試從原始來源載入資料，且該載入將被視為失敗。
