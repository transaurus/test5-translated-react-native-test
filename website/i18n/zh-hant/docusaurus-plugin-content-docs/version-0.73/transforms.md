...
變換(Transforms)是能幫助您使用2D或3D轉換來修改元件外觀和位置的樣式屬性。但需注意，套用變換後，元件周圍的佈局會保持原狀，可能導致與相鄰元件重疊。您可以透過為變換元件添加邊距(margin)、調整相鄰元件或設置容器內邊距(padding)來避免此情況。
...
`transform`可接受轉換物件陣列或以空格分隔的字串值。每個物件需以要變換的屬性作為鍵(key)，並指定變換值。請注意物件不可合併使用，每個物件應只包含一組鍵/值對。
