...
有時了解設備當前是否有啟用螢幕閱讀器非常有用。`AccessibilityInfo` API 正是為此目的而設計。您可以用它來查詢螢幕閱讀器的當前狀態，並註冊在螢幕閱讀器狀態改變時接收通知。
...
> **注意**：如果您僅需檢查 TalkBack 的狀態，請使用 [isScreenReaderEnabled](#isscreenreaderenabled)。
