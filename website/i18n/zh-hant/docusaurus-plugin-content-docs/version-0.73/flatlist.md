...
若要渲染多列，請使用 [`numColumns`](flatlist.md#numcolumns) 屬性。使用此方法而非 `flexWrap` 佈局可以避免與項目高度邏輯的衝突。

下方為更複雜、可選取的範例。

- 透過將 `extraData={selectedId}` 傳遞給 `FlatList`，我們確保當狀態變更時 `FlatList` 本身會重新渲染。若未設定此屬性，`FlatList` 將不會知道需要重新渲染任何項目，因為它是一個 `PureComponent`，且屬性比較不會顯示任何變更。
...
這是 [`<VirtualizedList>`](virtualizedlist.md) 的便利封裝，因此繼承了其屬性（以及 [`<ScrollView>`](scrollview.md) 的屬性），此處未明確列出的屬性，以及以下注意事項：
...
- 為了限制記憶體使用並實現平滑滾動，內容會異步渲染於螢幕外。這意味著可能會滾動得比填充速率更快，並暫時看到空白內容。這是一個可以根據每個應用需求調整的權衡，我們正在幕後努力改進它。
...
提供額外的元資料，如 `index`（若您需要），以及更通用的 `separators.updateProps` 函數，讓您可以設定任何屬性來變更前導分隔線或後續分隔線的渲染，以防更常見的 `highlight` 和 `unhighlight`（設定 `highlighted: boolean` 屬性）不足以滿足您的使用情境。
...
在每個項目之間渲染（但不在頂部或底部）。預設情況下會提供 `highlighted` 和 `leadingItem` 屬性。`renderItem` 提供 `separators.highlight`/`unhighlight` 來更新 `highlighted` 屬性，但您也可以透過 `separators.updateProps` 添加自訂屬性。可以是 React 元件（例如 `SomeComponent`）或 React 元素（例如 `<SomeComponent />`）。
...
一個標記屬性，用於通知列表重新渲染（因為它實作了 `PureComponent`）。如果您的 `renderItem`、Header、Footer 等函數依賴於 `data` 屬性之外的任何內容，請將其放在這裡並視為不可變的。
...
添加 `getItemLayout` 可以大幅提升數百個項目列表的性能。請記住，如果指定了 `ItemSeparatorComponent`，請在偏移計算中包含分隔符的長度（高度或寬度）。
...
初始批次要渲染多少項目。這個數量應該足以填滿螢幕，但不需要過多。請注意，這些項目永遠不會因為視窗化渲染而被卸載，以提升滾動至頂部的感知效能。
...
不從第一項開始顯示，而是從 `initialScrollIndex` 指定的索引處開始。這會停用「滾動至頂部」的優化（該優化會始終渲染前 `initialNumToRender` 個項目），並立即渲染從此初始索引開始的項目。需要實作 `getItemLayout`。
...
用於為指定索引處的項目提取唯一鍵。此鍵用於快取以及作為 React 的 key 來追蹤項目重新排序。預設的提取器會檢查 `item.key`，然後是 `item.id`，最後回退到使用索引（與 React 的行為相同）。
...
> 注意：在某些情況下可能會有 bug（內容缺失）——使用時需自行承擔風險。
...
至少需要設定 `viewAreaCoveragePercentThreshold` 或 `itemVisiblePercentThreshold` 其中一項。這需要在 `constructor` 中完成，以避免以下錯誤（參考[此問題](https://github.com/facebook/react-native/issues/17408)）：
...
視窗覆蓋百分比閾值（0-100），部分被遮擋的項目需達到此比例才會被視為「可見」。完全可見的項目始終被視為可見。數值為0表示只要有一個像素在視窗內即算可見；數值為100表示項目必須完全可見或覆蓋整個視窗才會被計入。

#### itemVisiblePercentThreshold

類似於 `viewAreaCoveragePercentThreshold`，但計算的是項目本身的可見百分比，而非其覆蓋視窗區域的比例。
...
滾動至內容末尾。若未設置 `getItemLayout` 屬性，可能出現卡頓。
...
滾動至指定索引的項目，並將其定位於視窗區域：`viewPosition` 為0時置頂，1時置底，0.5時居中。
...
- 'viewPosition' (number) - 數值 `0` 將指定索引項目置頂，`1` 置底，`0.5` 居中。
