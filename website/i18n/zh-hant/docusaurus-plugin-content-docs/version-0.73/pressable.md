...
手指並非最精確的工具，使用者常會意外觸發錯誤元素或錯過觸發區域。為此，`Pressable` 提供可選的 `HitRect`，可用來定義觸控可註冊的範圍距離包裝元素多遠。按壓可在 `HitRect` 內的任何位置開始。

`PressRect` 允許按壓移動超出元素及其 `HitRect`，同時保持觸發狀態並符合「按壓」資格——想像您按住按鈕時慢慢滑開手指的情境。
...
> `Pressable` 使用 React Native 的 `Pressability` API。有關 Pressability 狀態機流程及其運作原理的更多資訊，請查看 [Pressability](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Pressability/Pressability.js#L350) 的實作。
...
當觸碰開始時立即呼叫，發生於 `onPressOut` 和 `onPress` 之前。
...
在此視圖外額外的距離範圍內，觸碰仍會被視為按壓（直到觸發 `onPressOut`）。
...
可為視圖樣式，或接收反映元件當前按壓狀態的布林值並回傳視圖樣式的函式。
