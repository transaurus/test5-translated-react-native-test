...
設為 `false` 可移除為特定升部/降部預留的額外字體內邊距。某些字體使用此內邊距時，垂直置中的文字可能略微對齊不準。為達最佳效果，建議同時將 `textAlignVertical` 設為 `center`。
...
指定文字對齊方式。在 Android 上，'justify' 值僅支援 Oreo (8.0) 或更高版本 (API 等級 >= 26)。較低 Android 版本會回退至 `left`。
