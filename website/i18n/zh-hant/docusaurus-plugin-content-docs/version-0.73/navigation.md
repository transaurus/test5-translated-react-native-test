...
若您要將 React Native 整合至已使用原生導航的應用程式，或尋找 React Navigation 的替代方案，以下函式庫可在雙平台提供原生導航功能：[react-native-navigation](https://github.com/wix/react-native-navigation)。
...
- 若為純 React Native 專案，請使用 `npm` 安裝相依套件：
  ...
  純 React Native 專案的 iOS 端需確保已安裝 [CocoaPods](https://cocoapods.org/)。接著安裝 pods 以完成設定：
...
此 `native-stack` 導航器使用原生 API：iOS 端的 `UINavigationController` 與 Android 端的 `Fragment`，因此透過 `createNativeStackNavigator` 建置的導航行為將與基於這些 API 的原生應用程式具有相同特性與效能表現。

React Navigation 也提供不同類型的導航器套件（如分頁與抽屜導航），可用於實作應用程式中的各種導航模式。

完整的 React Navigation 入門教學，請參閱 [React Navigation 入門指南](https://reactnavigation.org/docs/getting-started)。