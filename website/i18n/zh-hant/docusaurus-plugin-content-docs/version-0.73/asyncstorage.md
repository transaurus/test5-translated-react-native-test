...
根據 `key` 取得項目並在完成時觸發回調。返回 `Promise` 物件。
...
清除 _所有_ `AsyncStorage` 資料（含其他客戶端、函式庫等）。通常不應直接呼叫此方法，建議改用 `removeItem` 或 `multiRemove` 僅清除自身應用金鑰。返回 `Promise` 物件。
...
取得應用程式 _所有_ 已知金鑰（含其他呼叫方、函式庫等）。返回 `Promise` 物件。
...
批次操作，用於合併給定鍵集合的現有值與新值。此操作假設這些值是已轉換為字串的 JSON。返回一個 `Promise` 物件。
