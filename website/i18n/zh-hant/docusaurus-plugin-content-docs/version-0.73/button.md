...
基礎按鈕元件，可在任何平台上呈現良好效果。支援最基本的客製化選項。

若此按鈕不符合您的應用程式需求，您可以使用 [Pressable](pressable) 自行建立按鈕。如需靈感，請參考 [Button 元件的原始碼](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Components/Button.js)。
...
顯示在按鈕內部的文字。在 Android 上，給定的標題將轉換為大寫形式。
...
無障礙操作允許輔助技術以程式化方式呼叫元件的操作。`accessibilityActions` 屬性應包含操作物件的列表。每個操作物件應包含名稱和標籤欄位。
