...
使用 React Native 時，您的 JavaScript 代碼可能會在三種環境中運行：

- 多數情況下，React Native 會使用 [Hermes](hermes)，這是一個專為 React Native 優化的開源 JavaScript 引擎。
- 如果禁用 Hermes，React Native 將使用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)（Safari 的 JavaScript 引擎）。請注意，在 iOS 上由於應用程式缺乏可寫入的執行記憶體，JavaScriptCore 無法使用 JIT 編譯。
- 使用 Chrome 調試時，所有 JavaScript 代碼會在 Chrome 內部運行，並通過 WebSocket 與原生代碼通信。Chrome 使用 [V8](https://v8.dev/) 作為其 JavaScript 引擎。

雖然這些環境非常相似，但仍可能遇到不一致的情況。建議避免依賴特定運行環境的特性。

## JavaScript 語法轉換器

語法轉換器讓編寫代碼更愉悅，允許您使用新的 JavaScript 語法，而無需等待所有直譯器的支援。

React Native 內置了 [Babel JavaScript 編譯器](https://babeljs.io)。詳情請查閱 [Babel 文檔](https://babeljs.io/docs/plugins/#transform-plugins) 了解其支援的轉換功能。

完整的 React Native 啟用轉換列表可在 [@react-native/babel-preset](https://github.com/facebook/react-native/tree/main/packages/react-native-babel-preset) 中找到。
...
許多標準函數在所有支援的 JavaScript 運行環境中均可使用。
