...
- `runAfterInteractions()`：延後執行程式碼，且不會延遲正在進行的動畫。

觸控處理系統會將一或多個有效觸控視為「互動」，並將延遲執行 `runAfterInteractions()` 回調，直到所有觸控結束或被取消。

InteractionManager 也允許應用程式在動畫開始時建立互動「控制代碼」，並在完成時清除它來註冊動畫：
...
預設情況下，排隊任務會在一個 `setImmediate` 批次中一起執行。若呼叫帶正數的 `setDeadline`，則任務僅會在截止時間（以 js 事件循環運行時間為準）接近前執行，屆時會透過 setTimeout 讓出執行權，允許觸控等事件開始互動並阻止排隊任務執行，從而提升應用程式響應速度。
...
傳入正數會使用 setTimeout 在 eventLoopRunningTime 達到截止值後安排任務，否則所有任務會在一個 setImmediate 批次中執行（預設）。