...
**若您是行動開發的新手**，最簡單的入門方式是使用 Expo Go。Expo 是一套圍繞 React Native 建置的工具和服務，雖然它擁有眾多[功能](https://docs.expo.dev)，但對我們目前最相關的功能是它能讓您在幾分鐘內開始編寫 React Native 應用程式。您只需要最新版本的 Node.js 和一支手機或模擬器。若您想在安裝任何工具前，直接在網頁瀏覽器中試用 React Native，可以嘗試 [Snack](https://snack.expo.dev/)。

**若您已熟悉行動開發**，您可能會想使用 React Native CLI。它需要 Xcode 或 Android Studio 才能開始。若您已安裝其中一個工具，應該能在幾分鐘內開始運作。若尚未安裝，則預計需花費約一小時來安裝和配置這些工具。
