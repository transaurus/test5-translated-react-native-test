...
當無障礙標籤無法清楚說明操作結果時，無障礙提示可幫助使用者理解在無障礙元素上執行操作時會發生的情況。
...
參考 [Android `View` 文檔](https://developer.android.com/reference/android/view/View.html#attr_android:accessibilityLiveRegion) 以獲取更多資訊。
...
- `'grid'` - 與 ScrollView、VirtualizedList、FlatList 或 SectionList 一起使用以代表網格。向 android GridView 添加網格內/外的宣告。
...
此 `View` 是否需要離屏渲染並以 alpha 值合成，以保留 100% 正確的色彩與混合行為。預設值 (`false`) 會改為在繪製每個元素時直接套用 alpha 值，而非將整個元件離屏渲染後再以 alpha 值合成回來。當你設定透明度的 `View` 包含多個重疊元素（例如多個重疊的 `View` 或文字與背景）時，預設行為可能會產生明顯且非預期的效果。
...
該視圖現在開始回應觸控事件。此時應高亮顯示並向使用者展示反饋。

在 Android 上，從此回調返回 true 可阻止其他原生組件在此回應者終止前取得回應權。
...
回應權已從該 `View` 被剝奪。可能因 `onResponderTerminationRequest` 調用後被其他視圖取得，也可能被系統未經詢問直接剝奪（例如 iOS 控制中心/通知中心觸發時）。
...
其他 `View` 請求成為回應者，並要求該 `View` 釋放回應權。返回 `true` 表示允許釋放。
...
這是一個由 `RCTView` 公開的保留性能屬性，對於滾動內容且包含許多子視圖（大多數位於屏幕外）的情況非常有用。要使此屬性生效，必須將其應用於包含許多超出其邊界的子視圖的視圖。子視圖還必須具有 `overflow: hidden`，包含視圖（或其父視圖之一）也應如此。
