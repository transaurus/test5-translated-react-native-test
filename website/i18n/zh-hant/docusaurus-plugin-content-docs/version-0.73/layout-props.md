...
- 在具有測量函式的節點上，長寬比視同測量函式測量 flex basis
...
詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/bottom 以了解 `bottom` 如何影響佈局。
...
當 `flex` 為 0 時，元件根據 `width` 和 `height` 設定大小，且不具彈性。

當 `flex` 為 -1 時，元件通常根據 `width` 和 `height` 設定大小。但如果空間不足，元件會縮小至 `minWidth` 和 `minHeight`。
...
[`flexShrink`](layout-props#flexshrink) 描述了當子項目在主軸上的總大小超出容器大小時，如何沿主軸縮小子項目。`flexShrink` 與 `flexGrow` 非常相似，可以將任何溢出的空間視為負的剩餘空間。這兩個屬性可以很好地協同工作，允許子項目根據需要伸展或收縮。
...
其運作方式類似於 CSS 中的 `max-width`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。
...
其運作方式類似於 CSS 中的 `min-height`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。
...
其運作方式類似於 CSS 中的 `min-width`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。
...
`overflow` 控制子元件的測量與顯示方式。`overflow: hidden` 會裁剪視圖，而 `overflow: scroll` 會使視圖獨立於父元件的主軸進行測量。其運作方式類似於 CSS 中的 `overflow`（預設值為 visible）。詳見 https://developer.mozilla.org/en/docs/Web/CSS/overflow 以獲取更多細節。
...
`paddingBottom` 的運作方式類似於 CSS 中的 `padding-bottom`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/padding-bottom 以獲取更多細節。
...
`paddingLeft` 的運作方式類似於 CSS 中的 `padding-left`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/padding-left 以獲取更多細節。
...
`paddingRight` 的運作方式類似於 CSS 中的 `padding-right`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/padding-right 以獲取更多細節。
...
若需相對於非父元素的對象定位子元素，請勿使用樣式，而應透過組件樹實現。
...
其作用類似於 CSS 中的 `right`，但在 React Native 中僅支援點數或百分比單位，不支援 em 等其他單位。
...
`top` 表示以邏輯像素為單位偏移此組件頂邊緣的數值。

其作用類似於 CSS 中的 `top`，但在 React Native 中僅支援點數或百分比單位，不支援 em 等其他單位。
...
其作用類似於 CSS 中的 `width`，但在 React Native 中僅支援點數或百分比單位，不支援 em 等其他單位。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/width 以獲取更多細節。
...
`zIndex` 控制哪些組件顯示在其他組件上方。通常情況下無需使用 `zIndex`，組件會依照文件樹中的順序渲染，後渲染的組件會覆蓋先前的組件。在需要自訂模態介面或動畫效果時，`zIndex` 可能會有幫助。
