...
雖然配色方案資訊可立即取得，但這可能會改變（例如在日出或日落時預定的配色方案變更）。任何依賴使用者偏好配色方案的渲染邏輯或樣式，應在每次渲染時呼叫此函數，而非快取值。例如，您可以使用 [`useColorScheme`](usecolorscheme) React 鉤子，因為它提供並訂閱配色方案的更新，或者您可以使用內聯樣式而非在 `StyleSheet` 中設定值。
...
表示當前使用者偏好的配色方案。此值可能會在之後更新，無論是透過直接的使用者操作（例如在裝置設定中選擇主題或透過 `setColorScheme` 設定應用程式層級的使用者介面風格），或是根據排程（例如遵循日夜循環的淺色和深色主題）。
