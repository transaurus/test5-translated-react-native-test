...
本地函式庫是指包含視圖或模組、且專屬於您應用程式的函式庫，這些函式庫並未發佈到註冊表中。與傳統的視圖和模組設置不同，本地函式庫與您應用程式的原生代碼是解耦的。
...
默認情況下，生成的函式庫會自動連結到項目中：使用 Yarn 時採用 `link:` 協議，使用 npm 時則採用 `file:` 協議：
...
這會在 `node_modules` 下創建一個指向函式庫的符號連結（symlink），從而實現自動連結功能。
