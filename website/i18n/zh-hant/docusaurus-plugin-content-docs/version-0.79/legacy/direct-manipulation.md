...
直接操作不應是頻繁使用的工具。通常僅在創建連續動畫以避免渲染元件層級和協調大量視圖的開銷時使用。
`setNativeProps` 是命令式的，並將狀態儲存在原生層（DOM、UIView 等）而非 React 元件中，這會使程式碼更難維護。
...
假設 `setNativeProps` 不可用。我們可能會將不透明度值儲存在狀態中，並在 `onPress` 觸發時更新該值：
...
複合元件並無原生視圖支持，因此無法對其調用 `setNativeProps`。參考以下範例：
...
你可能注意到我們使用 `{...props}` 將所有屬性傳遞給子視圖。這是因為 `TouchableOpacity` 實際上是一個複合元件，除了依賴子元件的 `setNativeProps` 外，還需要子元件處理觸控操作。為此，它會傳遞[多種屬性](view.md#onmoveshouldsetresponder)回調到 `TouchableOpacity` 元件。相比之下，`TouchableHighlight` 由原生視圖支持，僅需要我們實作 `setNativeProps`。
