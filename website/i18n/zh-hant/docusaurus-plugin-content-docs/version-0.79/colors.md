...
React Native 中的元件[使用 JavaScript 進行樣式設定](style)。色彩屬性通常與[網頁上的 CSS 運作方式](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value)相符。各平台色彩使用的一般指南如下：
...
React Native 提供了多種色彩 API，旨在讓您充分利用平台的設計和使用者偏好。

- [PlatformColor](platformcolor) 讓您可以參考平台的色彩系統。
- [DynamicColorIOS](dynamiccolorios) 是 iOS 專用的 API，允許您指定在淺色或深色模式中應使用的色彩。
...
React Native 也支援以 `int` 值表示色彩（RGB 色彩模式）：
...
在 React Native 中，您也可以使用色彩名稱字串作為值。

:::info
React Native 僅支援小寫的色彩名稱。不支援大寫的色彩名稱。
:::
...
命名色彩的實現遵循 [CSS3/SVG 規範](https://www.w3.org/TR/css-color-3/#svg-color)：
