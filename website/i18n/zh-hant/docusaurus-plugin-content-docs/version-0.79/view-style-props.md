...
這些陰影可以組合使用，因此單個`boxShadow`可包含多個不同的陰影效果。

`boxShadow`接受模仿[網頁語法](https://developer.mozilla.org/en-US/docs/Web/CSS/box-shadow#syntax)的字串，或[BoxShadowValue](./boxshadowvalue)物件組成的陣列。
...
在 iOS 17+ 中，設置為`pointer`可在指針（如觸控板、Apple Pencil 或 visionOS 的視線追蹤）懸停於視圖時顯示懸停效果。
...
使用 Android 底層的[高程 API](https://developer.android.com/training/material/shadows-clipping.html#Elevation)設置視圖的立體高度。這會為項目添加投影效果，並影響重疊視圖的 z 軸排序。僅支援 Android 5.0+，對更早版本無效。
...
:::note
由於效能與規範兼容性問題，iOS 僅支援上述兩種濾鏡函數。未來計劃探索使用 SwiftUI 替代 UIKit 實現的解決方案。
:::
