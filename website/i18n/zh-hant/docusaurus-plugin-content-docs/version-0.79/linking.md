...
`Linking` 提供了一個通用介面來處理應用程式的傳入與傳出連結。

每個連結 (URL) 都有一個 URL Scheme，例如網站前綴的 `https://` 或 `http://` 中的 `http` 就是 URL Scheme。我們簡稱其為 scheme。

除了 `https`，您可能也熟悉 `mailto` scheme。當您開啟一個帶有 mailto scheme 的連結時，作業系統會開啟已安裝的郵件應用程式。同樣地，也有用於撥打電話和發送簡訊的 scheme。更多關於[內建 URL](#built-in-url-schemes) scheme 的資訊請見下文。

就像使用 mailto scheme 一樣，也可以透過自訂 url scheme 連結到其他應用程式。例如，當您收到 Slack 的 **Magic Link** 電子郵件時，**Launch Slack** 按鈕是一個錨點標籤，其 href 類似於：`slack://secret/magic-login/other-secret`。與 Slack 一樣，您可以告訴作業系統您希望處理自訂 scheme。當 Slack 應用程式開啟時，它會接收到用於開啟它的 URL。這通常被稱為深度連結 (deep linking)。更多關於如何[獲取深度連結](#get-the-deep-link)到您的應用程式的資訊請見下文。
...
您可以使用 `Linking.getInitialURL()` 來處理這些事件 - 它會回傳一個解析為 URL 的 Promise（如果有的話）。

---

## 範例

### 開啟連結與深度連結 (Universal Links)
...
此方法會回傳一個 `Promise` 物件。當系統判斷該 URL 是否能被處理後，Promise 會解析並回傳第一個參數表示是否能開啟。
...
> 當目標為 Android 11 (SDK 30) 時，您必須在 `AndroidManifest.xml` 中聲明要處理的 scheme 對應的 intent。[常見 intent 列表](https://developer.android.com/guide/components/intents-common)可在此找到。
>
> 例如要處理 `https` scheme，需在 manifest 中加入：
>
> ```
> <manifest ...>
>     <queries>
...
> 啟用遠端 JS 除錯時，getInitialURL 可能回傳 `null`。請停用除錯器以確保正常傳遞。
...
此方法會回傳一個 `Promise` 物件。若使用者確認開啟對話框或 URL 自動開啟，Promise 會解析。若使用者取消對話框或沒有應用程式能處理該 URL，Promise 會拒絕。
...
> 若系統不知如何開啟指定的 URL，此方法會失敗。若傳入非 http(s) 的 URL，建議先檢查 `canOpenURL()`。
