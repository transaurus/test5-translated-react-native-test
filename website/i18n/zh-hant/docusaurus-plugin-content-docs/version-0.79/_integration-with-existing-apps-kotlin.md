...
每當你建立一個新的 Activity 時，都需要將其加入你的 `AndroidManifest.xml` 檔案中。你還需要將 `MyReactActivity` 的主題設定為 `Theme.AppCompat.Light.NoActionBar`（或任何非 ActionBar 主題），否則你的應用程式將在 React Native 畫面上方渲染一個 ActionBar：
