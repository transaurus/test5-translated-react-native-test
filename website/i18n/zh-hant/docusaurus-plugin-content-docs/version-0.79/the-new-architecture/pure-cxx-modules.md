...
純 C++ Turbo 原生模組是 Turbo 原生模組。它們需要一個規格檔案（也稱為 spec 檔案），以便 Codegen 可以為我們建立框架程式碼。規格檔案也是我們在 JS 中存取 Turbo 原生模組的方式。

規格檔案需要使用類型化的 JS 方言編寫。React Native 目前支援 Flow 或 TypeScript。

1. 在應用程式的根資料夾中，建立一個名為 `specs` 的新資料夾。
...
此配置告訴 Codegen 在 `specs` 資料夾中尋找規格檔案。它還指示 Codegen 僅為 `modules` 生成程式碼，並將生成的程式碼命名為 `AppSpecs`。

## 3. 編寫原生程式碼

編寫 C++ Turbo 原生模組允許您在 Android 和 iOS 之間共享程式碼。因此，我們將編寫一次程式碼，並研究需要對平台進行哪些變更，以便可以提取 C++ 程式碼。
...
- 使用 `target_sources` 指令添加我們需要構建的模組 C++ 原始碼。預設情況下，React Native 已經會用預設原始碼填充 `appmodules` 庫，這裡我們包含我們自訂的原始碼。你可以看到我們需要從 `jni` 資料夾回溯到我們的 C++ Turbo Module 所在的 `shared` 資料夾。
...
首先我們需要執行每次準備 iOS 應用程式時的例行步驟。CocoaPods 是我們用來設定和安裝 React Native 相依套件的工具，在此過程中它也會為我們執行 Codegen。
...
最後一步是更新 `package.json`，告知 React Native 關於原生模組的 JS 規格與其原生程式碼實作之間的關聯。
...
:::warning
為了保持範例簡潔，我們直接在應用中導入規格檔案。
最佳實踐是建立一個單獨的檔案來封裝規格，並在應用中使用該檔案。
這讓您能更靈活地在 JS 端準備輸入參數並控制規格。
:::

恭喜，您已成功撰寫第一個 C++ Turbo 原生模組！
