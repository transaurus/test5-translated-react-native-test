...
在某些情況下，您可能需要讓原生模組監聽平台層的事件，並將其發送至 JavaScript 層，以便應用程式能對這些原生事件作出反應。另外，當有長時間運行的操作時，也可以發送事件來更新 UI。

這兩種情況都很適合從原生模組發送事件。本指南將教您如何實現。

## 當儲存空間新增鍵值時發送事件

在這個範例中，您將學習如何在儲存空間新增鍵值時發送事件。修改現有鍵值的內容不會觸發事件，只有新增鍵值才會。

本指南以[原生模組](/docs/next/turbo-native-modules-introduction)指南為基礎。
在深入學習本指南前，請確保熟悉該指南的內容，並實作其中的範例。

## 步驟 1：更新 NativeLocalStorage 的規格

第一步是更新 `NativeLocalStorage` 的規格，讓 React Native 知道這個模組可以發送事件。
...
當事件被發送時，您預期它會接收一個 `string` 類型的參數。

## 步驟 2：生成 Codegen

由於您已經更新了原生模組的規格，現在需要重新執行 Codegen 以在原生代碼中生成相關文件。
...
其餘的變更是一般的 React 調整，以配合這個新功能改進應用程式。
