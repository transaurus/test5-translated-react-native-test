...
清除所有客戶端、函式庫等的 _全部_ `AsyncStorage` 資料。通常不建議直接調用，應改用 `removeItem` 或 `multiRemove` 僅清除應用自身的鍵值。返回 `Promise` 物件。
...
用於批次儲存多組鍵值對的操作。操作完成時會透過單一回調返回所有錯誤：
...
批次合併指定鍵集的現有值與新值（假設值為字串化的JSON格式）。返回一個 `Promise` 物件。
