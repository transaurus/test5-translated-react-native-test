...
變換(Transforms)是能幫助您使用2D或3D轉換來修改組件外觀和位置的樣式屬性。但需注意，當您應用變換後，變換組件周圍的佈局會保持不變，因此可能會與相鄰組件產生重疊。您可以通過為變換組件添加邊距(margin)、調整相鄰組件或為容器設置內邊距(padding)來避免這類重疊問題。
...
- 第一個值(x軸偏移)必須為`px`、`percentage`或關鍵字`left`、`center`、`right`之一
- 第二個值(y軸偏移)必須為`px`、`percentage`或關鍵字`top`、`center`、`bottom`之一
...
`transformOrigin`也支援陣列語法，這使其能更方便地與Animated API配合使用。同時避免了字串解析，因此具有更高效率。
