...
`AppRegistry` 是所有 React Native 應用的 JavaScript 入口點。應用程式的根組件應通過 `AppRegistry.registerComponent` 註冊自身，之後原生系統可以載入應用的程式包，並在準備就緒時通過調用 `AppRegistry.runApplication` 實際運行應用。
...
若要「停止」應用程式（當視圖需要銷毀時），請調用 `AppRegistry.unmountApplicationComponentAtRootTag` 並傳入當初傳給 `runApplication` 的標籤。這兩者應始終配對使用。

`AppRegistry` 應在 `require` 序列的早期引入，以確保 JavaScript 執行環境在其他模組引入前已設置完成。
...
返回一個 [Registry](appregistry#registry) 物件。
...
返回一個 [Runnable](appregistry#runnable) 物件。
...
返回一個 [Runnables](appregistry#runnables) 物件。
...
註冊一個可取消的無介面任務。無介面任務是指無需使用者介面即可運行的程式碼片段。
...
註冊無介面任務。無介面任務是指無需使用者介面即可運行的程式碼片段。

此方法可用於在應用處於後台時執行 JavaScript 任務，例如同步最新數據、處理推送通知或播放音樂。
