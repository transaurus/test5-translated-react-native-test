...
- **Native Modules (原生模組)** - 沒有使用者介面(UI)的原生函式庫。例如持久儲存、通知、網路事件等。這些可透過函式與物件讓您的 JavaScript 應用程式程式碼存取。
- **Native Component (原生元件)** - 可透過 React 元件讓您的應用程式 JavaScript 程式碼使用的原生平台視圖。
