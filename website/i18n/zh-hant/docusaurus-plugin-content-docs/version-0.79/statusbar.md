...
對於使用元件不理想的情況，還有作為元件靜態函數公開的命令式 API。但是不建議同時使用靜態 API 和元件來設置相同的屬性，因為靜態 API 設置的任何值將在下一次渲染時被元件設置的值覆蓋。
...
:::warning
由於 Android 15 引入了邊到邊強制執行，在 API 級別 35 中設置狀態列為透明已被棄用，設置它將不會有任何效果。您可以在此處閱讀更多關於我們的[邊到邊建議](https://github.com/react-native-community/discussions-and-proposals/discussions/827)。
:::
