...
- 在設定了寬度/高度的節點上，寬高比控制未設定維度的大小
- 在設定了 flex basis 的節點上，寬高比控制未設定時節點在交叉軸上的大小
- 在具有測量函式的節點上，寬高比的作用如同測量函式測量 flex basis
- 在具有 flex grow/shrink 的節點上，寬高比控制未設定時節點在交叉軸上的大小
- 寬高比會考慮最小/最大尺寸限制
...
當 `flex` 為 0 時，組件的大小由 `width` 和 `height` 決定，且不具彈性。
...
`flexGrow` 描述容器內的主軸空間應如何分配給其子項。在排列子項後，容器會根據子項指定的 flex grow 值分配剩餘空間。
...
[`flexShrink`](layout-props#flexshrink) 描述當子項在主軸上的總大小超出容器大小時，如何沿主軸縮小子項。`flexShrink` 與 `flexGrow` 非常相似，可以將溢出的空間視為負的剩餘空間來理解。這兩個屬性配合使用，可以讓子項根據需要伸縮。
...
`justifyContent` 沿主軸方向對齊子元件。例如，若子元件是垂直排列，`justifyContent` 控制它們如何垂直對齊。其功能類似 CSS 中的 `justify-content`（預設值為 flex-start）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/justify-content 以獲取更多資訊。
...
其功能類似於 CSS 中的 `max-height`，但在 React Native 中必須使用點數或百分比。不支援 em 等其他單位。
...
`overflow` 控制子元件的測量和顯示方式。`overflow: hidden` 會裁剪視圖，而 `overflow: scroll` 會使視圖獨立於其父元件的主軸進行測量。其功能類似於 CSS 中的 `overflow`（預設值為 visible）。詳見 https://developer.mozilla.org/en/docs/Web/CSS/overflow 以獲取更多細節。
...
`static` 會根據佈局的正常流來定位元素。偏移量不會產生任何效果。`static` 元素不會為絕對定位的子元素形成包含區塊。

更多資訊，請參閱 [Flexbox 佈局文件](./flexbox.md#position)。此外，[Yoga 文件](https://www.yogalayout.dev/docs/styling/position) 詳細說明了 `position` 在 React Native 與 CSS 中的差異。
...
詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/top 以瞭解 `top` 如何影響版面配置。
...
其功能類似於 CSS 中的 `width`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/width 以獲取更多細節。
...
`zIndex` 控制哪些元件會顯示在其他元件之上。通常情況下不需要使用 `zIndex`，元件會依照文件樹中的順序渲染，後渲染的元件會覆蓋先前的元件。但在動畫或自訂模態介面等不希望遵循此行為的情境下，`zIndex` 會很有用。

其運作方式類似 CSS 的 `z-index` 屬性——數值較大的 `zIndex` 元件會渲染在上層。可將 z 軸方向想像成從手機螢幕指向您的眼球。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/z-index 以獲取更多細節。

在 iOS 平台上，要使 `zIndex` 正常運作，可能需要讓 `View` 元件彼此成為兄弟節點。
