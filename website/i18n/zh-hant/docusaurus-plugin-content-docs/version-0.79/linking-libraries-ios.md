...
並非所有應用程式都會使用到所有原生功能，若包含支援這些功能的程式碼會影響二進位檔案的大小...但我們仍希望在您需要時能支援添加這些功能。
...
如果函式庫包含原生程式碼，其資料夾內應該有一個 `.xcodeproj` 檔案。將此檔案拖曳到 Xcode 中的專案（通常在 Xcode 的 `Libraries` 群組下）；

![](/docs/assets/AddToLibraries.png)

#### 步驟 2

點擊您的主要專案檔案（代表 `.xcodeproj` 的那個），選擇 `Build Phases`，然後從您要導入的函式庫內的 `Products` 資料夾中拖曳靜態函式庫到 `Link Binary With Libraries`
