...
若所有設定皆正確，您應能很快在 iOS 模擬器中看到新應用程式運行。

## 指定裝置

您可使用 `--simulator` 參數指定模擬器運行的裝置名稱（字串形式），預設為 `"iPhone 14"`。若要在 iPhone SE（第三代）上運行應用程式，請執行以下指令：
...
裝置名稱對應 Xcode 中的可用裝置清單。您可透過終端機執行 `xcrun simctl list devices` 來查閱可用裝置。
