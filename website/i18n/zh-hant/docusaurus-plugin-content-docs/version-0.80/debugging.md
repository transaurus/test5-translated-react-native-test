...
當發生無法恢復的錯誤（例如 JavaScript 語法錯誤）時，LogBox 會開啟並顯示錯誤位置。在此狀態下，LogBox 無法關閉，因為您的程式碼無法執行。LogBox 會在語法錯誤修正後自動消失——無論是透過 Fast Refresh 或手動重新載入。
...
當 React Native DevTools 開啟時，除致命錯誤外的所有錯誤都會對 LogBox 隱藏。由於 LogBox 的各種選項可能隱藏或調整特定日誌的層級，我們建議使用 React Native DevTools 中的主控台面板作為主要參考來源。
...
在 Android 和 iOS 上，開發期間可透過開發者選單選擇 **「Perf Monitor」** 來切換應用內效能疊加層。詳細資訊請參閱[效能文件](/docs/performance)。

![iOS 和 Android 上的效能監控疊加層](/docs/assets/debugging-performance-monitor.jpg)
