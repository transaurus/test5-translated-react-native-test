...
Codegen 將生成您需要實現的 `ViewManager` 介面以及網頁視圖的 `ViewManager` 委託。
...
代碼隨後定義了一個輔助函數來實際發出事件。要發出事件，您必須：
...
當 React 從應用程式接收到渲染特定組件的指令時，React 使用註冊的視圖管理器來創建視圖並傳遞所有必需的屬性。
...
然後它覆寫了 `getName` 函數，該函數必須返回與 spec 的 `codegenNativeComponent` 函數調用中使用的名稱相同。
...
最後，若元件能觸發事件，您需要透過覆寫 `getExportedCustomBubblingEventTypeConstants`（針對冒泡事件）或 `getExportedCustomDirectEventTypeConstants`（針對直接事件）來映射事件名稱。

### 4. 撰寫 `ReactWebViewPackage`

如同處理原生模組時的做法，原生元件也需實作 `ReactPackage` 類別。此物件可用於將元件註冊至 React Native 運行環境中。
...
最後，您需在應用程式中註冊 `ReactWebViewPackage`。修改 `MainApplication` 檔案，將 `ReactWebViewPackage` 加入 `getPackages` 函數返回的套件清單中即可完成註冊。
