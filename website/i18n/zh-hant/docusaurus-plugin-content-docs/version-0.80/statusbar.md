...
對於不適合使用元件的情況，還有作為元件靜態函數公開的命令式 API。但不建議同時使用靜態 API 和元件來設置相同的屬性，因為靜態 API 設置的任何值將在下一次渲染時被元件設置的值覆蓋。
...
在 Android 上，這只會影響 API 版本 23 及以上。
...
:::warning
由於 Android 15 引入了邊到邊強制執行，在 API 級別 35 中設定狀態列背景顏色已被棄用，設定後將無效。您可以在[這裡](https://github.com/react-native-community/discussions-and-proposals/discussions/827)閱讀更多關於我們的邊到邊建議。
:::
...
:::warning
由於 Android 15 引入了邊到邊強制執行，在 API 級別 35 中設定狀態列為半透明已被棄用，設定後將無效。您可以在[這裡](https://github.com/react-native-community/discussions-and-proposals/discussions/827)閱讀更多關於我們的邊到邊建議。
:::
