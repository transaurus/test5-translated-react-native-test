...
通過原生元素暴露的兩個方法是 `.focus()` 和 `.blur()`，它們可以以編程方式聚焦或模糊 TextInput。

請注意，某些屬性僅在 `multiline={true/false}` 時可用。此外，僅應用於元素一側的邊框樣式（例如 `borderBottomColor`、`borderLeftWidth` 等）在 `multiline=true` 時不會應用。要實現相同的效果，您可以將 `TextInput` 包裹在一個 `View` 中：
...
`TextInput` 默認在其視圖底部有一個邊框。此邊框的填充由系統提供的背景圖像設置，無法更改。避免此問題的解決方案要麼不顯式設置高度（在這種情況下，系統將負責在正確的位置顯示邊框），要麼通過將 `underlineColorAndroid` 設置為透明來不顯示邊框。
...
若設為 `true`，文字欄位會在提交時失去焦點。單行欄位的預設值為 true，多行欄位則為 false。請注意，對於多行欄位，將 `blurOnSubmit` 設為 `true` 意味著按下返回鍵會使欄位失去焦點並觸發 `onSubmitEditing` 事件，而非在欄位中插入換行符號。
...
一個可選的標識符，用於將自定義的 [InputAccessoryView](inputaccessoryview.md) 鏈接到此文本輸入框。當此文本輸入框獲得焦點時，InputAccessoryView 會渲染在鍵盤上方。
...
一個可選的標籤，用於覆蓋默認的 [InputAccessoryView](inputaccessoryview.md) 按鈕標籤。

默認情況下，默認按鈕標籤未本地化。使用此屬性可提供本地化版本。
...
類似於 HTML 中的 `inputmode` 屬性，它決定打開哪種鍵盤，例如 `numeric`，並且優先於 `keyboardType`。
...
所有類型的截圖請參見[這裡](https://lefkowitz.me/2018/04/30/visual-guide-to-react-native-textinput-keyboardtype-options/)。
...
限制可輸入的最大字元數。使用此屬性替代在 JavaScript 中實作邏輯以避免閃爍問題。
...
:::note
需注意在 iOS 上此設定會使文字對齊頂部，而在 Android 上則會置中。若要實現跨平台一致行為，請搭配設定 `textAlignVertical` 為 `top`。
:::
...
當按鍵被按下時觸發的回調函數。會傳遞一個物件參數，其中 `keyValue` 會是 `'Enter'`（確認鍵）或 `'Backspace'`（刪除鍵），其他按鍵則會回傳實際輸入的字符（包含空格 `' '`）。此事件會在 `onChange` 回調之前觸發。注意：在 Android 上僅處理軟鍵盤輸入，不支援硬體鍵盤輸入。
...
在內容滾動時觸發。可能包含來自 `ScrollEvent` 的其他屬性，但基於效能考量，Android 上不會提供 `contentSize` 參數。
...
注意：在 iOS 上若使用 `keyboardType="phone-pad"` 時此方法不會被呼叫。
...
設定返回鍵的顯示標籤。可替代 `returnKeyType` 使用。
...
若為 `true`，允許 TextInput 將觸控事件傳遞給父元件。這使得如 SwipeableListView 等元件能從 TextInput 區域滑動（Android 預設即支援此行為）。若為 `false`，TextInput 會始終要求處理輸入事件（除非被禁用）。預設值為 `true`。
...
設為 `false` 時，聚焦欄位將阻止軟鍵盤彈出。預設值為 `true`。
...
設為 `false` 時，停用拼字檢查樣式（如紅色底線）。預設值繼承自 `autoCorrect`。
...
:::note
[`autoComplete`](#autocomplete) 提供相同的功能且支援所有平台。您可以使用 [`Platform.select`](/docs/next/platform#select) 來處理不同平台的行為差異。
...
在 iOS 上使用 `textContentType` 設為 `newPassword` 時，我們可以讓作業系統知道密碼的最低要求，以便生成符合條件的密碼。要建立有效的 `PasswordRules` 字串，請參閱 [Apple 官方文件](https://developer.apple.com/password-rules/)。
...
文字輸入框顯示的值。`TextInput` 是一個受控元件，這意味著如果提供了這個屬性，原生的值將會被強制匹配此值。在大多數情況下，這運作良好，但在某些情況下可能會導致閃爍——一個常見的原因是通過保持相同的值來防止編輯。除了設置相同的值外，可以設置 `editable={false}`，或者設置/更新 `maxLength` 來防止不需要的編輯而不會閃爍。
...
如果為 `true`，則禁用鍵盤快捷鍵（復原/重做和複製按鈕）。預設值為 `false`。
...
如果輸入框當前獲得焦點，則返回 `true`；否則返回 `false`。
