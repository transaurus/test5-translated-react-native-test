...
請先依照[設定開發環境](set-up-your-environment)指南與[不使用框架的 React Native](getting-started-without-a-framework)指南，配置您的開發環境以建構 Android 版 React Native 應用。
本指南同時假設您已熟悉 Android 開發基礎知識，例如建立 Activity 與編輯 `AndroidManifest.xml` 檔案。

## 1. 設定目錄結構

為確保流程順暢，請為整合式 React Native 專案建立新資料夾，並**將現有 Android 專案**移至 `/android` 子資料夾中。
...
安裝程序會建立新的 `node_modules` 資料夾，此資料夾儲存建構專案所需的所有 JavaScript 依賴套件。
...
首先，請編輯您的 `settings.gradle` 檔案並加入以下內容（參考[社群範本建議](https://github.com/react-native-community/template/blob/0.77-stable/template/android/settings.gradle)）：
...
接著您需要開啟頂層的 `build.gradle` 並加入此行（參考[社群範本建議](https://github.com/react-native-community/template/blob/0.77-stable/template/android/build.gradle)）：
...
此步驟確保 React Native Gradle 插件（RNGP）可在專案中使用。
最後，在您的應用程式 `build.gradle` 檔案中加入以下內容（此為不同的 `build.gradle` 檔案，通常位於 `app` 資料夾內 - 可參考[社群範本檔案](https://github.com/react-native-community/template/blob/0.77-stable/template/android/app/build.gradle)）：
...
最後，開啟您的應用程式 `gradle.properties` 檔案並加入以下內容（參考[社群範本檔案](https://github.com/react-native-community/template/blob/0.77-stable/template/android/gradle.properties)）：
...
首先，請確認您的 `AndroidManifest.xml` 中已啟用網路權限：
...
`index.js` 是 React Native 應用程式的起點，且始終是必需的。它可以是一個小檔案，僅 `import` 其他屬於你的 React Native 元件或應用程式的檔案，或者它可以包含所有需要的程式碼。
...
每當你建立一個新的 Activity 時，你需要將其加入你的 `AndroidManifest.xml` 檔案中。你還需要將 `MyReactActivity` 的主題設定為 `Theme.AppCompat.Light.NoActionBar`（或任何非 ActionBar 的主題），否則你的應用程式將在 React Native 畫面上方渲染一個 ActionBar：
...
一旦你在應用程式中進入由 React 驅動的 Activity，它應該會從開發伺服器加載 JavaScript 代碼並顯示：
