...
請注意，若要在**Android**上正常運作，需透過`UIManager`設置以下標記：
...
- `duration`：以毫秒為單位
- `create`：用於動畫新增視圖的選用配置
- `update`：用於動畫更新視圖的選用配置
- `delete`：用於動畫移除視圖的選用配置
...
輔助函數，創建一個包含`create`、`update`和`delete`欄位的物件，用於傳入[`configureNext`](layoutanimation.md#configurenext)。`type`參數是[動畫類型](layoutanimation.md#types)，`creationProp`參數是[佈局屬性](layoutanimation.md#properties)。
