...
驅動動畫的標準值。一個 `Animated.Value` 可以同步驅動多個屬性，但同一時間只能由一種機制驅動。使用新的機制（例如啟動新動畫或調用 `setValue`）將會停止先前的所有機制。
...
設定一個偏移量，該偏移量會加在無論是通過 `setValue`、動畫還是 `Animated.event` 設定的數值之上。適用於補償平移手勢的起始點等情況。
...
將偏移量設定為基礎數值，並將基礎數值重置為零。數值的最終輸出保持不變。
