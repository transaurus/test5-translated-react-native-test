...
:::note
本指南假設您已熟悉[**純 C++ Turbo 原生模組**](pure-cxx-modules.md)指南。本文將以此為基礎延伸說明。
:::
...
由於 JavaScript 不支援大於 2^53 的數值，C++ Turbo 原生模組目前尚未支援 `int64_t` 數字。若要表示大於 2^53 的數值，可在 JS 中使用 `string` 型別，並自動轉換為 C++ 的 `int64_t`。
...
接著可修改 JS 規格，新增使用此新型別的方法。與往常相同，我們可使用 Flow 或 TypeScript 撰寫規格。
...
首先在 JS 規格中定義新的自定義類型，讓 Codegen 自動生成所有支援代碼，這樣就不需手動編寫。
...
此代碼定義了新的 `Address` 類型，並為 Turbo Native Module 新增了 `validateAddress` 函數。注意 `validateFunction` 需要一個 `Address` 物件作為參數。
...
2. 打開 `shared/NativeSampleModule.cpp` 文件並加入函數實現
...
在實現中，代表 `Address` 的物件是一個 `jsi::Object`。要從此物件提取值，需使用 `JSI` 提供的訪問器：
...
:::note
若想深入了解 `JSI` 運作原理，可參考 [此精彩演講](https://youtu.be/oLmGInjKU2U?feature=shared)（App.JS 2024）
:::

### 4. 在應用中測試代碼

要在應用程式中測試程式碼，我們需要修改 `App.tsx` 檔案。
