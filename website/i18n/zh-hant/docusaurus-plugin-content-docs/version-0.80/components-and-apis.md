...
React Native 提供了許多內建的[核心組件](intro-react-native-components)，可直接在應用程式中使用。您可以在左側邊欄（或螢幕頂部選單，若使用窄螢幕裝置）找到所有組件。若不確定從何開始，請參考以下分類：
...
- [其他組件](components-and-apis#others)

您不僅限於使用 React Native 內建的組件與 API。React Native 擁有數千名開發者組成的社群。若需尋找特定功能的函式庫，請參閱[函式庫尋找指南](libraries#finding-libraries)。

## 基礎組件

大多數應用程式最終會使用以下一個或多個基礎組件。
...
與通用型 [`ScrollView`](./scrollview) 不同，以下列表視圖組件僅渲染當前顯示在螢幕上的元素。這使其成為顯示長數據列表的高效能選擇。
...
這些組件可能對特定應用程式有所助益。如需完整組件與 API 清單，請查看左側邊欄（或螢幕頂部選單，若使用窄螢幕裝置）。
