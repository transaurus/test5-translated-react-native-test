...
- `setImmediate/setTimeout()` 稍後運行代碼，請注意這可能會延遲動畫。
...
預設情況下，排隊的任務會在一個 `setImmediate` 批次中一起執行。如果使用正數調用 `setDeadline`，則任務只會在截止時間（以 js 事件循環運行時間為準）接近時執行，此時執行會透過 setTimeout 讓步，允許觸控等事件開始互動並阻止排隊任務執行，使應用程式更具響應性。
