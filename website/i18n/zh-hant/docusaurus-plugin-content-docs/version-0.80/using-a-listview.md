...
此範例創建了一個基於硬編碼數據的基本 `FlatList`。`data` 屬性中的每個項目都會被渲染為一個 `Text` 元件。`FlatListBasics` 元件隨後會渲染 `FlatList` 及其所有 `Text` 元件。
...
列表視圖最常見的用途之一是顯示從伺服器獲取的數據。要實現這一點，您需要先 [學習 React Native 中的網路請求處理](network.md)。