...
您可以透過將堆疊追蹤與產生的原始碼對應檔傳遞給 [`metro-symbolicate`](http://npmjs.com/package/metro-symbolicate)，來符號化上述最小化函式名稱與位元組碼。

### 啟用原始碼對應檔

符號化堆疊追蹤需要原始碼對應檔。請確認在目標平台的建置設定中已啟用原始碼對應檔。
