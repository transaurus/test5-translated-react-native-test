...
`Systrace` 允許你用標籤和整數值標記 JavaScript (JS) 事件。在 EasyProfiler 中捕獲非計時的 JS 事件。
...
beginAsyncEvent/endAsyncEvent 用於開始和結束分析，其中結束可以發生在另一個執行緒或當前堆疊框架之外，例如 await 返回的 cookie 變量應作為輸入傳入 endAsyncEvent 呼叫以結束分析。
