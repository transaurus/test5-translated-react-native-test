...
`requestAnimationFrame(fn)` 與 `setTimeout(fn, 0)` 不同——前者會在所有畫面更新完成後觸發，而後者會盡可能快速地觸發（在 iPhone 5S 上每秒超過 1000 次）。
...
優秀的原生應用之所以感覺流暢，一個重要原因是它們在互動和動畫期間避免執行耗費資源的操作。在 React Native 中，目前有一個限制是只有單一的 JS 執行緒，但你可以使用 `InteractionManager` 來確保長時間運行的任務被安排在互動/動畫完成後才開始。
