...
就像使用 mailto scheme 一樣，可以透過自訂 URL scheme 連結到其他應用程式。例如，當你收到 Slack 的 **Magic Link** 郵件時，**Launch Slack** 按鈕實際上是一個帶有類似 `slack://secret/magic-login/other-secret` 這樣 href 屬性的錨點標籤。與 Slack 類似，你可以告訴作業系統你希望處理某個自訂 scheme。當 Slack 應用程式開啟時，它會接收到用於啟動它的 URL。這通常被稱為深度連結 (deep linking)。更多關於如何[獲取深度連結](#get-the-deep-link)到你的應用程式中的資訊請見下文。

自訂 URL scheme 並不是在行動裝置上開啟應用程式的唯一方式。例如，如果你想透過電子郵件發送一個在行動裝置上開啟的連結，使用自訂 URL scheme 並不理想，因為使用者可能在桌面上開啟郵件，而連結在那裡無法工作。相反，你應該使用標準的 `https` 連結，例如 `https://www.myapp.io/records/1234546`。在行動裝置上，這些連結可以配置為開啟你的應用程式。在 Android 上，此功能稱為 **Deep Links**，而在 iOS 上則稱為 **Universal Links**。
...
#### 2. 如果應用程式尚未開啟，它會被啟動並且 URL 會作為 initialURL 傳入
...
透過監聽 `url` 事件類型並提供處理函式，為 Linking 變更添加一個處理器。
...
此方法回傳一個 `Promise` 物件。當系統判斷該 URL 是否能被處理時，Promise 會被解析，且第一個參數會表示該 URL 是否能被開啟。
...
>
> 例如，若要處理 `https` scheme，需在 manifest 中加入以下內容：
>
> ```
> <manifest ...>
>     <queries>
...
若應用程式是由應用程式連結所觸發啟動，則會回傳該連結的 URL，否則回傳 `null`。

> 如需在 Android 上支援深層連結，請參考 https://developer.android.com/training/app-indexing/deep-linking.html#handling-intents
...
此方法回傳一個 `Promise` 物件。若使用者確認開啟對話框或 URL 自動開啟，Promise 會被解析。若使用者取消開啟對話框或沒有已註冊的應用程式可處理該 URL，Promise 會被拒絕。
...
> 若系統不知道如何開啟指定的 URL，此方法會失敗。若你傳入的是非 http(s) 的 URL，最好先檢查 `canOpenURL()`。
