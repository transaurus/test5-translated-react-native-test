...
返回一個布林值，表示裝置是否為 Apple Vision。_若您使用的是 [Apple Vision Pro (專為 iPad 設計)](https://developer.apple.com/documentation/visionos/checking-whether-your-app-is-compatible-with-visionos)，`isVision` 會是 `false`，但 `isPad` 會是 `true`_
...
這會讓容器在所有平台上擁有 `flex: 1`，在 Android 上擁有綠色背景，在 iOS 上擁有紅色背景，在其他平台上則擁有藍色背景。

由於對應平台鍵的值可以是 `any` 類型，[`select`](platform.md#select) 方法也可用於返回平台特定的元件，如下所示：
