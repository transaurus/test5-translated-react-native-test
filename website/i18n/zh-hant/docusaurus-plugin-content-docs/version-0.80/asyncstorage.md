...
根據 `key` 取得項目並於完成時觸發回呼函式。回傳 `Promise` 物件。
...
設定 `key` 對應的值並於完成時觸發回呼函式。回傳 `Promise` 物件。
...
移除 `key` 對應的項目並於完成時觸發回呼函式。回傳 `Promise` 物件。
...
清除 _所有_ `AsyncStorage` 資料（含所有客戶端、函式庫等）。通常不建議直接呼叫此方法，應改用 `removeItem` 或 `multiRemove` 僅清除應用程式自身的鍵值。回傳 `Promise` 物件。
...
取得應用程式 _所有_ 已知的鍵值（含所有呼叫端、函式庫等）。回傳 `Promise` 物件。
...
使用此方法批次儲存多個鍵值對。操作完成時，您將收到一個包含所有錯誤的單一回調：
...
批次操作，用於合併指定鍵集的現有值與新值。此操作假設值為字串化的 JSON。返回一個 `Promise` 物件。
