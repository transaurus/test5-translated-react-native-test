...
請注意，ScrollView 必須有固定高度才能正常運作，因為它將無限高度的子元件裝載到有限容器中（透過滾動互動）。要限制 ScrollView 的高度，可以直接設定視圖高度（不建議）或確保所有父視圖都有固定高度。忘記將 `{flex: 1}` 傳遞至下層視圖結構可能導致錯誤，此時元素檢查工具能快速除錯。
...
假設您需要顯示非常長的項目列表，可能橫跨多個螢幕。一次性為所有內容建立 JS 元件和原生視圖（其中許多可能根本不會顯示），將導致渲染變慢和記憶體用量增加。

這時就該使用 `FlatList`。`FlatList` 會惰性渲染項目（僅在即將顯示時才渲染），並移除滾出畫面的項目以節省記憶體和運算資源。
...
用於渲染黏性標頭的 React 元件，需與 `stickyHeaderIndices` 搭配使用。若黏性標頭需自訂變形效果（例如帶有動畫效果的可隱藏標頭），則需設定此元件。若未提供元件，將使用預設的 [`ScrollViewStickyHeader`](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Components/ScrollView/ScrollViewStickyHeader.js) 元件。
...
設為 true 時，即使內容小於滾動視圖本身，滾動至水平邊界時仍會彈跳。
...
設為 true 時，即使內容小於滾動視圖本身，滾動至垂直邊界時仍會彈跳。
...
當設為 `true` 時，手勢操作可驅動縮放超出最小/最大值限制，並在手勢結束時動畫回彈至限制值；否則縮放不會超過限制範圍。
...
當滾動視圖空間大於內容區域時，此屬性可指定填充剩餘空間的顏色，避免設定背景色造成不必要的過度繪製。此為進階優化選項，一般情況下無需使用。
...
控制黏性標頭應固定在滾動視圖底部而非頂部，通常與反向滾動視圖（inverted ScrollViews）搭配使用。
...
可選參數 `autoscrollToTopThreshold` 可在調整後，若使用者原本位於頂部閾值範圍內時，自動將內容滾動至頂部。這同樣適用於聊天類應用，希望新訊息能平滑滾入，但避免在使用者已向上滾動較多時造成干擾性大幅滾動。
...
限制滾動事件觸發的頻率，以毫秒為單位指定時間間隔。當滾動時需要執行昂貴操作時，這可能很有用。值 ≤ `16` 將禁用節流，無論設備的刷新率如何。
...
用於記錄此滾動視圖性能的標籤。將強制開啟動量事件（參見sendMomentumEvents）。此功能預設不生效，需實作自訂的原生FpsListener才能發揮作用。
...
當設為`true`時，滾動視圖可通過程式方式滾動超出其內容大小。
...
設置後，會使滾動視圖在`snapToInterval`值的倍數處停止。可用於分頁瀏覽長度小於滾動視圖的子項目。通常與`snapToAlignment`和`decelerationRate="fast"`配合使用。會覆蓋較不靈活的`pagingEnabled`屬性。
...
設置後，會使滾動視圖在指定的偏移量處停止。可用於分頁瀏覽大小不一且長度小於滾動視圖的子項目。通常與`decelerationRate="fast"`配合使用。會覆蓋較不靈活的`pagingEnabled`和`snapToInterval`屬性。
...
與`snapToOffsets`配合使用。預設情況下，列表起始處會被視為對齊點。將`snapToStart`設為`false`可禁用此行為，允許列表在其起始處與第一個`snapToOffsets`對齊點之間自由滾動。
...
當設為`true`時，向下滾動列表時會隱藏黏性標頭，向上滾動時會將標頭固定在列表頂部。
...
一個子元素索引陣列，決定哪些子元素在滾動時會固定在畫面頂部。例如，傳入 `stickyHeaderIndices={[0]}` 會讓第一個子元素固定在滾動視圖頂部。也可以使用 [x,y,z] 格式讓多個元素在到達頂部時保持固定。此屬性不支援與 `horizontal={true}` 同時使用。
...
> 注意：此函數簽名的奇怪設計是由於歷史原因，它同時支援以獨立參數替代選項物件。這種用法因歧義性（y 在 x 之前）已被棄用，不應繼續使用。
