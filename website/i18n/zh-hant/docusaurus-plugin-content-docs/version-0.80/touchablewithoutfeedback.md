...
`TouchableWithoutFeedback` 僅支援單個子元素。若需包含多個子元件，請將其包裹在 View 中。需特別注意，`TouchableWithoutFeedback` 的工作原理是克隆其子元素並套用響應屬性，因此任何中介元件都必須將這些屬性傳遞給底層的 React Native 元件。
...
此值決定當顏色反轉功能開啟時，此視圖是否應被反轉。設為 `true` 時，即使顏色反轉開啟，視圖也不會被反轉。
...
當設為 `true` 時，表示該視圖是一個無障礙元素。預設情況下，所有可觸碰元素都具有無障礙功能。
...
當無障礙標籤無法明確表達操作結果時，無障礙提示可幫助使用者理解在無障礙元素上執行操作會發生的結果。
...
無障礙操作允許輔助技術以程式化方式觸發元件的動作。`accessibilityActions`屬性應包含一組動作物件，每個動作物件應包含名稱和標籤欄位。
...
表示元素將被更新，並描述用戶代理、無障礙技術和用戶可以從實時區域獲得的更新類型。
...
當觸碰釋放時呼叫，但若被取消（例如被搶佔回應鎖的滾動）則不呼叫。第一個函數參數是 [PressEvent](pressevent) 形式的事件。
...
當滾動視圖被停用時，定義觸碰可以移出按鈕多遠的距離才會停用按鈕。一旦停用，嘗試移回按鈕會發現按鈕再次被啟用！在滾動視圖停用時來回移動多次。請確保傳入常數以減少記憶體配置。
