...
> 雖然尺寸資訊可以立即取得，但它們可能會發生變化（例如由於裝置旋轉、摺疊式裝置等），因此任何依賴這些常數的渲染邏輯或樣式，都應該在每次渲染時調用此函數，而不是緩存該值（例如，使用內聯樣式而不是在 `StyleSheet` 中設定值）。
...
初始尺寸在 `runApplication` 被調用之前就已設定，因此它們應該在其他任何 require 執行之前就可取得，但之後可能會更新。
