...
作為基礎元件，用於透過鍵盤在應用程式中輸入文字。其屬性可配置多項功能，例如自動校正、自動大寫、佔位文字，以及不同類型的鍵盤（如數字鍵盤）。

最基礎的使用方式是放置一個 `TextInput` 並訂閱 `onChangeText` 事件來讀取使用者輸入。此外也可訂閱其他事件如 `onSubmitEditing` 和 `onFocus`。以下為最小範例：
...
`TextInput` 預設在視圖底部有邊框。此邊框的內距由系統提供的背景圖設定，無法修改。解決方案是避免明確設置高度（系統會自動正確顯示邊框位置），或透過將 `underlineColorAndroid` 設為透明來隱藏邊框。
...
為系統指定自動完成提示，以便提供自動填充功能。在 Android 上，系統始終會嘗試透過啟發式識別內容類型來提供自動填充。要停用自動完成，請將 `autoComplete` 設為 `off`。
...
當設為 `false` 時，若文字輸入周圍空間有限（例如手機橫向模式），作業系統可能會讓使用者在全螢幕文字輸入模式中編輯文字。設為 `true` 時會停用此功能，使用者將始終直接在文字輸入框中編輯。預設值為 `false`。
...
限制可輸入的最大字元數。使用此屬性而非在 JavaScript 中實作邏輯，以避免閃爍問題。
...
:::note
需注意：在 iOS 上此設定會使文字對齊頂部，而在 Android 上會置中。若要兩平台行為一致，請搭配設定 `textAlignVertical` 為 `top`。
:::
...
當按鍵被按下時觸發的回調函數。會傳入一個物件，其中 `keyValue` 為 `'Enter'`（Enter鍵）或 `'Backspace'`（退格鍵），其他按鍵則回傳輸入的字符（包含空格 `' '`）。此回調會在 `onChange` 之前觸發。注意：在 Android 上僅處理軟鍵盤輸入，不處理硬體鍵盤輸入。
...
在內容滾動時觸發。可能包含 `ScrollEvent` 的其他屬性，但在 Android 上因效能考量不提供 `contentSize`。
...
設定選取把手的顏色。與 `selectionColor` 不同，此屬性可獨立於選取範圍顏色自訂把手色彩。
...
設為 `false` 時可防止欄位聚焦時顯示虛擬鍵盤。預設值為 `true`。
...
`newPassword` 可用於標示使用者可能想儲存至鑰匙圈的新密碼輸入欄位，而 `oneTimeCode` 則可用於標示可透過簡訊接收的一次性驗證碼自動填入欄位。
...
文字輸入框顯示的值。`TextInput` 是一個受控元件，這意味著如果提供了這個屬性，原生值將被強制與之匹配。在大多數情況下，這運作良好，但在某些情況下可能會導致閃爍——一個常見的原因是通過保持值不變來防止編輯。除了設置相同的值外，可以設置 `editable={false}`，或者設置/更新 `maxLength` 來防止不需要的編輯而不會閃爍。
