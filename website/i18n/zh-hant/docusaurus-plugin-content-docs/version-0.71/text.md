...
Android 和 iOS 都允許您通過為字串的特定範圍添加註釋來顯示格式化文字，例如粗體或彩色文字（iOS 上的 `NSAttributedString`，Android 上的 `SpannableString`）。實際上，這非常繁瑣。對於 React Native，我們決定使用網頁範式來實現相同的效果，您可以通過嵌套文字來達成。
...
您也失去了為整個子樹設定預設字體的能力。同時，`fontFamily` 只接受單一字體名稱，這與 CSS 中的 `font-family` 不同。建議在應用程式中使用一致的字體和大小的方式是創建一個包含這些樣式的元件 `MyAppText`，並在整個應用程式中使用此元件。您還可以使用此元件來創建更具體的元件，例如 `MyAppHeaderText` 用於其他類型的文字。
...
- （實現者）React Native 的實現也得到了簡化。我們不需要在每個元素上都有 `fontFamily` 字段，也不需要每次顯示文字節點時都遍歷樹直到根節點。樣式繼承僅編碼在原生 Text 元件內部，不會洩漏到其他元件或系統本身。
...
指定螢幕閱讀器與元素互動時應使用的語言值。需遵循 [BCP 47 規範](https://www.rfc-editor.org/info/bcp47)。
...
指示螢幕閱讀器將當前聚焦的元素視為處於特定狀態。

可提供單一狀態、無狀態或多重狀態。狀態需透過物件形式傳遞，例如：`{selected: true, disabled: true}`。
...
當使用者觸發輔助功能操作時呼叫。此函式的唯一參數是一個包含待執行操作名稱的事件物件。
...
設定 Android API 23+ 上決定斷字時的自動連字號使用頻率。
...
- `head` - 行尾顯示完整內容，行首被截斷部分以省略號表示。例：「...wxyz」
- `middle` - 行首行尾顯示完整內容，中間被截斷部分以省略號表示。例：「ab...yz」
- `tail` - 行首顯示完整內容，行尾被截斷部分以省略號表示。例：「abcd...」
- `clip` - 不繪製超出文字容器邊界的內容。
...
如果父級 `View` 想要阻止子級 `View` 在觸摸開始時成為響應者，它應該設置此處理器並返回 `true`。
...
當滾動視圖被禁用時，此屬性定義了觸摸點可以偏離按鈕多遠才會停用按鈕。一旦停用，嘗試將觸摸點移回按鈕範圍內，您會看到按鈕再次被激活！在滾動視圖禁用時來回移動觸摸點多次。建議傳入常數以減少記憶體分配。
...
`role` 向輔助技術的使用者傳達組件的用途。此屬性優先於 [`accessibilityRole`](text#accessibilityrole) 屬性。
...
當設為 `true` 時，文字被按下時不會有視覺變化。預設情況下，按下文字時會顯示灰色橢圓形高亮。
...
在 Android API 等級 23+ 上設置文字斷行策略，可能的值為 `simple`、`highQuality`、`balanced`。
...
`TextLayoutEvent` 物件在組件佈局變更時作為回調結果返回。它包含一個名為 `lines` 的鍵，其值是一個數組，包含與每個渲染的文字行對應的 [`TextLayout`](text#textlayout) 物件。
