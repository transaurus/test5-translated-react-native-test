...
`PressRect` 允許按壓在維持激活狀態下，移動超出元件及其 `HitRect` 範圍，同時仍符合「按壓」資格——想像您按住按鈕時慢慢滑開手指的情境。

> 觸控區域絕不會延伸超出父視圖邊界，且若觸控同時命中兩個重疊視圖，兄弟視圖的 Z-index 永遠優先。
...
> `Pressable` 使用 React Native 的 `Pressability` API。有關 Pressability 狀態機流程及其運作方式的更多資訊，請查閱 [Pressability](https://github.com/facebook/react-native/blob/16ea9ba8133a5340ed6751ec7d49bf03a0d4c5ea/Libraries/Pressability/Pressability.js#L347) 的實作程式碼。
...
在 `onPressOut` 觸發前，視為按壓的觸碰可超出此視圖的額外距離。
