...
在 iOS 平台自訂背景色。當開關值為 `false` 或開關被禁用時（且開關呈半透明狀態），可看到此背景色。
...
開關滑塊的顏色。在 iOS 平台設定此屬性會使滑塊失去陰影效果。
...
_iOS_: 當開關值為 `false` 時，軌道會向邊框收縮。若要調整收縮後露出的背景色，請使用 [`ios_backgroundColor`](switch.md#ios_backgroundColor)。
