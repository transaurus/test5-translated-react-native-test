...
除非有充分理由，否則請勿使用。所有響應按壓的元件在觸碰時都應具有視覺反饋。

`TouchableWithoutFeedback` 僅支援單一子元件。若需包含多個子元件，請將它們包裹在一個 View 中。重要的是，`TouchableWithoutFeedback` 的工作原理是克隆其子元件並應用響應屬性。因此，任何中介元件都必須將這些屬性傳遞給底層的 React Native 元件。
...
當值為 `true` 時，表示該視圖是一個無障礙元素。預設情況下，所有可觸碰元件都是無障礙的。
...
指定螢幕閱讀器在使用者與元件互動時應使用的語言。該值應遵循 [BCP 47 規範](https://www.rfc-editor.org/info/bcp47)。
...
如果在 `onPressIn` 之後持續超過 370 毫秒，則呼叫此函數。此時間段可透過 [`delayLongPress`](#delaylongpress) 自訂。
...
當觸碰釋放時呼叫，但如果被取消（例如因滾動搶佔了回應者鎖）則不呼叫。第一個函數參數是一個 [PressEvent](pressevent) 形式的事件。
...
當滾動視圖被停用時，此值定義觸碰可以移出按鈕多遠的距離才會停用按鈕。一旦停用，嘗試移回按鈕，您會看到按鈕再次被啟用！在滾動視圖停用時來回移動幾次。請傳入常數以減少記憶體分配。
