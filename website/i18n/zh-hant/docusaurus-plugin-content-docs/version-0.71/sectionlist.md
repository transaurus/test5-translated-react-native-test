...
如果您不需要分區支援且想要更簡單的介面，請使用 [`<FlatList>`](flatlist.md)。
...
- 為了限制記憶體使用並實現平滑滾動，內容會異步在螢幕外渲染。這意味著可能會滾動得比填充速度更快，並暫時看到空白內容。這是一個可以根據每個應用需求調整的權衡，我們正在幕後努力改進它。
...
列表底部邊緣距離內容末端多遠（以列表可見長度為單位）時，會觸發 `onEndReached` 回呼。因此，值為 0.5 時，當內容末端位於列表可見長度的一半以內時，會觸發 `onEndReached`。
...
在每個區段的頂部和底部渲染（注意這與 `ItemSeparatorComponent` 不同，後者僅在項目之間渲染）。這些旨在將區段與上方和下方的標頭分開，通常具有與 `ItemSeparatorComponent` 相同的高亮響應。也會接收 `highlighted`、`[leading/trailing][Item/Section]` 以及來自 `separators.updateProps` 的任何自定義屬性。
...
讓區段標頭固定在螢幕頂部，直到下一個標頭將其推離。預設僅在 iOS 上啟用，因為這是該平台的標準。
...
通知列表發生了互動，這應該觸發可見性計算，例如如果 `waitForInteractions` 為 true 且用戶尚未滾動。通常由點擊項目或導航操作調用。
