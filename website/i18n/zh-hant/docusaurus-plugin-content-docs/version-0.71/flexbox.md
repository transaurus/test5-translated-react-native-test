...
:::caution
Flexbox 在 React Native 中的運作方式與在 CSS 網頁上相同，但有一些例外。
...
在以下範例中，紅色、橙色和綠色視圖都是設有 `flex: 1` 的容器視圖的子元素。紅色視圖使用 `flex: 1`，橙色視圖使用 `flex: 2`，綠色視圖使用 `flex: 3`。**1+2+3 = 6**，這意味著紅色視圖將獲得 `1/6` 的空間，橙色視圖獲得 `2/6` 的空間，綠色視圖獲得 `3/6` 的空間。
...
[`flexDirection`](layout-props#flexdirection) 控制節點子元素的排列方向。這也被稱為主軸。交叉軸是與主軸垂直的軸，或是換行排列的軸。
...
佈局 [`direction`](layout-props#direction) 指定了層次結構中子元素和文字的排列方向。佈局方向也會影響 `start` 和 `end` 所指向的邊緣。預設情況下，React Native 使用 LTR（從左到右）佈局方向。在此模式下，`start` 指向左側，`end` 指向右側。
...
- `space-around` 在主軸上均勻分布子元素，剩餘空間分配在子元素周圍。與 `space-between` 相比，使用 `space-around` 會在第一個子元素之前和最後一個子元素之後也分配空間。
...
[`alignItems`](layout-props#alignitems) 描述了如何沿著容器的交叉軸對齊子元素。它與 `justifyContent` 非常相似，但 `alignItems` 適用於交叉軸而非主軸。
...
- `baseline` 將容器的子元素沿著共同的基線對齊。可以為每個子元素設置其父元素的參考基線。

:::info
要使 `stretch` 生效，子元素在次要軸上不能有固定尺寸。在以下範例中，設置 `alignItems: stretch` 不會有任何效果，除非從子元素中移除 `width: 50`。
:::
...
[`alignSelf`](layout-props#alignself) 的選項和效果與 `alignItems` 相同，但它不是影響容器內的所有子元素，而是可以應用於單個子元素以改變其在父元素內的對齊方式。`alignSelf` 會覆蓋父元素通過 `alignItems` 設置的任何選項。
...
您可以將 `flexWrap` 和 `alignContent` 與 `gap` 一起使用，以在項目之間添加一致的間距。
...
- `auto`（**預設值**）React Native 會根據元素的內容（無論是其他子元素、文字還是圖片）計算元素的寬度/高度。
...
- `relative` (**預設值**) 預設情況下，元素會採用相對定位。這表示元素會依照正常的版面配置流程進行定位，然後根據 `top`、`right`、`bottom` 和 `left` 的值進行相對偏移。這種偏移不會影響其他兄弟元素或父元素的位置。

- `absolute` 當元素採用絕對定位時，它不會參與正常的版面配置流程。相反地，它會獨立於其兄弟元素進行定位。其位置是根據 `top`、`right`、`bottom` 和 `left` 的值來決定的。
...
查看互動式的 [yoga 遊樂場](https://www.yogalayout.dev/playground)，你可以透過它更深入理解 flexbox 的運作方式。

我們已經涵蓋了基礎知識，但還有許多其他你可能需要用到的版面樣式。控制版面的完整屬性列表可以在[這裡](./layout-props.md)找到相關文件。
