...
此範例展示如何從本地儲存空間獲取並顯示圖片，以及從網路甚至使用 `'data:'` URI 方案提供的數據來顯示圖片。
...
> 注意：上述列出的版本可能未及時更新。請查看主倉庫中的 [`ReactAndroid/gradle.properties`](https://github.com/facebook/react-native/blob/0.71-stable/ReactAndroid/gradle.properties)，以了解特定標記版本中使用的 fresco 版本。
...
定義圖片替代文字描述的字符串，當用戶與圖片互動時，屏幕閱讀器將讀取此文字。使用此屬性會自動將此元素標記為可訪問。
...
當圖片被調整大小時，由 `capInsets` 指定大小的角落將保持固定大小，但圖片的中心內容和邊框將被拉伸。這對於創建可調整大小的圓角按鈕、陰影和其他可調整大小的資源非常有用。更多信息請參閱 [官方 Apple 文檔](https://developer.apple.com/library/ios/documentation/UIKit/Reference/UIImage_Class/index.html#//apple_ref/occ/instm/UIImage/resizableImageWithCapInsets)。
...
在圖片部分載入完成時觸發。「部分載入」的定義取決於載入器，但此功能主要用於漸進式 JPEG 載入。
...
- `scale`：圖片會被縮小或放大繪製。與 `resize` 相比，`scale` 更快（通常由硬體加速）且產生更高品質的圖片。如果圖片小於視圖，應使用此方法；若圖片略大於視圖，也應使用此方法。
...
此屬性也可以包含多個遠端 URL，並指定它們的寬度和高度，以及可能的縮放比例/其他 URI 參數。原生端將根據圖片容器的測量大小選擇最佳的 `uri` 來顯示。可以添加 `cache` 屬性來控制網路請求如何與本地緩存互動（更多資訊請參閱[圖片的快取控制（僅限 iOS）](images#cache-control-ios-only)）。
...
一個字串，表示以逗號分隔的候選圖片來源列表。每個圖片來源包含一個圖片的 URL 和一個像素密度描述符。如果未指定描述符，則預設為 `1x` 描述符。
...
為了獲取圖片的尺寸，可能需要先加載或下載圖片，之後圖片會被緩存。這意味著原則上您可以使用此方法來預加載圖片，但此方法並非為此目的而優化，且未來可能會以不完全加載/下載圖片數據的方式實現。預加載圖片的正確、支持方式將作為單獨的API提供。
...
在顯示圖片之前獲取其寬度和高度（以像素為單位），並能為請求提供標頭。如果無法找到圖片或下載失敗，此方法可能會失敗。此方法也不適用於靜態圖片資源。
...
- `force-cache`: 無論快取資料的時效性或過期日期為何，都將使用現有的快取資料來滿足請求。如果快取中沒有對應請求的資料，則從原始來源載入資料。
- `only-if-cached`: 無論快取資料的時效性或過期日期為何，都將使用現有的快取資料來滿足請求。如果快取中沒有對應 URL 載入請求的資料，則不會嘗試從原始來源載入資料，且該載入將視為失敗。
