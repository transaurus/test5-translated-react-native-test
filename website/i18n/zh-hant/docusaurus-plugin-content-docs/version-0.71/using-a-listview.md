...
這個範例建立了一個基本的 `FlatList`，使用硬編碼的資料。`data` 屬性中的每個項目都會被渲染為一個 `Text` 元件。`FlatListBasics` 元件接著會渲染 `FlatList` 和所有的 `Text` 元件。
...
列表視圖最常見的用途之一就是顯示從伺服器獲取的資料。要做到這一點，你需要 [學習 React Native 中的網路功能](network.md)。