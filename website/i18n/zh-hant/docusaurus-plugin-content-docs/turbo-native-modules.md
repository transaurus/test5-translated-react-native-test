...
1. **使用 Flow 或 TypeScript 等 JavaScript 型別註解語言定義一個具型別的 JavaScript 規格**；
...
本指南將展示如何實作 [Web Storage API](https://html.spec.whatwg.org/multipage/webstorage.html#dom-localstorage-dev) 的 `localStorage`。該 API 對於可能在您的專案中撰寫應用程式程式碼的 React 開發者來說相當熟悉。
...
React Native Codegen 工具會使用此規格來為我們生成平台特定的介面和樣板程式碼。為此，Codegen 需要知道在哪裡找到我們的規格以及如何處理它。更新您的 `package.json` 以包含以下內容：
...
使用 `NativeLocalStorage`，以下是修改後的 `App.tsx`，其中包含我們希望持久化的文字、一個輸入欄位和一些用於更新此值的按鈕。
