...
您可以使用 `--simulator` 參數指定模擬器運行的裝置，後接字串形式的裝置名稱。預設值為 `"iPhone 14"`。若要在 iPhone SE（第三代）上運行應用程式，請執行以下指令：
...
裝置名稱對應 Xcode 中可用的裝置列表。您可以透過終端機執行 `xcrun simctl list devices` 來查看可用裝置。
