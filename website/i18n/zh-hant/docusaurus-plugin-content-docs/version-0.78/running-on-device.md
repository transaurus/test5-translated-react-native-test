...
在將應用程式發布給用戶之前，於實體裝置上進行測試始終是個好主意。本文件將引導您完成必要的步驟，以便在裝置上運行您的 React Native 應用程式，並為生產環境做好準備。

:::info
如果您使用 `create-expo-app` 來設定專案，可以通過掃描運行 `npm start` 時顯示的 QR 碼，在 Expo Go 中於裝置上運行您的應用程式。有關更多信息，請參閱 Expo 的[在裝置上運行專案](https://docs.expo.dev/get-started/expo-go/)指南。
:::
