...
當使用者在 UI 中變更日期或時間時呼叫此函式。第一個且唯一的參數是一個事件。若要取得選擇器變更後的日期，請改用 onDateChange。
...
當使用者在 UI 中變更日期或時間時呼叫此函式。第一個且唯一的參數是一個代表新日期和時間的 Date 物件。
...
預設情況下，日期選擇器將使用裝置的時區。透過此參數，可以強制使用特定的時區偏移量。例如，要顯示太平洋標準時間，請傳入 -7 * 60。
...
提供一個初始值，當用戶開始選擇日期時會變更。這適用於您不想處理監聽事件並更新日期屬性來保持受控狀態同步的使用情境。受控狀態存在已知錯誤，可能導致其與原生狀態不同步。initialDate 屬性旨在讓您以原生狀態作為真實來源。
