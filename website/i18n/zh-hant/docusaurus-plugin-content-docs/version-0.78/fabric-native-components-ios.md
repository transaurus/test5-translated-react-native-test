...
您可以[手動執行](the-new-architecture/codegen-cli) Codegen，但更簡單的方法是使用您將用來展示元件的應用程式來代為執行。
...
重要的是，您會看到來自 Codegen 的日誌輸出，我們將在 Xcode 中使用這些輸出來建構我們的 WebView 原生元件。
...
:::note
此步驟僅因我們正在建立網頁視圖而需要。iOS上的網頁元件需連結至Apple提供的WebKit框架。若您的元件無需存取網頁特定功能，可跳過此步驟。
:::
