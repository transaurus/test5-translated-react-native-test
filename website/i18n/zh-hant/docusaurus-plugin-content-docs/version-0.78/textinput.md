...
一個基礎組件，用於通過鍵盤向應用程序輸入文本。屬性提供了多種功能的可配置性，例如自動更正、自動大寫、佔位文本以及不同類型的鍵盤，例如數字鍵盤。
...
若為 `true`，文字輸入框在提交時會失去焦點。單行輸入框的預設值為 true，多行輸入框則為 false。請注意，對於多行輸入框，將 `blurOnSubmit` 設為 `true` 意味著按下返回鍵會使輸入框失去焦點並觸發 `onSubmitEditing` 事件，而非在輸入框中插入換行符。
...
提供初始值，該值會在使用者開始輸入時改變。適用於不想處理事件監聽與更新值屬性來保持受控狀態同步的情境。
...
設為 `false` 時，若文字輸入周圍空間有限（例如手機橫向模式），作業系統可能會讓使用者在全螢幕文字輸入模式中編輯文字。設為 `true` 則停用此功能，使用者將始終直接在文字輸入框中編輯文字。預設值為 `false`。
...
可選標識符，用於將自定義的 [InputAccessoryView](inputaccessoryview.md) 連結到此文字輸入框。當此文字輸入框獲得焦點時，InputAccessoryView 會渲染在鍵盤上方。
...
:::note
請注意，此屬性在 iOS 上會將文字對齊頂部，而在 Android 上會置中對齊。若需在雙平台達成相同行為，請搭配將 `textAlignVertical` 設為 `top` 使用。
:::
...
若為 `false`，將禁用文字視圖的滾動功能。預設值為 `true`。僅在 `multiline={true}` 時有效。
...
文字輸入選取範圍的起始與結束位置。將起始與結束設為相同值可定位游標位置。
...
當設為 `false` 時，可防止欄位聚焦時顯示虛擬鍵盤。預設值為 `true`。
...
`newPassword` 用於標記使用者可能想儲存至鑰匙圈的新密碼輸入欄位，`oneTimeCode` 則表示該欄位可透過簡訊收到的驗證碼自動填寫。
...
文字輸入框顯示的值。`TextInput` 是一個受控元件，這意味著如果提供了此屬性，原生值將被強制匹配此值。在大多數情況下，這運作良好，但在某些情況下可能會導致閃爍——常見原因是通過保持值相同來防止編輯。除了設定相同的值外，還可以設定 `editable={false}`，或設定/更新 `maxLength` 以防止不需要的編輯而不會閃爍。
