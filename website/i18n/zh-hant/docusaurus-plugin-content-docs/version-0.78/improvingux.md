...
在觸控手機上輸入文字是一項挑戰——螢幕小、軟體鍵盤佔空間。但根據您需要的資料類型，您可以透過適當配置文字輸入欄位來簡化操作：
...
軟體鍵盤會佔用近半個螢幕。如果有可互動元素可能被鍵盤遮擋，請使用 [`KeyboardAvoidingView` 元件](keyboardavoidingview.md) 確保這些元素仍可存取。
...
在手機上精準點擊按鈕相當困難。請確保所有可互動元素尺寸至少為 44x44 像素。可透過預留足夠空間、使用 `padding`、`minWidth` 和 `minHeight` 樣式值來實現。或者，您可以使用 [`hitSlop` 屬性](touchablewithoutfeedback.md#hitslop) 在不影響版面的情況下擴大互動區域。以下為示範：
...
除非您使用 `Dimensions` API 且未處理方向變更，否則多種螢幕方向預設應能正常運作。若不想支援多方向，可將螢幕鎖定為直向或橫向。

在 iOS 上，於 Xcode 的 General 標籤與 Deployment Info 區段啟用欲支援的裝置方向（確保在變更時已從 Devices 選單選擇 iPhone）。對於 Android，開啟 AndroidManifest.xml 檔案，在 activity 元素內添加 `'android:screenOrientation="portrait"'` 鎖定直向，或 `'android:screenOrientation="landscape"'` 鎖定橫向。
