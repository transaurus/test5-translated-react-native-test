...
直接操作不應是經常使用的手段。通常僅在創建連續動畫以避免元件層級渲染和大量視圖協調的開銷時使用。
...
您可以使用 [`clear`](../textinput#clear) 方法來清空 `TextInput`，該方法採用相同方式清除當前輸入文字。

## 避免與渲染函數衝突

若您更新的屬性同時被渲染函數管理，可能導致不可預測的錯誤。因為每當元件重新渲染且該屬性變化時，先前透過 `setNativeProps` 設置的值將被完全忽略並覆蓋。