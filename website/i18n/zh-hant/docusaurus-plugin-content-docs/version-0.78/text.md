...
假設 `MyAppText` 是一個僅將其子元素渲染到帶有樣式的 `Text` 元件中的元件，則 `MyAppHeaderText` 可以定義如下：
...
當其他 `View` 想要成為觸控回應者並要求此 `View` 釋放其回應者權限時，返回 `true` 允許釋放。
...
在 iOS 14+ 上設定換行策略，可能值為 `none`、`standard`、`hangul-word` 和 `push-out`。
