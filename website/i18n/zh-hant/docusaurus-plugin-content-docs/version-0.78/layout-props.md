...
- 在已設定 flex basis 的節點上，若交叉軸尺寸未設定，寬高比控制節點在交叉軸的尺寸
- 在具有測量函式的節點上，寬高比的作用如同測量函式測量 flex basis
- 在具有 flex grow/shrink 的節點上，若交叉軸尺寸未設定，寬高比控制節點在交叉軸的尺寸
- 寬高比會考慮最小/最大尺寸限制
...
詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/bottom 以瞭解 `bottom` 如何影響佈局。
...
其功能類似 CSS 中的 `display`，但僅支援 'flex'、'none' 和 'contents' 等值。預設值為 `flex`。
...
`flexGrow` 接受任何 >= 0 的浮點數值，預設值為 0。容器會根據子元件的 `flexGrow` 值加權分配剩餘空間。
...
[`flexShrink`](layout-props#flexshrink) 描述當子元件在主軸上的總尺寸超出容器主軸尺寸時，應如何縮小子元件。`flexShrink` 與 `flexGrow` 非常相似，可將溢出的尺寸視為負的剩餘空間來理解。這兩個屬性也能很好地協同工作，讓子元件能根據需要伸縮。

`flexShrink` 接受任何 >= 0 的浮點數值，預設值為 0。容器會根據子元件的 `flexShrink` 值加權縮小其尺寸。
...
其功能類似 CSS 中的 `height`，但在 React Native 中必須使用點數或百分比。不支援 em 等其他單位。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/height 以獲取更多資訊。
...
`justifyContent` 沿主軸方向對齊子元件。例如，若子元件垂直排列，`justifyContent` 控制其垂直對齊方式。其功能類似 CSS 中的 `justify-content`（預設值：flex-start）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/justify-content 以獲取更多資訊。
...
其功能類似 CSS 中的 `max-height`，但在 React Native 中必須使用點數或百分比，不支援 em 等其他單位。
...
其功能類似 CSS 中的 `max-width`，但在 React Native 中必須使用點數或百分比，不支援 em 等其他單位。
...
其功能類似 CSS 中的 `min-height`，但在 React Native 中必須使用點數或百分比，不支援 em 等其他單位。
...
其功能類似 CSS 中的 `min-width`，但在 React Native 中必須使用點數或百分比，不支援 em 等其他單位。
...
`overflow` 控制子元件的測量與顯示方式。`overflow: hidden` 會裁剪視圖，而 `overflow: scroll` 會使視圖獨立於其父元件的主軸進行測量。其功能類似 CSS 中的 `overflow`（預設值為 visible）。詳見 https://developer.mozilla.org/en/docs/Web/CSS/overflow 以獲取更多細節。
...
`static` 元素不會為絕對定位的子元素形成包含塊。

更多資訊，請參閱 [使用 Flexbox 佈局文檔](./flexbox.md#position)。此外，[Yoga 文檔](https://www.yogalayout.dev/docs/styling/position) 詳細說明了 `position` 在 React Native 和 CSS 之間的差異。
...
詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/right 以獲取更多關於 `right` 如何影響佈局的細節。
...
`rowGap` 的作用類似於 CSS 中的 `row-gap`。在 React Native 中僅支援像素單位。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/row-gap 以獲取更多細節。
...
它的功能類似於 CSS 中的 `top`，但在 React Native 中你必須使用點數或百分比。不支援 em 等其他單位。
...
`zIndex` 控制哪些元件顯示在其他元件之上。通常情況下，你不需要使用 `zIndex`。元件會根據它們在文件樹中的順序進行渲染，因此後面的元件會繪製在前面的元件之上。`zIndex` 在你需要動畫或自定義模態介面時可能很有用，因為你不希望這種行為發生。
...
在 iOS 上，`zIndex` 可能需要 `View` 元件彼此為兄弟節點才能如預期般工作。
