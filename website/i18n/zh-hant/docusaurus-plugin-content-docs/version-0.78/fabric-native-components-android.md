...
然後它覆寫了 `getName` 函數，該函數必須返回與 spec 中 `codegenNativeComponent` 函數調用所使用的名稱相同。
...
最後，您需在應用程式中註冊 `ReactWebViewPackage`。修改 `MainApplication` 檔案，將 `ReactWebViewPackage` 加入 `getPackages` 函數返回的套件清單中。
