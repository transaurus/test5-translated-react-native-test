...
2. 右鍵點擊應用並選擇 <code>New Group</code>，將新群組命名為 `NativeLocalStorage`。
...
- `RCT_EXPORT_MODULE` 使用我們在 JavaScript 環境中訪問模組的標識符 `NativeLocalStorage` 來導出和註冊模組。詳情請參閱這些 [文檔](./legacy/native-modules-ios#module-name)。
