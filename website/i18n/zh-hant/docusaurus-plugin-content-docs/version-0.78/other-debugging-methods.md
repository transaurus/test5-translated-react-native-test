...
:::warning
遠端 JavaScript 除錯在 React Native 0.73 中已棄用，並將在未來的版本中移除。
:::

遠端 JavaScript 除錯將外部網頁瀏覽器（Chrome）連接到您的應用程式，並在網頁中運行您的 JavaScript 程式碼。這讓您可以使用 Chrome 的除錯器，就像對任何網頁應用程式一樣。請注意，瀏覽器環境可能非常不同，並非所有 React Native 模組在這種除錯方式下都能正常運作。
...
:::note
在 Android 上，如果除錯器和裝置之間的時間漂移，動畫和事件行為等可能無法正常運作。可以通過運行 ``adb shell "date `date +%m%d%H%M%Y.%S%3N`"`` 來修復此問題。如果使用實體裝置，則需要 root 權限。
:::
