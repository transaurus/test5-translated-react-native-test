...
在 Finder 中雙擊 `Double-Click to Install Azul Zulu JDK 17.pkg` 安裝包來安裝 JDK。
...
對於 Android 開發新手來說，設置開發環境可能有些繁瑣。若您已有 Android 開發經驗，則只需進行部分配置。無論哪種情況，請務必仔細遵循後續步驟。
...
Android Studio 預設會安裝最新版 Android SDK。但使用原生程式碼建置 React Native 應用程式需要特定的 `Android 15 (VanillaIceCream)` SDK。可透過 Android Studio 的 SDK 管理器安裝其他版本 SDK。
...
若您最近才安裝 Android Studio，可能需要[建立新的 AVD](https://developer.android.com/studio/run/managing-avds.html)。選擇「Create Virtual Device...」，然後從清單中任選一款手機，點擊「Next」，接著選擇 **VanillaIceCream** API Level 35 映像檔。

點擊「Next」然後「Finish」以建立 AVD。此時您應可點擊 AVD 旁的綠色三角形按鈕來啟動它。
