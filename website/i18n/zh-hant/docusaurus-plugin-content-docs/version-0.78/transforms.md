...
變換(Transforms)是能幫助您使用2D或3D轉換來修改組件外觀和位置的樣式屬性。但需注意，當應用變換後，變換組件周圍的佈局會保持原樣，因此可能與相鄰組件產生重疊。您可以通過為變換組件添加邊距(margin)、調整相鄰組件或設置容器內邊距(padding)來避免這類重疊問題。
...
`transform`可接受轉換對象數組或以空格分隔的字符串值。每個對象需以要變換的屬性作為鍵(key)，並指定變換使用的值。注意：不應合併多個鍵值對到單個對象中，每個對象應只包含一組鍵值對。

旋轉(rotate)變換要求使用字符串來表示角度值，單位可以是度(deg)或弧度(rad)。例如：
...
傾斜(skew)變換要求使用字符串來表示角度值(deg)。例如：
...
- 第一個值(x軸偏移)必須是`px`、`percentage`或關鍵字`left`、`center`、`right`之一。
- 第二個值(y軸偏移)必須是`px`、`percentage`或關鍵字`top`、`center`、`bottom`之一。
...
`transformOrigin`也支持數組語法。這種語法便於與Animated API配合使用，同時避免了字符串解析，因此具有更高效率。
...
更多詳細信息可參考MDN關於[變換原點](https://developer.mozilla.org/en-US/docs/Web/CSS/transform-origin)的指南。