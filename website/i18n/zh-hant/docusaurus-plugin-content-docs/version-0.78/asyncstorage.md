...
`AsyncStorage` 是一個未加密、非同步、持久化且全域的鍵值儲存系統，應取代 LocalStorage 使用。
...
JavaScript 層的 `AsyncStorage` 為門面模式，提供清晰的 API、標準 `Error` 物件和非批次方法，所有 API 均返回 `Promise` 物件。
...
為 `key` 設定數值並於完成時觸發回調。返回 `Promise` 物件。
...
清除 _所有_ `AsyncStorage` 資料（含其他函式庫的資料）。通常應改用 `removeItem` 或 `multiRemove` 僅清除自身應用金鑰。返回 `Promise` 物件。
...
取得應用程式 _所有_ 已知金鑰（含其他呼叫方的金鑰）。返回 `Promise` 物件。
...
此方法允許您根據輸入的`key`陣列批次獲取項目。回調函數將被調用並傳入對應的鍵值對陣列：
