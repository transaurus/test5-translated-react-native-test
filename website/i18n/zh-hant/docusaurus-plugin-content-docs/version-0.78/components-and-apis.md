...
您不僅限於使用 React Native 內建的組件和 API。React Native 擁有數千名開發者組成的社群。如果需要尋找特定功能的函式庫，請參閱[尋找函式庫指南](libraries#finding-libraries)。
...
這些組件可能對某些應用程式有用。如需完整的組件和 API 列表，請查看左側邊欄（或窄螢幕上方的選單）。
