...
請先完成[開發環境設置指南](set-up-your-environment)並學習[不使用框架的 React Native](getting-started-without-a-framework)以配置 Android 版 React Native 應用的開發環境。
本指南同時假設您已掌握 Android 開發基礎知識，例如建立 Activity 和編輯 `AndroidManifest.xml` 文件。

## 1. 建立目錄結構

為確保流程順暢，請為整合專案建立新資料夾，並將**現有 Android 專案**移至 `/android` 子目錄。
...
安裝過程會建立新的 `node_modules` 資料夾，該資料夾儲存專案建構所需的所有 JavaScript 依賴項。

請將 `node_modules/` 加入您的 `.gitignore` 文件（可參考[社群預設文件](https://github.com/react-native-community/template/blob/0.75-stable/template/_gitignore)）。

## 3. 將 React Native 添加至應用

### 配置 Gradle

React Native 使用 React Native Gradle 插件來配置依賴項與專案設置。

首先編輯 `settings.gradle` 文件並添加以下內容（參照[社群模板建議](https://github.com/react-native-community/template/blob/0.77-stable/template/android/settings.gradle)）：
...
接著在頂層 `build.gradle` 中加入這行程式碼（參照[社群模板建議](https://github.com/react-native-community/template/blob/0.77-stable/template/android/build.gradle)）：
...
最後開啟應用層級的 `gradle.properties` 文件並加入以下內容（參照[社群模板文件](https://github.com/react-native-community/template/blob/0.77-stable/template/android/gradle.properties)）：
...
這是必要的，因為你的應用程式需要透過 HTTP 與本地端的打包工具 [Metro][https://metrobundler.dev/] 進行通訊。

請確保僅在 **debug** 版的清單檔案中加入此設定。

## 4. 撰寫 TypeScript 程式碼

現在我們將實際修改原生 Android 應用程式來整合 React Native。

我們要撰寫的第一段程式碼，是為新畫面實際編寫的 React Native 程式碼，該畫面將整合到我們的應用程式中。

### 建立 `index.js` 檔案

首先，在你的 React Native 專案根目錄中建立一個空的 `index.js` 檔案。

`index.js` 是 React Native 應用程式的進入點，且始終是必需的。它可以是一個小檔案，用於 `import` 其他屬於你的 React Native 元件或應用程式的檔案，也可以包含所有所需的程式碼。
...
最後，我們需要建立一個新的 `Activity`，該 Activity 將擴展 `ReactActivity` 並託管 React Native 程式碼。此 Activity 將負責啟動 React Native 運行時環境並渲染 React 元件。
...
每當你建立一個新的 Activity 時，你都需要將其加入到你的 `AndroidManifest.xml` 檔案中。你還需要將 `MyReactActivity` 的主題設定為 `Theme.AppCompat.Light.NoActionBar`（或任何非 ActionBar 的主題），否則你的應用程式會在 React Native 畫面上方渲染一個 ActionBar：
...
你已完成整合 React Native 與應用程式的基本步驟。現在我們將啟動 [Metro 打包工具](https://metrobundler.dev/)，將你的 TypeScript 應用程式代碼打包成 bundle。Metro 的 HTTP 伺服器會將 bundle 從開發環境的 `localhost` 分享到模擬器或裝置，這使得[熱重載](https://reactnative.dev/blog/2016/03/24/introducing-hot-reloading)成為可能。

首先，你需要在專案根目錄下創建一個 `metro.config.js` 文件，內容如下：
...
配置文件就位後，你就可以運行打包工具。在專案的根目錄下運行以下命令：
...
你也可以使用 Android Studio 來創建發布版本！這與創建你之前原生 Android 應用程式的發布版本一樣快速。
...
此時你可以像平常一樣繼續開發你的應用程式。參考我們的[調試](debugging)和[部署](running-on-device)文檔，了解更多關於使用 React Native 的資訊。