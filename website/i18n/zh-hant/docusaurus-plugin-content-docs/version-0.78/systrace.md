...
`Systrace` 允許您用標籤和整數值來標記 JavaScript (JS) 事件。在 EasyProfiler 中擷取非計時的 JS 事件。
...
beginAsyncEvent/endAsyncEvent 用於開始和結束分析，其中結束可能發生在另一個執行緒或當前堆疊框架之外，例如 await 返回的 cookie 變數應作為輸入參數傳遞給 endAsyncEvent 呼叫以結束分析。
