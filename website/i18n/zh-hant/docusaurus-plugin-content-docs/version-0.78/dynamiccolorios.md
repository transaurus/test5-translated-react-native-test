...
`DynamicColorIOS` 接收一個物件作為參數，該物件包含兩個必要鍵值：`dark` 和 `light`，以及兩個可選鍵值 `highContrastLight` 和 `highContrastDark`。這些鍵值分別對應 iOS 的「淺色模式」和「深色模式」下要使用的顏色，以及當高對比度無障礙模式啟用時的高對比版本顏色。

在運行時，系統會根據當前系統外觀和無障礙設定自動選擇要顯示的顏色。動態色彩特別適用於品牌色彩或其他需要自動響應系統設定變更的應用程式特定色彩。
