...
在 iOS 17+ 中，設置為`pointer`可在指針（如觸控板、iOS 觸控筆或 visionOS 上的用戶視線）懸停於視圖時啟用懸停效果。
...
使用 Android 底層的[高程 API](https://developer.android.com/training/material/shadows-clipping.html#Elevation)設置視圖的高程。這會為項目添加投影並影響重疊視圖的 z 軸順序。僅支援 Android 5.0+，對更早版本無效。
...
:::note
由於性能和規範合規性問題，iOS 僅支援上述兩種濾鏡函數。未來計劃探索使用 SwiftUI 替代 UIKit 實現的潛在解決方案。
:::
...
`filter`可接受由上述濾鏡函式組成的物件陣列，或模仿[web語法](https://developer.mozilla.org/en-US/docs/Web/CSS/filter#syntax)的字串。
...
設定元素輪廓線的顏色。詳見[web文檔](https://developer.mozilla.org/en-US/docs/Web/CSS/outline-color)。
...
設定輪廓線與元素邊界之間的間距。不影響佈局。詳見[web文檔](https://developer.mozilla.org/en-US/docs/Web/CSS/outline-offset)。
...
設定元素輪廓線的樣式。詳見[web文檔](https://developer.mozilla.org/en-US/docs/Web/CSS/outline-style)。
...
- `'auto'`: 該 View 可成為觸控事件目標。
- `'none'`: 該 View 永遠不會成為觸控事件目標。
- `'box-none'`: 該 View 本身不會成為觸控事件目標，但其子視圖可以。
- `'box-only'`: 該 View 可成為觸控事件目標，但其子視圖不可。
