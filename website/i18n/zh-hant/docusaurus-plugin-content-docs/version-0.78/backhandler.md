...
- **若沒有任何監聽器返回 true 或未註冊監聽器**，系統將預設執行返回按鈕功能來退出應用程式。
...
`BackHandler.addEventListener` 會建立事件監聽器並返回一個 `NativeEventSubscription` 物件，應使用 `NativeEventSubscription.remove` 方法來清除該監聽器。
