...
清除 _所有_ `AsyncStorage` 資料（含其他客戶端、函式庫等）。通常應避免直接呼叫，建議改用 `removeItem` 或 `multiRemove` 僅清除應用自身的鍵值。回傳 `Promise` 物件。
...
取得應用已知的 _所有_ 鍵值（含其他呼叫者、函式庫等）。回傳 `Promise` 物件。
...
使用此方法作為批次操作來儲存多個鍵值對。操作完成時，您將收到一個包含任何錯誤的單一回調：
...
批次操作以合併給定鍵集的現有值和新值。此操作假設這些值是經過JSON字串化的。返回一個`Promise`物件。
