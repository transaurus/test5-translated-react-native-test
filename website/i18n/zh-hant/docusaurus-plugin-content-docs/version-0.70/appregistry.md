...
`AppRegistry` 是所有 React Native 應用程式的 JavaScript 進入點。應用程式的根元件應透過 `AppRegistry.registerComponent` 註冊自身，之後原生系統便能載入應用程式的套件包，並在準備就緒時透過呼叫 `AppRegistry.runApplication` 實際運行應用程式。
...
此方法可在應用程式處於背景狀態時執行 JavaScript 任務，例如用於同步最新資料、處理推播通知或播放音樂。
...
一個以 `appKey` 為鍵、[`Runnable`](appregistry#runnable) 型別為值的物件。
