...
在進行此操作前，您應熟悉 [原生 UI 元件](native-components-android) 的概念。同時也應了解 [WebView 的原生程式碼](https://github.com/react-native-webview/react-native-webview/blob/master/android/src/main/java/com/reactnativecommunity/webview/RNCWebViewManagerImpl.kt)，因為在實現新功能時需要參考這些程式碼——儘管不需要深入理解。

## 原生程式碼

:::info
此範例假設您已安裝 [`react-native-webview`](https://github.com/react-native-webview/react-native-webview)，如果尚未安裝，請先遵循其 [入門指南](https://github.com/react-native-webview/react-native-webview/blob/master/docs/Getting-Started.md)。
:::

首先，您需要建立 `RNCWebViewManager`、`RNCWebView` 和 `RNCWebViewClient` 的子類別。在您的視圖管理器中，您需要覆寫以下方法：
...
您需要按照常規步驟 [註冊模組](native-modules-android.md#register-the-module)。

### 新增屬性

要新增屬性，您需要將其添加到 `CustomWebView` 中，然後在 `CustomWebViewManager` 中公開它。
...
您可以在 WebView 客戶端中觸發事件。如果您的事件基於現有處理程序，可以掛接到這些處理程序上。

您應參考 React Native WebView 程式碼庫中的 [RNCWebViewManagerImpl.kt](https://github.com/react-native-webview/react-native-webview/blob/master/android/src/main/java/com/reactnativecommunity/webview/RNCWebViewManagerImpl.kt)，以了解可用的處理程序及其實現方式。您可以擴展這裡的任何方法來提供額外功能。
...
要獲取您的原生元件，您必須使用 `requireNativeComponent`：與常規自訂元件相同。但您需要傳入額外的第三個參數 `WebView.extraNativeComponentConfig`。此第三個參數包含僅用於原生程式碼的屬性類型。
...
如果您想為原生元件添加自訂屬性，可以在 WebView 上使用 `nativeConfig.props`。
...
與常規的原生元件類似，你必須在元件中提供所有的 prop 類型，以便將它們轉發到原生元件。然而，如果有些 prop 類型僅在元件內部使用，你可以將它們添加到前面提到的第三個參數的 `nativeOnly` 屬性中。對於事件處理器，你必須使用 `true` 值而不是常規的 prop 類型。
