...
- 如果停用 Hermes，React Native 會使用 [JavaScriptCore](http://trac.webkit.org/wiki/JavaScriptCore)，這是 Safari 瀏覽器所使用的 JavaScript 引擎。請注意，在 iOS 上，由於應用程式缺乏可寫入的執行記憶體，JavaScriptCore 不會使用 JIT 編譯。
- 當使用 Chrome 進行偵錯時，所有 JavaScript 程式碼會在 Chrome 內部執行，並透過 WebSockets 與原生程式碼進行通訊。Chrome 使用 [V8](https://v8.dev/) 作為其 JavaScript 引擎。

雖然這些環境非常相似，但您可能會遇到一些不一致的情況。建議避免依賴任何運行環境的特定行為。
