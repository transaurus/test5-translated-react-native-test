...
當使用者放開滑桿時呼叫的回呼函式，無論數值是否改變。目前的數值會作為引數傳遞給回呼處理函式。
...
滑桿的步進值。數值應介於 0 與 (maximumValue - minimumValue) 之間。預設值為 0。
...
滑桿的初始值。數值應介於 minimumValue 與 maximumValue 之間，兩者預設分別為 0 和 1。預設值為 0。
...
用於為 iOS 上的預設拇指圖片著色的顏色，或 Android 上開關前景握把的顏色。
