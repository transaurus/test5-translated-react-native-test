...
**若您剛接觸行動開發**，最簡單的方式是使用 Expo Go。Expo 是一套圍繞 React Native 建構的工具與服務，雖然它具備眾多[功能](https://docs.expo.dev)，目前對我們最相關的特性是它能讓您在幾分鐘內開始編寫 React Native 應用。您只需要最新版的 Node.js 和一支手機或模擬器。若想在安裝任何工具前先透過網頁瀏覽器試用 React Native，可以嘗試 [Snack](https://snack.expo.dev/)。

**若您已熟悉行動開發**，可能會想使用 React Native CLI。這需要先安裝 Xcode 或 Android Studio。若您已安裝其中一項工具，應該能在幾分鐘內開始使用。若尚未安裝，預計需花費約一小時進行安裝與設定。
