...
在觸控手機上輸入文字是一項挑戰——螢幕小、軟體鍵盤佔用空間。但根據您需要的資料類型，您可以透過適當配置文字輸入欄位來簡化操作：
...
軟體鍵盤幾乎佔據半個螢幕。如果有可互動元素可能被鍵盤遮蓋，請使用 [`KeyboardAvoidingView` 元件](keyboardavoidingview.md) 確保這些元素仍可存取。
...
在手機上點擊按鈕時很難做到非常精確。確保所有可互動元素的尺寸至少為 44x44 像素。一種方法是為元素留出足夠空間，`padding`、`minWidth` 和 `minHeight` 樣式值對此很有幫助。或者，您可以使用 [`hitSlop` 屬性](touchablewithoutfeedback.md#hitslop) 在不影響版面的情況下增大互動區域。以下是一個示範：
...
除非您使用 `Dimensions` API 且未處理方向變更，否則預設應支援多種螢幕方向。如果您不想支援多種螢幕方向，可以將螢幕方向鎖定為直向或橫向。

在 iOS 上，於 Xcode 的 General 標籤和 Deployment Info 部分啟用您想支援的裝置方向（確保在進行變更時已從 Devices 選單中選擇 iPhone）。對於 Android，開啟 AndroidManifest.xml 檔案，並在 activity 元素內添加 `'android:screenOrientation="portrait"'` 以鎖定為直向，或 `'android:screenOrientation="landscape"'` 以鎖定為橫向。
