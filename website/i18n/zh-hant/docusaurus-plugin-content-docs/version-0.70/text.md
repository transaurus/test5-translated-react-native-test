...
Android 和 iOS 都允許您通過用特定格式（如粗體或彩色文字）標註字符串的範圍來顯示格式化文字（iOS 上的 `NSAttributedString`，Android 上的 `SpannableString`）。實際上，這非常繁瑣。對於 React Native，我們決定使用網頁範式來實現相同的效果，您可以通過嵌套文字來達成。
...
在底層，React Native 會將其轉換為一個扁平的 `NSAttributedString` 或 `SpannableString`，其中包含以下信息：
...
在網頁上，通常為整個文檔設置字體家族和大小的方式是利用繼承的 CSS 屬性，如下所示：
...
您還失去了為整個子樹設置默認字體的能力。同時，`fontFamily` 只接受單一字體名稱，這與 CSS 中的 `font-family` 不同。建議在應用程序中一致使用字體和大小的方式是創建一個包含這些樣式的元件 `MyAppText`，並在整個應用程序中使用此元件。您還可以使用此元件來創建更具體的元件，如 `MyAppHeaderText`，用於其他類型的文字。
...
假設 `MyAppText` 是一個僅將其子元素渲染到具有樣式的 `Text` 元件中的元件，那麼 `MyAppHeaderText` 可以定義如下：
...
- (開發者) React 元件設計時考慮了強隔離性：您應該能夠將元件放置在應用程序的任何位置，相信只要屬性相同，它將以相同的方式外觀和行為。可以從屬性外部繼承的文字屬性將破壞這種隔離性。
...
指定當啟用 `adjustsFontSizeToFit` 時字體可達到的最小縮放比例 (值為 0.01-1.0)。
...
用於在計算文字佈局（包括自動換行）後，以省略號截斷文字，使總行數不超過此數字。將此屬性設為 `0` 會取消此限制，意味著不應用任何行數限制。
...
觸控開始時立即調用，在 `onPressOut` 和 `onPress` 之前觸發。
...
當滾動視圖被禁用時，此屬性定義觸控點可偏離按鈕多遠才會停用按鈕。停用後，若將觸控點移回範圍內，按鈕會重新啟用。在滾動視圖禁用期間反覆移動觸控點可觀察此行為。建議傳入常數以減少記憶體分配。
...
當設為 `true` 時，文字被按下時不會產生視覺變化。預設情況下，文字被按下時會顯示灰色橢圓形高亮效果。
...
`TextLayoutEvent` 物件會在元件佈局變更時作為回調結果返回。它包含一個名為 `lines` 的鍵，其值是一個陣列，包含與每個渲染文字行對應的 [`TextLayout`](text#textlayout) 物件。
