...
若 React Native 應用在正式版本中拋出未處理的例外，輸出內容可能經過混淆而難以閱讀：
...
類似 `p@1:132161` 的區段是經過最小化的函式名稱與位元組碼偏移量。要偵錯此問題，您需要將其轉換為檔案名稱、行號與函式名稱，例如：`AwesomeProject/App.js:54:initializeMap`。此過程稱為**符號化 (symbolication)**。您可以透過將產生的原始碼對應檔 (source map) 與堆疊追蹤傳遞給 `metro-symbolicate`，來對上述最小化函式名稱與位元組碼進行符號化。
