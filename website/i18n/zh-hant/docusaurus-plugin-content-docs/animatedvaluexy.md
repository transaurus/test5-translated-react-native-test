...
用於驅動二維動畫（如平移手勢）的2D數值物件。其API幾乎與標準的[`Animated.Value`](animatedvalue)相同，但採用多工處理機制。底層實際包含兩個常規的`Animated.Value`。
...
設定一個偏移量，該偏移量會疊加到無論通過`setValue`、動畫或`Animated.event`設定的任何數值上。適用於補償平移手勢起始點等情況。
...
取消註冊監聽器。`id`參數需與先前`addListener()`返回的識別碼匹配。
...
停止所有正在運行的動畫或追蹤。可通過`callback`獲取動畫停止後的最終值，這對於將狀態更新至與動畫位置同步非常有用。
