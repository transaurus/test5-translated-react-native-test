...
有時需要直接修改元件而不透過狀態/屬性觸發整個子樹重新渲染。例如在瀏覽器中使用 React 時，有時需要直接修改 DOM 節點，移動應用中的視圖也是如此。`setNativeProps` 是 React Native 中等同於直接設定 DOM 節點屬性的方法。

:::caution
請在頻繁重新渲染導致效能瓶頸時使用 `setNativeProps`！

直接操作不應是您經常使用的工具。通常僅在建立連續動畫以避免渲染元件層級和協調多個視圖的開銷時使用。
`setNativeProps` 是命令式的，並將狀態儲存在原生層（DOM、UIView 等）而非 React 元件中，這會使您的程式碼更難理解。

在使用前，請先嘗試透過 `setState` 和 [`shouldComponentUpdate`](https://reactjs.org/docs/optimizing-performance.html#shouldcomponentupdate-in-action) 解決問題。
:::
...
這讓我們能夠撰寫以下程式碼，並確知子元件會根據點擊更新不透明度，而無需子元件知曉此事或修改其實作方式：
...
與原始範例相比，這種方式計算量較大——即使視圖及其子元件的其他屬性未變，React 仍需在每次不透明度變化時重新渲染元件層級。通常這種開銷無需擔心，但在執行連續動畫和回應手勢時，謹慎優化元件可提升動畫的流暢度。
...
複合元件並非由原生視圖支援，因此無法對其呼叫 `setNativeProps`。請看以下範例：
...
您可能已經注意到，我們使用 `{...props}` 將所有屬性傳遞給子視圖。這樣做的原因是 `TouchableOpacity` 實際上是一個複合元件，因此除了依賴子元件的 `setNativeProps` 外，還要求子元件處理觸控操作。為此，它會傳遞[各種屬性](view.md#onmoveshouldsetresponder)來回調 `TouchableOpacity` 元件。相比之下，`TouchableHighlight` 由原生視圖支持，僅需要我們實現 `setNativeProps`。
...
您可以使用 [`clear`](../textinput#clear) 方法來清除 `TextInput`，該方法會採用相同的方式清除當前輸入文本。
...
通過[智能應用 `shouldComponentUpdate`](https://reactjs.org/docs/optimizing-performance.html#avoid-reconciliation)，您可以避免在協調未變更的元件子樹時產生不必要的開銷，從而可能使性能足夠好，以至於可以使用 `setState` 而非 `setNativeProps`。
...
請注意，這些測量值在原生渲染完成後才可用。如果您需要盡快獲取測量值且不需要 `pageX` 和 `pageY`，可以考慮使用 [`onLayout`](view.md#onlayout) 屬性替代。
...
類似於 `measure()`，但會測量視圖相對於祖先的位置，該祖先由 `relativeToNativeComponentRef` 引用指定。這意味著返回的坐標是相對於祖先視圖原點 `x`、`y` 的。

:::note
此方法也可以使用 `relativeToNativeNode` 處理程序（而非引用）調用，但此變體在新架構中已過時。
:::
