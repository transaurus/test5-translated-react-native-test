...
您可以使用 `PlatformColor` 函數，通過提供對應的原生顏色字符串值來存取目標平台上的原生顏色。將字符串傳遞給 `PlatformColor` 函數，如果該顏色在該平台上存在，它將返回對應的原生顏色，您可以將其應用於應用程序的任何部分。
...
提供給 `PlatformColor` 函數的字符串值必須與應用程序運行的原生平台上的字符串完全匹配。為了避免運行時錯誤，該函數應通過 `Platform.OS === 'platform'` 或 `Platform.select()` 進行平台檢查，如上例所示。
