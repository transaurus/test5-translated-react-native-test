...
手指並非最精確的操作工具，使用者常會意外觸發錯誤元素或錯過觸發區域。為此，`Pressable` 提供可選的 `HitRect`，可用來定義觸控可註冊的範圍距離包裝元件多遠。按壓可在 `HitRect` 內的任何位置開始。

`PressRect` 允許按壓在維持觸發狀態的同時，移動超出元件及其 `HitRect` 範圍，並仍符合「按壓」資格——想像當你按住一個按鈕時，手指慢慢滑開的情景。
...
> `Pressable` 使用 React Native 的 `Pressability` API。欲了解更多關於 Pressability 的狀態機流程及其運作方式，請查閱 [Pressability](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Pressability/Pressability.js#L350) 的實作程式碼。
...
在呼叫 `onPressIn` 前，等待按壓觸發的持續時間（毫秒）。
...
當 `onPressIn` 觸發後持續超過 500 毫秒時調用。此時間間隔可透過 [`delayLongPress`](#delaylongpress) 自訂。
...
當觸碰開始時立即調用，早於 `onPressOut` 和 `onPress`。
...
在 `onPressOut` 觸發前，視為按壓的觸碰可超出此元件的額外距離範圍。
...
可以是視圖樣式，或接收一個反映元件當前是否被按壓的布林值並返回視圖樣式的函數。
