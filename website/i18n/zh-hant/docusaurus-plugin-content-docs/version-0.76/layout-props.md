...
以下範例展示不同屬性如何影響或塑造 React Native 的佈局。您可以嘗試在修改 `flexWrap` 屬性值的同時，從 UI 中新增或移除方塊。
...
- 在已設定 flex basis 的節點上，長寬比控制未設定交叉軸方向的節點大小
- 在具有測量函式的節點上，長寬比的作用如同測量函式測量 flex basis
- 在具有 flex grow/shrink 的節點上，長寬比控制未設定交叉軸方向的節點大小
- 長寬比會考慮最小/最大尺寸限制
...
[`flexShrink`](layout-props#flexshrink) 描述了當子項目的總大小在主軸上超出容器大小時，如何沿主軸縮小子項目。`flexShrink` 與 `flexGrow` 非常相似，可以將其視為將任何溢出的空間視為負剩餘空間。這兩個屬性也可以很好地協同工作，允許子項目根據需要增長和縮小。
...
其功能類似於 CSS 中的 `max-width`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。
...
其功能類似於 CSS 中的 `min-height`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。
...
其功能類似於 CSS 中的 `min-width`，但在 React Native 中必須使用點數或百分比單位，不支援 em 等其他單位。
...
`overflow` 控制子元件的測量與顯示方式。`overflow: hidden` 會裁剪視圖，而 `overflow: scroll` 會讓視圖獨立於父元件主軸進行測量。其功能類似 CSS 中的 `overflow`（預設值：visible）。詳見 https://developer.mozilla.org/en/docs/Web/CSS/overflow 了解更多細節。
...
`paddingBottom` 的功能類似 CSS 中的 `padding-bottom`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/padding-bottom 了解更多細節。
...
`paddingLeft` 的功能類似 CSS 中的 `padding-left`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/padding-left 了解更多細節。
...
`paddingRight` 的功能類似 CSS 中的 `padding-right`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/padding-right 了解更多細節。
...
`static` 元素不會為絕對定位的子元素形成包含區塊。

更多資訊，請參閱 [Flexbox 佈局文件](./flexbox.md#position)。此外，[Yoga 文件](https://www.yogalayout.dev/docs/styling/position) 詳細說明了 `position` 在 React Native 和 CSS 之間的差異。
...
`zIndex` 控制哪些元件會顯示在其他元件之上。通常情況下，您不需要使用 `zIndex`。元件會按照它們在文件樹中的順序渲染，因此後面的元件會繪製在先前元件的上方。`zIndex` 在您有動畫或自定義模態介面且不希望這種行為時可能很有用。
