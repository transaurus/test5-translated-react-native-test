...
Android 和 iOS 都允許你通過為字串的特定範圍添加格式（如粗體或彩色文字）來顯示格式化文字（iOS 上的 `NSAttributedString`，Android 上的 `SpannableString`）。實際上，這非常繁瑣。對於 React Native，我們決定使用網頁的範例來實現相同的效果，即通過嵌套文字來達成。
...
你還失去了為整個子樹設置默認字體的能力。同時，`fontFamily` 只接受單一字體名稱，這與 CSS 中的 `font-family` 不同。建議在應用中使用一致的字體和大小的方式是創建一個包含這些樣式的元件 `MyAppText`，並在整個應用中使用該元件。你還可以使用此元件來創建更具體的元件，如 `MyAppHeaderText` 以用於其他類型的文字。
...
- (開發者) React 元件設計時考慮了強隔離性：你應該能夠將元件放置在應用中的任何位置，並相信只要屬性相同，它的外觀和行為就會相同。可能從屬性外部繼承的文字屬性將破壞這種隔離性。
...
在 Android 上，這些角色在 TalkBack 中的功能類似於 iOS 中 Voiceover 添加輔助功能特徵的功能。
...
在 Android API 23+ 上設定自動斷字的頻率，用於確定單詞斷開位置。
...
- `head` - 行尾顯示完整內容，行首被截斷部分以省略號表示，如：「...wxyz」
- `middle` - 行首行尾顯示完整內容，中間被截斷部分以省略號表示，如：「ab...yz」
- `tail` - 行首顯示完整內容，行尾被截斷部分以省略號表示，如：「abcd...」
- `clip` - 不繪製超出文字容器邊界的內容
...
當其他 `View` 想要成為回應者並要求此 `View` 釋放其回應者權限時觸發。返回 `true` 表示允許釋放。
...
若父層 `View` 希望阻止子層 `View` 在觸碰開始時成為回應者，應實作此處理函式並返回 `true`。
...
當滾動視圖被禁用時，此屬性定義觸碰點可偏離按鈕多遠才會停用按鈕。一旦停用，將觸碰點移回即可重新啟用按鈕。在滾動視圖禁用期間來回移動可觀察此行為。建議傳入常數值以減少記憶體配置。
...
在 iOS 14+ 設定換行策略，可選值為 `none`、`standard`、`hangul-word` 與 `push-out`。
...
`TextLayout` 物件是 [`TextLayoutEvent`](text#textlayoutevent) 回調的一部分，包含 `Text` 行次的測量數據。
