...
2. 設定依賴管理系統，使其能根據規格自動生成程式碼並完成自動連結
3. 實作原生程式碼
4. 在應用程式中使用該元件
...
React Native 的 Codegen 工具會根據規格生成平台特定介面與樣板程式碼。為此，Codegen 需要知道規格檔案的位置與處理方式。請更新您的 `package.json` 包含以下內容：
...
現在是時候編寫原生平台代碼，以便當 React 需要渲染視圖時，平台能夠創建正確的原生視圖並將其顯示在螢幕上。

您應該分別完成 Android 和 iOS 平台的開發工作。
...
這段程式碼創建了一個應用程式，使用我們新建立的 `WebView` 元件來載入 `react.dev` 網站。
