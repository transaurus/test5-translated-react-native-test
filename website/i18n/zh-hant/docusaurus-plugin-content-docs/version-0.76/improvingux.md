...
在觸控手機上輸入文字是一項挑戰——螢幕小、軟體鍵盤佔空間。但根據您需要的資料類型，您可以透過適當配置文字輸入框來簡化操作：
...
查看 [`TextInput` 文件](textinput.md) 以獲取更多配置選項。
...
軟體鍵盤會佔據近半個螢幕。如果有可互動元素可能被鍵盤遮擋，請使用 [`KeyboardAvoidingView` 元件](keyboardavoidingview.md) 確保它們仍可存取。
...
在手機上按鈕時很難精準點擊。確保所有互動元素的尺寸至少為 44x44 像素。可透過留白空間實現，`padding`、`minWidth` 和 `minHeight` 樣式值對此很有幫助。或者，您可以使用 [`hitSlop` 屬性](touchablewithoutfeedback.md#hitslop) 在不影響版面的情況下擴大互動區域。以下為示範：
...
Android API 21+ 使用 Material Design 漣漪效果在使用者觸碰螢幕互動區域時提供視覺回饋。React Native 透過 [`TouchableNativeFeedback` 元件](touchablenativefeedback.md) 實現此效果。相較於透明度或高亮效果，使用此觸碰效果通常能讓應用更符合平台風格。但需注意此效果不支援 iOS 或 Android API < 21，因此需在 iOS 上使用其他 Touchable 元件作為備案。您可以使用 [react-native-platform-touchable](https://github.com/react-community/react-native-platform-touchable) 等函式庫自動處理平台差異。
...
除非使用 `Dimensions` API 且未處理方向變更，否則多種螢幕方向預設應能正常運作。若不想支援多方向，可將螢幕鎖定為直向或橫向。

在 iOS 上，於 Xcode 的 General 標籤頁中 Deployment Info 區段啟用欲支援的裝置方向（確保在變更時已從 Devices 選單選擇 iPhone）。Android 則需在 AndroidManifest.xml 檔案的 activity 元素內添加 `'android:screenOrientation="portrait"'` 鎖定直向，或 `'android:screenOrientation="landscape"'` 鎖定橫向。

# 延伸學習

[Material Design](https://material.io/) 和 [人機介面指南](https://developer.apple.com/design/human-interface-guidelines) 是學習行動平台設計的絕佳資源。