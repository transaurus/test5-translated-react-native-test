...
最簡單的安裝方式是透過 [Mac App Store](https://itunes.apple.com/us/app/xcode/id497799835?mt=12)。安裝 Xcode 會同時安裝 iOS 模擬器及構建 iOS 應用程式所需的所有工具。
...
從 React Native 0.69 版本開始，可以透過模板提供的 `.xcode.env` 文件來配置 Xcode 環境。
...
此外，您可以在 `.xcode.env` 文件中添加其他環境變數，並在構建腳本階段引入該文件。若需執行特定環境的腳本，這是 **建議的做法**：可將構建階段與特定環境解耦。
