...
`TouchableWithoutFeedback` 僅支援單個子元素。若需包含多個子組件，請將其包裹在一個 View 中。重要的是，`TouchableWithoutFeedback` 通過克隆其子元素並應用響應屬性來工作。因此，任何中介組件都必須將這些屬性傳遞給底層的 React Native 組件。
...
當值為 `true` 時，表示該視圖是一個無障礙元素。預設情況下，所有可觸碰元素都是無障礙的。
...
無障礙操作允許輔助技術以程式化方式觸發元件的操作。`accessibilityActions`屬性應包含操作物件列表，每個操作物件應包含名稱和標籤欄位。
...
表示組件的當前值。可以是組件值的文字描述，或者對於基於範圍的組件（如滑塊和進度條），它包含範圍資訊（最小值、當前值和最大值）。
...
> 觸碰區域永遠不會超出父視圖的邊界，且如果觸碰同時命中兩個重疊的視圖，兄弟視圖的 Z-index 始終具有優先權。
...
如果 `onPressIn` 後的時間超過 370 毫秒則呼叫。此時間段可透過 [`delayLongPress`](#delaylongpress) 自訂。
...
當觸碰釋放時呼叫，但如果被取消（例如因滾動搶佔了回應鎖）則不呼叫。第一個函數參數是 [PressEvent](pressevent) 形式的事件。
...
當滾動視圖被禁用時，此值定義觸碰可以從按鈕移開多遠的距離才會停用按鈕。一旦停用，嘗試移回按鈕，您會看到按鈕再次被啟用！在滾動視圖禁用時來回移動多次。請確保傳入常數以減少記憶體分配。
