...
控制應用程式狀態列的元件。狀態列通常是螢幕頂部的區域，顯示當前時間、Wi-Fi和行動網路資訊、電量等級和/或其他狀態圖標。
...
對於使用元件不理想的情況，還有一個命令式 API 作為元件的靜態函數公開。但是，不建議對相同的屬性同時使用靜態 API 和元件，因為靜態 API 設置的任何值將在下一次渲染時被元件設置的值覆蓋。
