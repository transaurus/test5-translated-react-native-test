...
如果用戶先前已關閉某項權限，系統會建議應用程式說明需要該權限的理由。可選的 `rationale` 參數僅在必要時顯示說明對話框——否則將直接顯示標準的權限請求提示。
...
向使用者請求啟用權限，並回傳一個解析為字串值的 Promise（參見上方的結果字串），表示使用者是允許、拒絕請求，或選擇不再被詢問。
...
在同一個對話框中向使用者請求啟用多個權限，並回傳一個物件，其鍵為權限名稱，值為字串（參見上方的結果字串），表示使用者是允許、拒絕請求，或選擇不再被詢問。
