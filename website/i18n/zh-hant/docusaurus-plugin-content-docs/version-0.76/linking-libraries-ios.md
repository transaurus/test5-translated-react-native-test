...
並非所有應用程式都會使用所有原生功能，包含支援這些功能的程式碼會影響二進位檔大小...但我們仍希望能在您需要時支援添加這些功能。
...
:::note
我們隨 React Native 發布的所有函式庫都存放在儲存庫根目錄的 `Libraries` 資料夾中。其中有些是純 JavaScript，您只需 `require` 即可使用。
...
如果函式庫包含原生程式碼，其資料夾內必須有一個 `.xcodeproj` 檔案。將此檔案拖放到 Xcode 專案中（通常在 Xcode 的 `Libraries` 群組下）；

![](/docs/assets/AddToLibraries.png)

#### 步驟 2

點擊您的主要專案檔案（代表 `.xcodeproj` 的那個），選擇 `Build Phases`，然後從您要導入的函式庫的 `Products` 資料夾中，將靜態函式庫拖放到 `Link Binary With Libraries`
...
如果您需要從原生端呼叫它，那麼我們需要知道函式庫的標頭檔。為此，您需要前往專案檔案，選擇 `Build Settings` 並搜尋 `Header Search Paths`。在此處您應包含函式庫的路徑。（本文檔過去建議使用 `recursive`，但現在不再推薦，因為這可能導致細微的建置失敗，特別是與 CocoaPods 一起使用時。）
