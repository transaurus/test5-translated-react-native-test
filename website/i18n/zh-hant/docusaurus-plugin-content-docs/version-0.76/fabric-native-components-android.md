...
代碼隨後定義了一個輔助函數來實際發送事件。要發送事件，你需要：
...
當 React 從應用程序接收到渲染特定組件的指令時，React 會使用註冊的視圖管理器來創建視圖並傳遞所有必需的屬性。
...
然後它覆寫了 `getName` 函數，該函數必須返回與 spec 中 `codegenNativeComponent` 函數調用所使用的名稱相同的名稱。
...
如同原生模組的做法，原生元件也需要實作 `ReactPackage` 類別。這個物件可用於在 React Native 執行時期註冊元件。
...
最後，你需要在應用程式中註冊 `ReactWebViewPackage`。我們透過修改 `MainApplication` 檔案來達成，將 `ReactWebViewPackage` 加入 `getPackages` 函式返回的套件清單中。
