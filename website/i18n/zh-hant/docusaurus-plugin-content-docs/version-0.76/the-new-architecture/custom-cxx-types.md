...
由於 JavaScript 不支援超過 2^53 的數字，C++ Turbo 原生模組目前尚未支援 `int64_t` 數字類型。若要表示大於 2^53 的數字，我們可在 JS 中使用 `string` 類型，並在 C++ 中自動轉換為 `int64_t`。
...
現在我們可以修改 JS 規格，新增使用此類型的函式。與往常一樣，我們可以使用 Flow 或 TypeScript 來編寫規格。
...
首先需更新 `App.tsx` 檔案以使用 TurboModule 的新方法，然後我們可以在 Android 和 iOS 上建置應用程式。

1. 開啟 `App.tsx` 並進行以下修改：
...
首先在 JS 規格中定義新的自定義類型，讓 Codegen 能自動生成所有支援代碼，從而避免手動編寫。
...
此代碼定義了新的 `Address` 類型，並為 Turbo Native Module 新增了 `validateAddress` 函數。注意 `validateFunction` 需要以 `Address` 物件作為參數。

亦可定義返回自定義類型的函數。

### 2. 定義橋接代碼

根據規格中定義的 `Address` 類型，Codegen 會生成兩種輔助類型：`NativeSampleModuleAddress` 和 `NativeSampleModuleAddressBridging`。
...
- 名稱第二部分始終是規格中定義的 JS 類型名稱（本例為 `Address`）
:::

### 3. 實現原生代碼

現在需在 C++ 中實現 `validateAddress` 函數。首先在 `.h` 文件添加函數聲明，然後在 `.cpp` 文件中實現。
...
在實現中，代表 `Address` 的物件是 `jsi::Object`。要從此物件提取值，需使用 `JSI` 提供的訪問器：
