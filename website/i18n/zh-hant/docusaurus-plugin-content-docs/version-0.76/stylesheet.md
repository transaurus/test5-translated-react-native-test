...
設置一個函數用於預處理樣式屬性值。此功能內部用於處理顏色和變換值。除非您確切知道自己在做什麼且已用盡其他選項，否則不應使用此功能。
...
一個非常常見的模式是創建具有絕對定位和零定位的覆蓋層（`position: 'absolute', left: 0, right: 0, top: 0, bottom: 0`），因此 `absoluteFill` 可用於方便性和減少這些重複樣式的冗餘。如果需要，absoluteFill 可用於在 StyleSheet 中創建自定義條目，例如：
