...
請注意，ScrollView 必須具有固定高度才能正常運作，因為它們是將無限高度的子元件裝載到有限容器中（透過滾動互動）。若要限制 ScrollView 的高度，可以直接設定視圖高度（不建議）或確保所有父視圖都具有固定高度。忘記將 `{flex: 1}` 向下傳遞至視圖結構可能導致錯誤，此時使用元素檢查器能快速除錯。
...
假設您需要顯示一個非常長的項目列表，內容可能多達數個畫面。一次性為所有內容建立 JS 元件和原生視圖（其中許多甚至不會立即顯示），將導致渲染速度變慢和記憶體使用量增加。

這時就輪到 `FlatList` 發揮作用。`FlatList` 會惰性渲染項目（僅在即將顯示時才渲染），並移除滾動超出畫面的項目以節省記憶體和處理時間。
...
當為 `true` 時，手勢操作可驅動縮放超出最小/最大值，並在手勢結束時動畫回彈至限制值；否則縮放不會超過設定範圍。
...
當為 `true` 時，若內容小於滾動視圖邊界，會自動置中顯示；內容大於滾動視圖時此屬性無效。
...
此屬性指定如何利用安全區域邊距來調整滾動視圖的內容區域。僅支援 iOS 11 及以上版本。
...
此浮點數決定用戶放開手指後滾動視圖的減速速度。也可使用快捷字串 `"normal"` 和 `"fast"`，分別對應 iOS 的 `UIScrollViewDecelerationRateNormal` 和 `UIScrollViewDecelerationRateFast` 底層設定。
...
有時滾動視圖佔用的空間會比其內容更多。此時，此屬性會以指定顏色填充滾動視圖的剩餘空間，避免設定背景色而導致不必要的過度繪製。這是一項進階優化，一般情況下不需要使用。
...
是否讓黏性標頭固定在滾動視圖底部而非頂部。通常與反向滾動視圖（inverted ScrollViews）搭配使用。
...
- `'interactive'`，鍵盤會隨拖曳互動式關閉，並與觸碰同步移動，向上拖曳可取消關閉。Android 不支援此模式，行為會與 `'none'` 相同。
...
設定此屬性後，滾動視圖會調整滾動位置，確保當前可見且位於或超過 `minIndexForVisible` 的第一個子元件位置保持不變。這適用於雙向載入內容的列表（例如聊天串），避免新訊息進入時導致滾動位置跳動。常見值為 0，但也可設為 1 以跳過載入指示器或其他不應固定位置的內容。

可選參數 `autoscrollToTopThreshold` 可在調整後，若使用者原本位於頂部閾值範圍內，則自動將內容滾動至頂部。這同樣適用於聊天類應用，希望新訊息平滑載入，但避免在使用者已向上滾動較多時造成干擾。

注意事項 1：啟用此功能時重新排序滾動視圖中的元件可能導致跳動或卡頓。此問題可修復，但目前無相關計畫。現階段請勿對使用此功能的滾動視圖或列表重新排序內容。

注意事項 2：此功能透過原生代碼中的 `contentOffset` 和 `frame.origin` 計算可見性。遮擋、變形或其他複雜因素不會納入內容是否「可見」的判斷。
...
設為 true 時，滾動視圖會在滾動時停在視圖大小的整數倍位置。這可用於實現水平分頁效果。
...
用於實現下拉刷新功能的 RefreshControl 組件。僅適用於垂直滾動視圖（必須將 `horizontal` 屬性設為 `false`）。
...
限制滾動事件觸發頻率（單位：毫秒）。當滾動時需執行耗費資源的操作時，此屬性很有用。設為 ≤ `16` 的值會禁用節流，無論設備刷新率如何。
...
滾動指示器與滾動視圖邊緣的內邊距。通常應設為與 `contentInset` 相同的值。
...
設定後，滾動視圖會在 `snapToInterval` 值的倍數處停止。可用於分頁瀏覽長度小於滾動視圖的子項目。通常與 `snapToAlignment` 及 `decelerationRate="fast"` 搭配使用。會覆蓋較不靈活的 `pagingEnabled` 屬性。
...
設定後，滾動視圖會在指定的偏移點停止。可用於分頁瀏覽大小不一且長度小於滾動視圖的子項目。通常與 `decelerationRate="fast"` 搭配使用。會覆蓋較不靈活的 `pagingEnabled` 和 `snapToInterval` 屬性。
...
設為 `true` 時，向下滾動列表會隱藏黏性標頭，向上滾動時則會固定在列表頂部。
...
一個子元素索引陣列，決定哪些子元素在滾動時會固定在畫面頂部。例如，傳入 `stickyHeaderIndices={[0]}` 會讓第一個子元素固定在滾動視圖頂部。也可以使用 [x,y,z] 格式讓多個項目在到達頂部時保持固定。此屬性不支援與 `horizontal={true}` 同時使用。
