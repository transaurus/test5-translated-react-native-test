...
可選提供按鈕列表。點擊任一按鈕將觸發相應的 onPress 回調並關閉警示框。預設情況下，僅會顯示一個「確定」按鈕。

這是一個同時支援 Android 和 iOS 的 API，可顯示靜態警示框。僅 iOS 支援提示用戶輸入資訊的警示框。
...
Android 上的警示框可透過點擊框外區域關閉。此功能預設禁用，可透過提供可選的 [AlertOptions](alert#alertoptions) 參數並將 cancelable 屬性設為 `true` 來啟用，例如<br/>`{cancelable: true}`。
