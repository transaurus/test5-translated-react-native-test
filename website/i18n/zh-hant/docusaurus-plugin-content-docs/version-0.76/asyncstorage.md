...
清除 _所有_ `AsyncStorage` 資料（含其他客戶端、函式庫等）。通常不建議直接呼叫此方法，應改用 `removeItem` 或 `multiRemove` 僅清除自身應用程式的鍵值。回傳 `Promise` 物件。
...
此方法允許您批次獲取給定`key`陣列對應的項目。回調函數將被調用並傳入找到的鍵值對陣列：
...
使用此方法批次儲存多個鍵值對。操作完成時將收到單一回調函數並傳入可能發生的錯誤：
...
批次合併現有值與新值至指定鍵組的操作。此操作假設值為字串化的JSON。返回一個`Promise`物件。
