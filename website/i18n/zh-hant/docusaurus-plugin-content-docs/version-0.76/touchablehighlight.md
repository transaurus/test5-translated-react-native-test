...
用於使視圖正確響應觸控操作的封裝元件。按下時，被包裝視圖的不透明度會降低，使底色顯現，從而讓視圖變暗或著色。

底色效果是通過將子元件包裹在新的 View 中實現的，這可能會影響佈局，若使用不當（例如未明確將被包裹視圖的 backgroundColor 設為不透明顏色）有時會導致非預期的視覺瑕疵。
...
決定觸控激活時被包裹視圖的不透明度值。取值應介於 0 到 1 之間，預設為 0.85。需設定 `underlayColor` 方可生效。
