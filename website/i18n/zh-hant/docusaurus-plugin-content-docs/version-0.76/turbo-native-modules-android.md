...
現在是時候編寫一些 Android 平台代碼，以確保 `localStorage` 在應用程式關閉後仍能保留。
...
接下來我們需要創建 `NativeLocalStoragePackage`。它提供了一個物件，通過將其包裝為 Turbo Native Package，來在 React Native 運行時註冊我們的模組：
...
最後，我們需要告訴主應用程式中的 React Native 如何找到這個 `Package`。我們稱之為在 React Native 中「註冊」該套件。

在這個案例中，你將其添加到 [getPackages](https://github.com/facebook/react-native/blob/8d8b8c343e62115a5509e1aed62047053c2f6e39/packages/react-native/ReactAndroid/src/main/java/com/facebook/react/ReactNativeHost.java#L233) 方法返回的列表中。

:::info
稍後你將學習如何將 Turbo Native 模組作為 [npm 套件](the-new-architecture/create-module-library.md#publish-the-library-on-npm) 分發，我們的構建工具會自動為你進行連結。
:::
