...
設定一個偏移量，該偏移量會附加在透過 `setValue`、動畫或 `Animated.event` 設定的數值之上。適用於補償平移手勢的起始位置等情況。
...
將偏移量合併到基礎數值中，並將偏移量重置為零。最終輸出的數值保持不變。
...
將偏移量設為基礎數值，並將基礎數值重置為零。最終輸出的數值保持不變。
...
停止任何正在運行的動畫或追蹤。`callback` 會在停止動畫後以最終數值調用，這對於將狀態更新至與動畫位置匹配的佈局非常有用。
