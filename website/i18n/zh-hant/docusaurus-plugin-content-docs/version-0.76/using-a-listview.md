...
此範例創建了一個基於硬編碼數據的基本 `FlatList`。`data` 屬性中的每個項目都會被渲染為一個 `Text` 元件。接著 `FlatListBasics` 元件會渲染 `FlatList` 和所有的 `Text` 元件。
...
如果您需要渲染分組數據（可能包含分組標題），類似 iOS 的 `UITableView`，那麼 [SectionList](sectionlist.md) 會是更合適的選擇。
...
列表視圖最常見的用途之一是顯示從伺服器獲取的數據。要實現這一點，您需要 [學習 React Native 的網路相關功能](network.md)。