...
> 預設情況下，iOS 9.0 及以上版本強制執行 App Transport Security (ATS)。ATS 要求所有 HTTP 連接都必須使用 HTTPS。如果您需要從明文字 URL（以 `http` 開頭）獲取資料，首先需要 [添加 ATS 例外](integration-with-existing-apps.md#test-your-integration)。若您事先知道需要存取的網域，僅為這些網域添加例外會更安全；若網域在運行時才知曉，您可以 [完全禁用 ATS](publishing-to-app-store.md#1-enable-app-transport-security)。但請注意，自 2017 年 1 月起，[Apple 的 App Store 審核會要求提供禁用 ATS 的合理理由](https://forums.developer.apple.com/thread/48979)。詳情請參閱 [Apple 官方文件](https://developer.apple.com/library/ios/documentation/General/Reference/InfoPlistKeyReference/Articles/CocoaKeys.html#//apple_ref/doc/uid/TP40009251-SW33)。
...
`RCTSetCustomNSURLSessionConfigurationProvider` 應在應用程式生命週期的早期呼叫，以確保 React 需要時能立即使用，例如：
