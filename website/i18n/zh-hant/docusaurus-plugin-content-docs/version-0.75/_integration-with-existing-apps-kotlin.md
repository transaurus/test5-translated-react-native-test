...
安裝過程會建立新的 `/node_modules` 資料夾，該資料夾儲存建構專案所需的所有 JavaScript 依賴項。
...
若要進一步了解網路安全配置和明文流量政策，[請參閱此連結](https://developer.android.com/training/articles/security-config#CleartextTrafficPermitted)。
...
這允許 JavaScript 控制硬體返回按鈕的行為（例如實現導航邏輯）。若 JavaScript 未處理返回事件，則會調用 `invokeDefaultOnBackPressed` 方法，預設情況下會結束當前 `Activity`。

最後需連接開發者選單。預設通過搖動設備觸發，但在模擬器中不太實用。因此改為透過硬體選單按鈕觸發（Android Studio 模擬器請使用 <kbd>Ctrl</kbd> + <kbd>M</kbd>）：
...
至此已完成 React Native 與現有應用的基礎整合。現在我們將啟動 [Metro 打包工具][metro] 來建構 `index.bundle` 套件，並啟動本地伺服器提供該套件。
...
像往常一樣建構並執行您的 Android 應用程式。

當進入應用內整合 React 的活動時，應會從開發伺服器載入 JavaScript 程式碼並顯示：

![螢幕截圖](/docs/assets/EmbeddedAppAndroid.png)

### 在 Android Studio 建立正式版建構

您也可以使用 Android Studio 建立正式版建構！流程與原本建構原生 Android 應用程式完全相同。

若按照前述方式使用 React Native Gradle 插件，在 Android Studio 中執行應用程式時所有功能應正常運作。

若未使用該插件，則每次正式建構前需額外執行以下命令來建立 React Native 套件（該套件將被包含在原生 Android 應用中）：
...
現在像往常一樣透過 Android Studio 建立正式版建構即可！

### 後續步驟

現在您可以繼續常規開發流程。詳閱 [除錯指南](debugging) 和 [部署文件](running-on-device) 以深入瞭解 React Native 開發。
