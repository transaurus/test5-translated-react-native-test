...
實際上，我們發現開發者並不希望此功能，他們必須透過手動四捨五入來避免元素模糊。在 React Native 中，我們會自動對所有像素進行四捨五入。

我們必須謹慎決定何時進行此四捨五入。您絕不希望同時處理已四捨五入和未四捨五入的值，因為這會累積捨入誤差。即使只有一個捨入誤差也可能導致嚴重後果，例如一像素的邊框可能消失或變為兩倍寬。

在 React Native 中，JavaScript 和佈局引擎中的所有計算均使用任意精度的數字。僅當在主線程上設定原生元素的位置和尺寸時，我們才會進行四捨五入。此外，捨入是相對於根元素而非父元素進行的，這同樣是為了避免累積捨入誤差。
...
將佈局尺寸(dp)四捨五入至最接近對應整數像素的佈局尺寸。例如，在像素密度為3的裝置上，`PixelRatio.roundToNearestPixel(8.4) = 8.33`，這正好對應於(8.33 * 3) = 25像素。