...
大多數函式庫只需拖曳兩個檔案即可完成，有時可能需要第三步驟，但不會超過這個範圍。
...
點擊您的主專案檔案（代表 `.xcodeproj` 的那個），選擇 `Build Phases` 並將要導入函式庫的 `Products` 資料夾中的靜態函式庫拖曳至 `Link Binary With Libraries`
...
若您需要從原生端呼叫它，則我們需要知道函式庫的標頭檔。為此，您必須前往專案檔案，選擇 `Build Settings` 並搜尋 `Header Search Paths`。在此處您應包含函式庫的路徑。（此文件過去建議使用 `recursive`，但現在不再推薦，因為這可能導致細微的建置失敗，特別是與 CocoaPods 一起使用時。）
