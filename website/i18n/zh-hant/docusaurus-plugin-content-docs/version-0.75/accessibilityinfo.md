...
有時了解設備當前是否有啟用螢幕閱讀器非常有用。`AccessibilityInfo` API 正是為此目的而設計。您可以使用它來查詢螢幕閱讀器的當前狀態，並註冊以在螢幕閱讀器狀態變化時接收通知。
...
> **注意**：如果您僅想檢查 TalkBack 的狀態，請使用 [isScreenReaderEnabled](#isscreenreaderenabled)。
...
查詢當前是否啟用了減少動畫效果並偏好使用淡入淡出過渡設定。回傳一個解析為布林值的 Promise，當偏好淡入淡出過渡啟用時結果為 `true`，否則為 `false`。
