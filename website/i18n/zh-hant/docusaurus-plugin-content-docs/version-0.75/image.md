...
您需要在 `android/app/build.gradle` 中添加一些可選模組，具體取決於應用程式的需求。
...
定義圖片替代文字說明的字串，當使用者與圖片互動時，螢幕閱讀器會朗讀此文字。使用此屬性會自動將此元素標記為可無障礙存取。
...
一個指定在獲取圖片資源時使用的 CORS 模式的關鍵字字串。其功能類似於 HTML 中的 crossorigin 屬性。
...
- `scale`：圖片被縮小或放大繪製。與 `resize` 相比，`scale` 更快（通常是硬體加速）並產生更高品質的圖片。如果圖片比視圖小，應使用此方法。如果圖片比視圖稍大，也應使用此方法。
...
- `cover`: 均勻縮放圖片（保持圖片寬高比），使：
  - 圖片的寬度和高度至少有一個維度等於或大於視圖的對應維度（減去內邊距）
  - 縮放後的圖片至少有一個維度與視圖的對應維度（減去內邊距）相等

- `contain`: 均勻縮放圖片（保持圖片寬高比），使圖片的寬度和高度都等於或小於視圖的對應維度（減去內邊距）。

- `stretch`: 獨立縮放寬度和高度，這可能會改變原始圖片的寬高比。

- `repeat`: 重複圖片以填滿視圖框架。圖片將保持其大小和寬高比，除非它比視圖大，在這種情況下它將被均勻縮小以適應視圖。
...
一個表示逗號分隔的潛在候選圖片來源列表的字串。每個圖片來源包含圖片的 URL 和像素密度描述符。如果未指定描述符，則默認為 `1x` 描述符。
...
- `only-if-cached`：無論快取資料的新舊或過期日期為何，都將使用現有的快取資料來滿足請求。如果快取中沒有對應 URL 載入請求的現有資料，則不會嘗試從原始來源載入資料，且該載入將被視為失敗。
