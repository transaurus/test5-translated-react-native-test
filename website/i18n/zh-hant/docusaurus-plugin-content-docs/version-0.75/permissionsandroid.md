...
在 SDK 版本 23 之前的裝置上，只要權限出現在清單中，就會自動授予，因此 `check` 應該總是回傳 `true`，而 `request` 應該總是解析為 `PermissionsAndroid.RESULTS.GRANTED`。

如果使用者之前關閉了您請求的權限，作業系統會建議您的應用程式顯示需要該權限的理由。可選的 `rationale` 參數僅在必要時顯示對話框提示——否則將顯示正常的權限提示。
