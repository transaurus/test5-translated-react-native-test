...
您可透過將堆疊追蹤與產生的原始碼對應檔傳遞至 [`metro-symbolicate`](http://npmjs.com/package/metro-symbolicate)，來符號化上述的最小化函式名稱與位元組碼。

### 啟用原始碼對應檔

符號化堆疊追蹤需使用原始碼對應檔。請確認在建置設定中已為目標平台啟用原始碼對應檔生成功能。
...
- 建置過程可能產生多個原始碼對應檔，請確保使用範例中所示路徑的對應檔。
