...
為指定 `key` 設定數值並在完成時觸發回調。返回 `Promise` 物件。
...
清除 _所有_ `AsyncStorage` 資料（含其他客戶端、函式庫等）。通常應改用 `removeItem` 或 `multiRemove` 僅清除自身應用金鑰。返回 `Promise` 物件。
...
取得當前應用已知的 _所有_ 金鑰（含其他呼叫方、函式庫等）。返回 `Promise` 物件。
...
使用此方法批次儲存多組鍵值對。操作完成時將收到單一回調並包含任何錯誤：
