...
您可以使用 [Hermes](https://github.com/facebook/hermes) 來視覺化 React Native 應用程式中 JavaScript 的效能表現。Hermes 是一個輕量級的 JavaScript 引擎，專為運行 React Native 而優化（您可以在[這裡閱讀更多關於與 React Native 搭配使用的資訊](hermes)）。Hermes 不僅能提升應用程式效能，還提供了分析其所運行 JavaScript 效能的方法。
...
您可以使用 [React Native CLI](https://github.com/react-native-community/cli) 將 Hermes 追蹤分析檔轉換為 Chrome 追蹤分析檔，並透過以下指令將其拉取到本地機器：
...
- **解決方法** 確保您的 Android 裝置/模擬器已連接並正在運行。此指令僅在能存取 adb 時有效。
...
接著可結合 `samples` 和 `stackFrames` 來產生對應時間戳下的所有開始與結束事件，其中：
