...
- 使用 Chrome 除錯時，所有 JavaScript 程式碼會在 Chrome 本身執行，並透過 WebSockets 與原生程式碼通訊。Chrome 使用 [V8](https://v8.dev/) 作為其 JavaScript 引擎。

雖然這些環境非常相似，但您可能會遇到一些不一致的情況。建議避免依賴特定執行環境的特性。
...
許多標準函式在所有支援的 JavaScript 執行環境中皆可用。
