...
現有大量原生UI元件可供最新應用程式使用——有些是平台內建的，有些來自第三方函式庫，還有些可能是您過往專案中自行開發的。React Native已封裝了部分核心平台元件（如`ScrollView`和`TextInput`），但並未涵蓋所有元件，特別是您過去為其他應用開發的自訂元件。幸運的是，我們可以將這些現有元件封裝起來，無縫整合至您的React Native應用中。
...
這些子類別本質上是單例模式——橋接器僅會建立每個子類別的單一實例。它們將原生視圖傳送至`NativeViewHierarchyManager`，該管理器會反過來委派它們按需設定與更新視圖屬性。`ViewManagers`通常也作為視圖的代理，透過橋接器將事件回傳至JavaScript。
...
需在JavaScript端反映的屬性，必須透過`@ReactProp`（或`@ReactPropGroup`）註解的設定器方法公開。設定器方法的第一個參數應為待更新的視圖（當前視圖類型），第二個參數為屬性值。設定器應為公開方法且無返回值（即Java返回類型為`void`，Kotlin為`Unit`）。傳送至JS的屬性類型會根據設定器參數值類型自動判定。目前支援的值類型包括（Java端）：`boolean`、`int`、`float`、`double`、`String`、`Boolean`、`Integer`、`ReadableArray`、`ReadableMap`；Kotlin對應類型為`Boolean`、`Int`、`Float`、`Double`、`String`、`ReadableArray`、`ReadableMap`。
...
最後一步是創建 JavaScript 模組，該模組定義了 Java/Kotlin 與 JavaScript 之間的接口層，供新視圖的使用者調用。建議在此模組中記錄組件接口（例如使用 TypeScript、Flow 或普通註釋）。
...
現在我們知道如何暴露可以從 JS 自由控制的原生視圖組件，但如何處理來自用戶的事件，例如縮放或平移？當原生事件發生時，原生代碼應向 JavaScript 表示的視圖發出事件，兩個視圖通過 `getId()` 方法返回的值進行關聯。
