...
進行此操作前，您應熟悉 [原生 UI 元件](native-components-android) 的概念，並預先了解 [WebView 的原生程式碼](https://github.com/react-native-webview/react-native-webview/blob/master/android/src/main/java/com/reactnativecommunity/webview/RNCWebViewManagerImpl.kt)，因為實作新功能時需以此為參考——但無需深入理解所有細節。
...
開始前，您需建立 `RNCWebViewManager`、`RNCWebView` 和 `RNCWebViewClient` 的子類別。接著在視圖管理器中覆寫以下方法：
...
請參考 React Native WebView 程式庫中的 [RNCWebViewManagerImpl.kt](https://github.com/react-native-webview/react-native-webview/blob/master/android/src/main/java/com/reactnativecommunity/webview/RNCWebViewManagerImpl.kt)，了解可用處理程序及其實作方式。您可擴展此處任何方法以提供額外功能。
...
要使用自訂 WebView，您需建立對應類別。該類別必須：

- Export all the prop types from `WebView.propTypes`
- Return a `WebView` component with the prop `nativeConfig.component` set to your native component (see below)

與常規自訂元件相同，您需使用 `requireNativeComponent` 取得原生元件，但需額外傳入第三個參數 `WebView.extraNativeComponentConfig`。此參數包含僅原生程式碼所需的屬性類型。
