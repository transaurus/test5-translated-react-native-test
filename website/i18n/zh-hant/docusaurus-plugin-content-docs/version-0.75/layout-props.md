...
- 在設定了 flex basis 的節點上，若未設定交叉軸尺寸，長寬比控制節點大小
- 在具有測量函式的節點上，長寬比作用如同測量函式測量 flex basis
- 在具有 flex grow/shrink 的節點上，若未設定交叉軸尺寸，長寬比控制節點大小
- 長寬比會考慮最小/最大尺寸限制
...
當 `flex` 為 -1 時，元件通常會根據 `width` 和 `height` 設定大小。但如果空間不足，元件會縮小至 `minWidth` 和 `minHeight`。
...
[`flexShrink`](layout-props#flexshrink) 描述當子項目在主軸上的總大小超過容器大小時，應如何縮小子項目。`flexShrink` 與 `flexGrow` 非常相似，可以將溢出的空間視為負的剩餘空間來理解。這兩個屬性通常一起使用，以允許子項目根據需要伸縮。
...
如果想相對於非父元素的物件定位子元素，請不要使用樣式來實現，而應使用元件樹結構。
...
`zIndex` 控制哪些元件顯示在其他元件之上。通常情況下不需要使用 `zIndex`。元件會根據其在文件樹中的順序渲染，因此後面的元件會覆蓋前面的元件。`zIndex` 在動畫或自定義模態介面等不希望出現此行為的情況下可能有用。
