...
- `runAfterInteractions()` 稍後運行代碼，且不會延遲當前動畫。

觸控處理系統將一或多個活躍觸控視為「互動」，並會延遲 `runAfterInteractions()` 回調，直到所有觸控結束或被取消。

InteractionManager 還允許應用程式在動畫開始時創建互動「句柄」並在完成時清除，藉此註冊動畫：
...
預設情況下，排隊任務會在一個 `setImmediate` 批次中一起執行。若呼叫帶正數的 `setDeadline`，則任務僅執行至截止時間（以 js 事件循環運行時間為準），屆時會透過 setTimeout 讓出執行權，使觸控等事件能開始互動並阻止排隊任務執行，提升應用響應性。
...
正數會使用 setTimeout 在 eventLoopRunningTime 達到截止值後安排任務，否則所有任務將在一個 setImmediate 批次中執行（預設）。