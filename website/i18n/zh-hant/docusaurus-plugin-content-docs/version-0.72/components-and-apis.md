...
React Native 提供了許多內建的[核心元件](intro-react-native-components)，可直接用於您的應用程式。您可以在左側邊欄（或螢幕頂部選單，如果您使用的是窄螢幕）中找到所有元件。如果不確定從何開始，請參考以下分類：
...
與通用的 [`ScrollView`](./scrollview) 不同，以下列表視圖元件僅渲染當前顯示在螢幕上的元素。這使得它們成為顯示長數據列表的高效選擇。
...
這些元件可能對特定應用程式有所幫助。如需完整的元件和 API 列表，請查看左側邊欄（或螢幕頂部選單，如果您使用的是窄螢幕）。
