...
React 18 的並行功能建立在新的並行渲染引擎之上。並行渲染是一種新的底層機制，讓 React 能夠同時準備多個版本的 UI。
...
這意味著你可以在遷移後立即使用 React 18 的新功能。由於新的並行功能是透過 `startTransition` 或 `Suspense` 等功能選擇性啟用的，我們預期對於遷移至新架構或啟用新架構建立新應用程式的使用者來說，React 18 能夠開箱即用，且僅需極少變更。
