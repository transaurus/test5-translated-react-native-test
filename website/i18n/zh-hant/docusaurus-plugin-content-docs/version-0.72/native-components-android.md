...
現有大量原生UI元件可供最新應用程式使用——有些是平台內建元件，其他則來自第三方函式庫，甚至可能是您過往開發的專屬元件。React Native已封裝了部分核心平台元件（如`ScrollView`和`TextInput`），但並未涵蓋所有元件，特別是您過去為其他應用程式自行開發的元件。幸運的是，我們可以封裝這些現有元件，使其與React Native應用程式無縫整合。

與原生模組指南類似，這同樣屬於進階指南，假設您已具備Android SDK開發基礎。本指南將透過實作React Native核心函式庫中現有`ImageView`元件的子集，示範如何建構原生UI元件。

## ImageView範例

本範例將逐步說明如何實作必要功能，以允許在JavaScript中使用ImageView元件。

原生視圖透過繼承`ViewManager`或更常用的`SimpleViewManager`來建立與操作。此案例中使用`SimpleViewManager`較為便利，因為它已實作常見屬性設定，如背景色、透明度與Flexbox佈局。

這些子類別本質上是單例模式——橋接器只會建立每個子類別的一個實例。它們將原生視圖傳送至`NativeViewHierarchyManager`，該管理器會視需要回調這些子類別來設定與更新視圖屬性。`ViewManagers`通常也作為視圖的代理，透過橋接器將事件回傳至JavaScript。
...
需要在JavaScript中反映的屬性必須透過使用`@ReactProp`（或`@ReactPropGroup`）註解的設置器方法公開。設置器方法應將要更新的視圖（當前視圖類型）作為第一個參數，屬性值作為第二個參數。設置器應為公開方法且不返回值（即Java中的返回類型應為`void`，Kotlin中為`Unit`）。傳送至JS的屬性類型會根據設置器值參數的類型自動判斷。目前支援以下值類型（Java中）：`boolean`、`int`、`float`、`double`、`String`、`Boolean`、`Integer`、`ReadableArray`、`ReadableMap`。Kotlin中對應類型為`Boolean`、`Int`、`Float`、`Double`、`String`、`ReadableArray`、`ReadableMap`。

註解`@ReactProp`有一個必要參數`name`（類型為`String`）。分配給設置器方法的`@ReactProp`註解名稱用於在JS端引用該屬性。
...
最後一步是將 ViewManager 註冊到應用程序中，這與 [原生模組](native-modules-android.md) 的註冊方式類似，透過應用程序套件的成員函數 `createViewManagers` 完成。
...
最終步驟是建立 JavaScript 模組，該模組為新視圖的使用者定義 Java/Kotlin 與 JavaScript 之間的介面層。建議在此模組中記錄組件介面（例如使用 TypeScript、Flow 或普通註解）。
...
現在我們知道如何暴露可從 JS 自由控制的原生視圖組件，但該如何處理來自使用者的事件（例如縮放或平移）？當原生事件發生時，原生代碼應向該視圖的 JavaScript 表示層發送事件，兩個視圖透過 `getId()` 方法的返回值進行關聯。
...
此回調會接收原始事件，我們通常在包裝組件中處理以簡化 API：
