...
React Native 使用 CocoaPods 管理 iOS 專案相依性，多數 React Native 函式庫遵循此慣例。若您使用的函式庫未採用此方式，請參閱其 README 以獲取額外指引。多數情況下，以下指令皆適用。
...
若在 React Native Directory 中找不到專為 React Native 設計的函式庫，[npm 套件庫](https://www.npmjs.com/) 是次佳選擇。npm 套件庫是 JavaScript 函式庫的權威來源，但所列函式庫未必皆相容於 React Native。React Native 是眾多 JavaScript 執行環境之一，其他環境包含 Node.js、網頁瀏覽器、Electron 等，而 npm 收錄適用於所有這些環境的函式庫。
...
[React Native Directory](https://reactnative.directory) 允許您按平台相容性進行篩選，例如 iOS、Android、Web 和 Windows。如果您想使用的函式庫目前未列在其中，請參考該函式庫的 README 文件以獲取更多資訊。
