...
底層效果是通過將子元件封裝在新的 View 中實現的，這可能影響佈局，若使用不當（例如未明確將被包裝視圖的 backgroundColor 設為不透明顏色）可能導致非預期的視覺瑕疵。
...
決定觸控激活時被包裝視圖的不透明度值。應介於 0 到 1 之間，預設為 0.85。需設定 `underlayColor` 方可生效。
...
_(僅限 Apple TV)_ 電視首選焦點（參見 View 元件的文檔說明）。
