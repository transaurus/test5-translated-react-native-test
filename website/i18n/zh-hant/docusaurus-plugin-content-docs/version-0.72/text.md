...
假設 `MyAppText` 是一個僅將其子元素渲染為帶有樣式的 `Text` 元件的元件，那麼 `MyAppHeaderText` 可以如下定義：
...
- （開發者）React 元件設計時考慮了強隔離性：您應該能夠將元件放置在應用程式的任何位置，並相信只要屬性相同，它的外觀和行為就會相同。從屬性外部繼承的文字屬性將破壞這種隔離性。
...
詳情請參閱 [iOS `accessibilityLanguage` 文檔](https://developer.apple.com/documentation/objectivec/nsobject/1615192-accessibilitylanguage)。
...
在 iOS 上，這些角色會映射到對應的輔助功能特徵（Accessibility Traits）。圖片按鈕的功能與同時將特徵設置為「image」和「button」相同。詳情請參閱[輔助功能指南](accessibility.md#accessibilitytraits-ios)。
...
輔助功能操作允許輔助技術以程式方式呼叫元件的操作。`accessibilityActions` 屬性應包含一個操作物件列表。每個操作物件應包含名稱和標籤欄位。

詳情請參閱[輔助功能指南](accessibility.md#accessibility-actions)。
...
詳情請參閱[輔助功能指南](accessibility#accessible-ios-android)。
...
決定文字元素中哪些資料類型會被轉換為可點擊的連結。預設不偵測任何資料類型。
...
在 iOS 上套用於此元素的[動態類型](https://developer.apple.com/documentation/uikit/uifont/scaling_fonts_automatically)級別。
...
> 在 Android 上，當 `numberOfLines` 設為大於 `1` 的值時，僅 `tail` 能正常運作。
...
在使用者按壓時呼叫的函式，於 `onPressOut` 之後觸發。
...
在觸控開始時立即呼叫，於 `onPressOut` 和 `onPress` 之前。
...
在 Android 上，從此回調返回 true 可防止任何其他原生元件在此回應者終止前成為回應者。
...
回應者已從此 `View` 被取走。可能是在呼叫 `onResponderTerminationRequest` 後被其他視圖取走，也可能是未經詢問即被作業系統取走（例如 iOS 上的控制中心/通知中心）。
...
當滾動視圖禁用時，此屬性定義觸點可偏離按鈕多遠才會停用按鈕。停用後若觸點移回範圍內，按鈕會重新激活。建議傳入常數以減少記憶體分配。
...
`role` 向輔助技術使用者傳達組件用途，優先級高於 [`accessibilityRole`](text#accessibilityrole) 屬性。
...
設為 `true` 時，文字按下不會顯示視覺反饋。預設情況下，按下時會顯示灰色橢圓形高亮。
