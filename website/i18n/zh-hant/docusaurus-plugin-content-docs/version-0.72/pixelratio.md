...
若您處於高像素密度的設備上，應獲取更高解析度的圖片。一個實用的經驗法則是將顯示圖片的尺寸乘以像素比例。
...
將布局尺寸(dp)四捨五入至最接近的對應整數像素值的布局尺寸。例如，在像素密度比(PixelRatio)為3的設備上，`PixelRatio.roundToNearestPixel(8.4) = 8.33`，這正好對應於(8.33 * 3) = 25像素。