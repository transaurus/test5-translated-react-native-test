...
除了 `https`，您可能還熟悉 `mailto` scheme。當您打開一個帶有 mailto scheme 的連結時，您的作業系統會打開已安裝的郵件應用程式。同樣地，也有用於撥打電話和發送 SMS 的 scheme。請參閱下方關於[內建 URL](#built-in-url-schemes) scheme 的更多資訊。

就像使用 mailto scheme 一樣，可以通過自定義 url scheme 連結到其他應用程式。例如，當您從 Slack 收到一封 **Magic Link** 電子郵件時，**Launch Slack** 按鈕是一個錨標籤，其 href 看起來像這樣：`slack://secret/magic-login/other-secret`。與 Slack 一樣，您可以告訴作業系統您希望處理自定義 scheme。當 Slack 應用程式打開時，它會接收到用於打開它的 URL。這通常被稱為深度連結。請參閱關於如何[獲取深度連結](#get-the-deep-link)到您的應用程式中的更多資訊。

自定義 URL scheme 並不是在移動設備上打開應用程式的唯一方式。您不希望在使用電子郵件中的連結時使用自定義 URL scheme，因為這樣連結在桌面上會失效。相反，您希望使用常規的 `https` 連結，例如 `https://www.myapp.io/records/1234546`，並在移動設備上讓該連結打開您的應用程式。Android 稱之為 **Deep Links**（iOS 稱為 Universal Links）。
...
您可以使用 `Linking.getInitialURL()` 來處理這些事件 - 它會返回一個解析為 URL 的 Promise（如果有）。
...
該方法返回一個 `Promise` 物件。如果用戶確認打開對話框或 URL 自動打開，Promise 會被解析。如果用戶取消打開對話框或沒有註冊的應用可以處理該 URL，Promise 會被拒絕。
...
> 如果系統不知道如何打開指定的 URL，此方法會失敗。如果你傳入非 http(s) URL，最好先檢查 `canOpenURL()`。
