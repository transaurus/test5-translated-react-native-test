...
根據 `key` 獲取項目並在完成時觸發回調。返回 `Promise` 物件。
...
為 `key` 設置值並在完成時觸發回調。返回 `Promise` 物件。
...
清除 _所有_ 客戶端、函式庫等的 `AsyncStorage` 資料。通常不應直接調用此方法，建議使用 `removeItem` 或 `multiRemove` 僅清除應用自身的鍵值。返回 `Promise` 物件。
...
獲取應用已知的 _所有_ 鍵值（含其他調用者與函式庫）。返回 `Promise` 物件。
...
此方法允許您批次獲取給定 `key` 陣列對應的項目。您的回調函數將被調用，並傳入一組對應的鍵值對：
...
使用此方法作為批次操作來儲存多個鍵值對。當操作完成時，您將獲得一個包含任何錯誤的單一回調：
...
批次操作以合併給定鍵集的現有值和新值。此操作假設這些值是已字符串化的 JSON。返回一個 `Promise` 物件。
