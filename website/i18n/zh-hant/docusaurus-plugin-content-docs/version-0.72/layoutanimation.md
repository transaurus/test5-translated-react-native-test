...
請注意，在**Android**上要使此功能生效，需通過`UIManager`設置以下標記：
...
- `duration`（以毫秒為單位）
- `create`（可選，用於動畫新視圖的配置）
- `update`（可選，用於動畫已更新視圖的配置）
- `delete`（可選，用於動畫移除視圖的配置）
...
- `springDamping`（數字，可選，僅用於`type: Type.spring`）
...
輔助函數，創建一個對象（帶有`create`、`update`和`delete`字段）以傳入[`configureNext`](layoutanimation.md#configurenext)。`type`參數是[動畫類型](layoutanimation.md#types)，`creationProp`參數是[佈局屬性](layoutanimation.md#properties)。
