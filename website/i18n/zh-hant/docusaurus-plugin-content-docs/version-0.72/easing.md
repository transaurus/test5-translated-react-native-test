...
預設彈力值為 1 會略微超出一次。彈力值 0 完全不超出，N > 1 時會超出約 N 次。
...
與 `Animated.parallel()` 併用可建立基礎回彈效果，物件會在動畫開始時略微後退。
...
提供三次貝茲曲線，等同於 CSS Transitions 的 `transition-timing-function`。
