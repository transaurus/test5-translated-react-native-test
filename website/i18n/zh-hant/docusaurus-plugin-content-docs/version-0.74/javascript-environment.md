...
- 若停用 Hermes，React Native 會改用 [JavaScriptCore](https://trac.webkit.org/wiki/JavaScriptCore)（Safari 的 JavaScript 引擎）。注意在 iOS 上，由於應用程式缺乏可寫入的執行記憶體，JavaScriptCore 無法使用 JIT 編譯。
- 使用 Chrome 偵錯時，所有 JavaScript 程式碼會在 Chrome 瀏覽器內執行，並透過 WebSocket 與原生程式碼通訊。Chrome 使用 [V8](https://v8.dev/) 作為 JavaScript 引擎。

雖然這些環境非常相似，但仍可能遇到執行差異。建議避免依賴特定執行環境的特性。

## JavaScript 語法轉換器

語法轉換器讓您能使用最新的 JavaScript 語法，無需等待所有直譯器支援，提升開發體驗。
