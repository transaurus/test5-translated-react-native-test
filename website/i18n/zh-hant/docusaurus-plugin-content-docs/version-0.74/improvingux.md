...
在手機上精準點擊按鈕相當困難。請確保所有互動元素尺寸至少為 44x44 像素。可透過留白空間實現，使用 `padding`、`minWidth` 和 `minHeight` 樣式值會有所幫助。或使用 [`hitSlop` 屬性](touchablewithoutfeedback.md#hitslop) 在不影響版面下擴大觸控區域，範例如下：
...
Android API 21+ 採用 Material Design 漣漪效果，在使用者觸碰螢幕互動區域時提供視覺回饋。React Native 透過 [`TouchableNativeFeedback` 元件](touchablenativefeedback.md) 實現此效果。相較透明度或高亮效果，使用此觸控效果能讓應用更貼合平台體驗。但需注意此效果不支援 iOS 或 Android API < 21，因此需在 iOS 上切換至其他 Touchable 元件。可使用 [react-native-platform-touchable](https://github.com/react-community/react-native-platform-touchable) 等函式庫自動處理平台差異。
...
除非使用 `Dimensions` API 且未處理方向變更，否則多螢幕方向應能預設正常運作。若不想支援多方向，可將螢幕鎖定為直向或橫向。

在 iOS 上，於 Xcode 的 General 標籤頁中 Deployment Info 區段啟用欲支援的裝置方向（需先從 Devices 選單選擇 iPhone）。Android 則需在 AndroidManifest.xml 檔案的 activity 元素內添加 `'android:screenOrientation="portrait"'` 鎖定直向，或 `'android:screenOrientation="landscape"'` 鎖定橫向。
