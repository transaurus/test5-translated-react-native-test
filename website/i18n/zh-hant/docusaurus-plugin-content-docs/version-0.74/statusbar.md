...
對於不適合使用元件的情況，該元件還提供了以靜態函數形式公開的命令式 API。但不建議同時使用靜態 API 和元件來控制同一個屬性，因為靜態 API 設定的值會在下次渲染時被元件設定的值覆蓋。
...
在 Android 上，此屬性僅在 API 23 及以上版本生效。
...
狀態列是否為半透明。當設為 `true` 時，應用程式將在狀態列下方繪製內容。這在使用半透明狀態列顏色時非常有用。
