...
清除 _所有_ `AsyncStorage` 資料（含其他函式庫的資料）。通常應改用 `removeItem` 或 `multiRemove` 僅清除自身應用鍵值。返回 `Promise` 物件。
...
獲取應用程式 _所有_ 已知鍵值（含其他呼叫者的資料）。返回 `Promise` 物件。
...
使用此方法批次儲存多個鍵值對。操作完成時，您將收到一個包含錯誤信息的單一回調：
...
批次操作，用於合併給定鍵集合的現有值與新值。此操作假設這些值是經過 JSON 字符串化的。返回一個 `Promise` 物件。
