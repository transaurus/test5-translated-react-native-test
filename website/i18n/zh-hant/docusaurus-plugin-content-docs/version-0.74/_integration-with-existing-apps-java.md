...
請先依照[環境設定指南](set-up-your-environment)配置開發環境，以便建構 Android 版 React Native 應用
...
安裝過程將顯示類似以下訊息（請向上滾動查看完整輸出）：

> 警告："`react-native@0.70.5`" 缺少必要依賴項 "`react@18.1.0`"

此為正常現象，表示我們需同時安裝 React：
...
安裝程序會建立新的 `/node_modules` 資料夾，用於存放專案建構所需的所有 JavaScript 依賴項
...
:::info
我們刻意不指定 `implementation` 依賴項的版本號，React Native Gradle 插件會自動處理。若未使用該插件，則需手動指定版本
:::
...
這允許 JavaScript 控制用戶點擊硬體返回鍵時的行為（例如實現導航）。當 JavaScript 未處理返回事件時，會調用 `invokeDefaultOnBackPressed` 方法，預設情況下會結束當前 `Activity`。

最後，我們需要連接開發者選單。預設通過搖動設備觸發，但在模擬器中不太實用。因此改為通過硬體選單按鈕觸發（在 Android Studio 模擬器中使用 <kbd>Ctrl</kbd> + <kbd>M</kbd>）：
...
當進入應用中由 React 驅動的 Activity 時，它應從開發伺服器加載 JavaScript 程式碼並顯示：

![螢幕截圖](/docs/assets/EmbeddedAppAndroid.png)

### 在 Android Studio 中建立正式版建構

您也可以使用 Android Studio 建立正式版建構！過程與為既有原生 Android 應用建立正式版同樣快速。

若按照前述方式使用 React Native Gradle 插件，從 Android Studio 運行應用時所有功能應正常運作。

若未使用 React Native Gradle 插件，則每次正式版建構前需額外執行以下命令來建立 React Native 套件（該套件將被包含在原生 Android 應用中）：

```shell
$ npx react-native bundle --platform android --dev false --entry-file index.js --bundle-output android/com/your-company-name/app-package-name/src/main/assets/index.android.bundle --assets-dest android/com/your-company-name/app-package-name/src/main/res/
```

> 請注意替換正確路徑，若 assets 目錄不存在請先建立。

現在像往常一樣通過 Android Studio 建立原生應用的正式版建構即可！

### 後續步驟

現在您可以繼續常規的應用開發。參考我們的[除錯指南](debugging)和[部署文件](running-on-device)以了解更多 React Native 開發相關內容。
