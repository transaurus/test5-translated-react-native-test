...
網路請求本質上是非同步操作。Fetch方法會返回一個[Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise)，讓非同步程式碼編寫更直觀：
...
請務必捕獲`fetch`可能拋出的錯誤，否則它們會被靜默忽略。
...
> 由於原生應用中沒有[跨來源資源共用(CORS)](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing)概念，XMLHttpRequest的安全模型與網頁端不同。
