...
插件預設採用**開箱即用**的合理配置，僅在需要時才需參照本指南調整行為。
...
此參數指向 React Native 專案根目錄（即存放 `package.json` 的目錄）。預設值為 `..`。可依下列方式調整：
...
此參數列出可偵錯的變體版本（關於變體版本的詳細說明請參閱[使用變體](#using-variants)）。
...
列為 `debuggableVariants` 的變體不會包含預打包的 bundle，需依賴 Metro 運行。
...
此參數定義執行腳本時呼叫的 node 指令與參數。預設值為 `[node]`，可依下列方式添加額外參數：
...
所有建置類型與風味的組合會產生一組 **建置變體**。例如，若建置類型為 `debug`/`staging`/`release` 且風味為 `full`/`lite`，您將會有 6 種建置變體：`fullDebug`、`fullStaging`、`fullRelease` 等。
...
此設定是必要的，因為外掛程式會跳過所有 `debuggableVariants` 的 JS 套件生成：您需要 Metro 來執行它們。例如，若您將 `fullStaging` 列入 `debuggableVariants`，則無法將其發佈至商店，因為它會缺少套件。

## 外掛程式在底層做了什麼？

React Native Gradle 外掛程式負責設定您的應用程式建置，以將 React Native 應用程式發佈至生產環境。
此外掛程式也用於第三方函式庫中，以執行新架構所需的 [Codegen](https://github.com/reactwg/react-native-new-architecture/blob/main/docs/codegen.md)。
