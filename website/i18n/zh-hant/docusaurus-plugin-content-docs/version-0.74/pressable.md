...
手指並非最精確的工具，使用者常會意外激活錯誤元素或錯過激活區域。為此，`Pressable` 提供可選的 `HitRect`，可用於定義觸控在距離包裝元素多遠處仍能註冊。按壓可在 `HitRect` 範圍內的任何位置開始。

`PressRect` 允許按壓移動超出元素及其 `HitRect`，同時保持激活狀態並有資格成為「按壓」——想像您按住按鈕時慢慢滑開手指的情況。
...
在此元件範圍外，觸碰仍被視為按壓行為的額外距離範圍（在此範圍內鬆開手指仍會觸發 `onPressOut`）。
...
可接受一般視圖樣式，或接收一個反映元件當前按壓狀態的布林值並返回視圖樣式的函式。
