...
裝置名稱對應 Xcode 中的可用裝置清單。您可以透過終端機執行 `xcrun simctl list devices` 來查看可用裝置。

### 指定裝置版本

若您安裝了多個 iOS 版本，還需指定對應的版本號。例如：要在 iPhone 14 Pro（16.0 版）上運行應用程式，請執行以下指令：
...
您可以指定從 `xcrun simctl list devices` 指令回傳的裝置 UDID。例如：要使用 UDID `AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA` 運行應用程式，請執行以下指令：
