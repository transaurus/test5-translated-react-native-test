...
設定元件尺寸的常見方法是在樣式中添加固定的 `width` 和 `height`。React Native 中的所有尺寸都是無單位的，代表與螢幕密度無關的像素。
...
:::caution
點數與實際測量單位之間沒有通用的對應關係。這意味著具有固定尺寸的元件在不同設備和螢幕尺寸上可能不會具有相同的物理大小。然而，對於大多數使用情況來說，這種差異並不明顯。
:::
...
在你能控制元件的尺寸之後，下一步是[學習如何在螢幕上佈局](flexbox.md)。
