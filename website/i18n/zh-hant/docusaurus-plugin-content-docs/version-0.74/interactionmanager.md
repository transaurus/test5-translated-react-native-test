...
InteractionManager 還允許應用程式在動畫開始時建立互動「句柄」，並在完成時清除它：
...
預設情況下，排隊任務會在一個 `setImmediate` 批次中一起執行。若使用正數呼叫 `setDeadline`，則任務僅會在截止時間（以 js 事件循環運行時間為準）接近時執行，此時執行會透過 setTimeout 讓步，允許觸控等事件開始互動並阻止排隊任務執行，從而提高應用程式的響應速度。
...
使用正數將透過 setTimeout 在 eventLoopRunningTime 達到截止值後安排任何任務，否則所有任務將在一個 setImmediate 批次中執行（預設）。