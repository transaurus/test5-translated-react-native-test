...
並非每個應用程式都會使用所有原生功能，而包含支援這些功能的程式碼會影響二進位檔案的大小...但我們仍希望在您需要時能支援新增這些功能。
...
:::note
所有與 React Native 一同發佈的函式庫都存放在儲存庫根目錄的 `Libraries` 資料夾中。其中一些是純 JavaScript，您只需 `require` 即可使用。
...
如果函式庫包含原生程式碼，其資料夾內應該會有一個 `.xcodeproj` 檔案。將此檔案拖曳到 Xcode 中的專案（通常在 Xcode 的 `Libraries` 群組下）；

![](/docs/assets/AddToLibraries.png)

#### 步驟 2

點擊您的主要專案檔案（代表 `.xcodeproj` 的那個），選擇 `Build Phases`，然後從您正在匯入的函式庫的 `Products` 資料夾中，將靜態函式庫拖曳到 `Link Binary With Libraries`
