...
這個例子創建了一個基於硬編碼數據的基本 `FlatList`。`data` 屬性中的每個項目都會被渲染為一個 `Text` 組件。然後 `FlatListBasics` 組件會渲染 `FlatList` 和所有的 `Text` 組件。
...
如果你想渲染一組按邏輯分段的數據，可能還需要分段標題（類似 iOS 的 `UITableView`），那麼 [SectionList](sectionlist.md) 就是你的選擇。
...
列表視圖最常見的用途之一就是顯示從服務器獲取的數據。要做到這一點，你需要先 [學習 React Native 中的網絡請求](network.md)。