...
除非有充分理由，否則請勿使用。所有響應按壓的元素在觸碰時都應有視覺反饋。

`TouchableWithoutFeedback` 僅支持單個子元素。若需多個子組件，請用 View 包裹它們。需特別注意，`TouchableWithoutFeedback` 的工作原理是克隆其子元素並添加響應屬性，因此任何中介組件都必須將這些屬性傳遞給底層 React Native 組件。
...
設為 `true` 時表示該視圖為無障礙元素。預設情況下，所有可觸碰元素均具無障礙特性。
...
覆寫螢幕閱讀器在用戶操作時朗讀的文本。預設情況下，標籤由遍歷所有子元素並合併以空格分隔的 `Text` 節點生成。
...
當無障礙標籤無法明確表達操作結果時，此提示可幫助用戶理解觸發動作後的預期行為。
...
表示元素將會更新，並描述使用者代理、輔助技術和用戶可以預期的即時區域更新類型。
...
當滾動視圖被停用時，此值定義觸碰可以移出按鈕多遠的距離才會停用按鈕。一旦停用，嘗試移回按鈕，您會看到按鈕再次被啟用！在滾動視圖停用時來回移動幾次。請確保傳入常數以減少記憶體分配。
