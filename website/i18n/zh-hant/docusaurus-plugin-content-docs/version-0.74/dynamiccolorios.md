...
`DynamicColorIOS` 接收一個物件作為參數，該物件包含兩個必要鍵值：`dark` 和 `light`，以及兩個可選鍵值 `highContrastLight` 和 `highContrastDark`。這些鍵值對應於您希望在 iOS 上使用的「淺色模式」和「深色模式」色彩，以及當高對比度無障礙模式啟用時的高對比版本。

在運行時，系統會根據當前的系統外觀和無障礙設定來選擇顯示哪種色彩。動態色彩適用於品牌色彩或其他應用程式特定色彩，這些色彩會自動響應系統設定的變更。
