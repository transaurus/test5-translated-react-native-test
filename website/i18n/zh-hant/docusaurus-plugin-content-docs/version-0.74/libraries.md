...
如果您的電腦已安裝 Node.js，則已內建 npm CLI。部分開發者偏好使用 Yarn Classic，因其安裝速度稍快且提供進階功能如 Workspaces。這兩種工具皆適用於 React Native。為簡化說明，本指南將以 npm 為例。
...
React Native 使用 CocoaPods 管理 iOS 專案相依性，多數 React Native 函式庫遵循此慣例。若您使用的函式庫未採用此方式，請參閱其 README 以取得額外指示。多數情況下，以下指示皆適用。
...
[React Native Directory](https://reactnative.directory) 允許你根據平台相容性進行篩選，例如 iOS、Android、Web 和 Windows。如果你想使用的函式庫目前未列在其中，請參考該函式庫的 README 以獲取更多資訊。

### 它是否與我的 React Native 版本相容？
