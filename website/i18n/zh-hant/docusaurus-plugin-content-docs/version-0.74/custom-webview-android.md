...
進行此操作前，您應熟悉 [原生 UI 元件](native-components-android) 的概念。同時也應了解 [WebView 的原生程式碼](https://github.com/react-native-webview/react-native-webview/blob/master/android/src/main/java/com/reactnativecommunity/webview/RNCWebViewManagerImpl.kt)，因為在實作新功能時需以此為參考——儘管不需要深入理解。
...
您需按照常規步驟 [註冊模組](native-modules-android.md#register-the-module)。

### 新增屬性

要新增屬性，您需將其加入 `CustomWebView`，並在 `CustomWebViewManager` 中公開。
...
您應參考 React Native WebView 程式碼庫中的 [RNCWebViewManagerImpl.kt](https://github.com/react-native-webview/react-native-webview/blob/master/android/src/main/java/com/reactnativecommunity/webview/RNCWebViewManagerImpl.kt)，查看可用的處理程序及其實作方式。您可以擴展此處的任何方法以提供額外功能。
...
要取得原生元件，您必須使用 `requireNativeComponent`：與常規自訂元件相同。但您需傳入額外的第三個參數 `WebView.extraNativeComponentConfig`。此第三參數包含僅原生程式碼需要的屬性類型。
...
若要在原生元件中加入自訂屬性，您可以使用 WebView 上的 `nativeConfig.props`。
