...
您也失去了為整個子樹設置默認字體的能力。同時，`fontFamily` 只接受單一字體名稱，這與 CSS 中的 `font-family` 不同。建議在應用程式中使用一致的字體和大小的方式是創建一個包含這些樣式的元件 `MyAppText`，並在整個應用程式中使用此元件。您還可以使用此元件來創建更具體的元件，如 `MyAppHeaderText`，用於其他類型的文字。
...
- （開發者）React 元件設計時考慮了強隔離性：您應該能夠將元件放置在應用程式的任何位置，並相信只要屬性相同，它將以相同的方式顯示和行為。可以從屬性外部繼承的文字屬性將破壞這種隔離性。
...
覆蓋螢幕閱讀器在使用者與元素互動時所讀取的文字。預設情況下，標籤是通過遍歷所有子元素並累積所有以空格分隔的 `Text` 節點來構建的。
...
在 iOS 上，這些角色映射到相應的輔助功能特性。圖片按鈕的功能與將特性同時設置為「圖片」和「按鈕」相同。詳見 [輔助功能指南](accessibility.md#accessibilitytraits-ios) 以獲取更多資訊。

在 Android 上，這些角色在 TalkBack 上的功能類似於 iOS 上 Voiceover 添加輔助功能特性的功能。
...
輔助功能操作允許輔助技術以程式方式調用組件的操作。`accessibilityActions` 屬性應包含一個操作物件列表。每個操作物件應包含名稱和標籤字段。
...
在使用者執行輔助功能操作時調用。此函數的唯一參數是一個包含要執行操作名稱的事件。
...
回應者已從 `View` 中移除。可能是在呼叫 `onResponderTerminationRequest` 後被其他視圖取得，也可能是作業系統未經詢問直接取得（例如 iOS 上的控制中心/通知中心）。
...
當其他 `View` 試圖成為響應者並要求當前 `View` 釋放響應權時，返回 `true` 表示允許釋放。
...
若父層 `View` 需阻止子層 `View` 在觸碰開始時成為響應者，應設置此處理函數並返回 `true`。
...
設為 `true` 時，文字被按壓時不會產生視覺變化。預設情況下，按壓時會顯示灰色橢圓形高亮效果。
...
在 Android API 等級 23+ 上設定文字斷行策略，可選值為 `simple`、`highQuality`、`balanced`。
