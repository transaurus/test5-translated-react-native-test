...
React Native 提供了許多內建的[核心元件](intro-react-native-components)，可直接用於您的應用程式。您可以在左側邊欄（或螢幕上方的選單，如果您使用的是窄螢幕）找到所有元件。如果不確定從何開始，請參考以下分類：
...
與通用的 [`ScrollView`](./scrollview) 不同，以下列表視圖元件僅渲染當前顯示在螢幕上的元素。這使得它們成為顯示長列表數據的高效能選擇。
...
這些元件可能對特定應用程式有用。如需完整的元件和 API 列表，請查看左側邊欄（或螢幕上方的選單，如果您使用的是窄螢幕）。
