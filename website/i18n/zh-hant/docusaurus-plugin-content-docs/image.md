...
> 注意：上述版本可能未及時更新。請查看主倉庫中的 [`packages/react-native/gradle/libs.versions.toml`](https://github.com/facebook/react-native/blob/main/packages/react-native/gradle/libs.versions.toml) 以了解特定標記版本中使用的 fresco 版本。
...
定義圖片替代文字描述的字符串，當用戶與圖片互動時，屏幕閱讀器會讀取此文字。使用此屬性會自動將該元素標記為無障礙元素。
...
當圖片調整大小時，由 `capInsets` 指定的尺寸的角落將保持固定大小，但圖片的中心內容和邊框將被拉伸。這對於創建可調整大小的圓角按鈕、陰影和其他可調整大小的資源非常有用。更多信息請參閱 [官方 Apple 文檔](https://developer.apple.com/library/ios/documentation/UIKit/Reference/UIImage_Class/index.html#//apple_ref/occ/instm/UIImage/resizableImageWithCapInsets)。
...
- `none`：不進行採樣，圖像以其完整解析度顯示。僅在極少數情況下使用，因為這被視為不安全，Android 在嘗試渲染消耗過多記憶體的圖像時會拋出運行時異常。
...
當 `resizeMethod` 設為 `resize` 時，目標尺寸會乘以此值。`scale` 方法用於執行剩餘的調整大小操作。預設值 `1.0` 表示點陣圖大小設計為適應目標尺寸。大於 `1.0` 的乘數會將調整選項設為大於目標尺寸，生成的點陣圖將從硬體大小縮小。預設為 `1.0`。

此屬性在目標尺寸非常小且源圖像顯著較大的情況下最為有用。`resize` 調整方法執行下採樣，源圖像和目標圖像尺寸之間會損失顯著的圖像品質，通常導致圖像模糊。通過使用乘數，解碼後的圖像略大於目標尺寸但小於源圖像（如果源圖像足夠大）。這允許通過縮放操作在乘積圖像上產生偽品質的混疊效果。
...
此屬性也可以包含多個遠端 URL，並指定其寬度、高度以及可能的縮放比例/其他 URI 參數。原生端會根據圖片容器的測量尺寸選擇最佳的 `uri` 來顯示。可以添加 `cache` 屬性來控制網路請求與本地快取的互動方式。（更多資訊請參閱[圖片快取控制](images#cache-control)）。
...
請注意，`webp` 格式在 iOS 上**僅**在與 JavaScript 代碼捆綁時才支援。
...
表示逗號分隔的候選圖片來源清單的字串。每個圖片來源包含圖片的 URL 和像素密度描述符。如果未指定描述符，則默認為 `1x` 描述符。

如果 `srcSet` 不包含 `1x` 描述符，則會使用 `src` 中的值作為 `1x` 描述符的圖片來源（如果提供）。
...
在顯示圖片之前獲取其寬度和高度（以像素為單位）。如果找不到圖片或下載失敗，此方法可能會失敗。

為了獲取圖片尺寸，可能需要先加載或下載圖片，之後圖片會被快取。這意味著原則上可以使用此方法預加載圖片，但此方法並非為此目的優化，未來可能會以不完全加載/下載圖片數據的方式實現。預加載圖片的正式支援方法將作為單獨的 API 提供。
...
為了取得圖片的尺寸，圖片可能需要先被載入或下載，之後會被快取。這意味著原則上你可以使用此方法來預載圖片，但此方法並非為此目的而優化，且未來可能會以不完全載入/下載圖片資料的方式實作。預載圖片的正確且支援方式將作為獨立的API提供。
...
執行快取查詢。返回一個Promise，解析為從URL到快取狀態的映射，例如「disk」、「memory」或「disk/memory」。如果請求的URL不在映射中，則表示它不在快取中。
...
- `force-cache`: 將使用現有的快取資料來滿足請求，無論其年齡或過期日期。如果快取中沒有對應請求的現有資料，則從原始來源載入資料。
- `only-if-cached`: 將使用現有的快取資料來滿足請求，無論其年齡或過期日期。如果快取中沒有對應URL載入請求的現有資料，則不會嘗試從原始來源載入資料，且載入被視為失敗。
