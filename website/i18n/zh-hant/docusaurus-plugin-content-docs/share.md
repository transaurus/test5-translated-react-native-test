...
在 iOS 中，回傳一個 Promise，該 Promise 將被調用並帶有一個包含 `action` 和 `activityType` 的物件。如果用戶關閉對話框，Promise 仍會解析，其中 action 為 `Share.dismissedAction`，其他鍵則為 undefined。請注意，某些分享選項在 iOS 模擬器上不會顯示或無法使用。

在 Android 中，回傳一個 Promise，該 Promise 總是會解析為 action 為 `Share.sharedAction`。
