...
在[撰寫新原生元件的基礎指南](/docs/fabric-native-components-introduction)中，您已了解如何建立新元件、如何從 JS 端傳遞屬性至原生端，以及如何從原生端發送事件至 JS 端。

自訂元件也能以命令式呼叫原生程式碼實作的函式，以實現更進階的功能，例如以程式化方式重新載入網頁。

本指南將透過新概念「原生指令」來達成此目的。

本指南以[原生元件](/docs/fabric-native-components-introduction)指南為基礎，假設您已熟悉該內容且了解[Codegen](/docs/next/the-new-architecture/what-is-codegen)。
...
更新規格且程式碼準備好使用指令後，現在需實作原生程式碼。但在編寫原生程式碼前，必須重新執行Codegen以生成原生程式碼所需的新類型。
...
現在是實作原生變更的時機，這些變更將讓您的JS能直接呼叫原生視圖上的方法。
...
最後，您可使用常規指令執行應用程式。應用程式執行後，點擊重新整理按鈕即可看到頁面重新載入。
