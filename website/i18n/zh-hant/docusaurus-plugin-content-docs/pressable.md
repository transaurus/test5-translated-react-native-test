...
手指並非最精確的工具，使用者經常會意外激活錯誤的元素或錯過激活區域。為此，`Pressable` 提供了一個可選的 `HitRect`，可用於定義觸控能夠在距離包裝元素多遠的地方註冊。按壓可以在 `HitRect` 內的任何地方開始。
...
在 `onPressOut` 觸發前，定義可視為按壓動作的觸碰範圍超出元件邊界的額外距離。
...
可為視圖樣式物件，或接收當前按壓狀態（布林值）並回傳視圖樣式的函式。
