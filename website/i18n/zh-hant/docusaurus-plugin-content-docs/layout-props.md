...
`bottom` 表示以邏輯像素為單位，偏移此元件底部邊緣的數值。
...
在 React Native 中，`flex` 的運作方式與 CSS 不同。`flex` 是一個數字而非字串，其行為遵循 [Yoga](https://github.com/facebook/yoga) 佈局引擎的規則。
...
當 `flex` 為 0 時，元件的大小由 `width` 和 `height` 決定，且不具彈性。

當 `flex` 為 -1 時，元件通常會根據 `width` 和 `height` 調整大小。然而，如果空間不足，元件會縮小至其 `minWidth` 和 `minHeight`。
...
`flexGrow` 描述容器內剩餘空間應如何沿主軸分配給其子元件。在佈局子元件後，容器會根據子元件指定的 flex grow 值分配剩餘空間。
...
[`flexShrink`](layout-props#flexshrink) 描述當子元件的總大小在主軸上超出容器大小時，應如何沿主軸縮小子元件。`flexShrink` 與 `flexGrow` 非常相似，可以將溢出的空間視為負的剩餘空間來理解。這兩個屬性也能很好地協同工作，讓子元件能根據需要伸縮。
...
`justifyContent` 在主軸方向上對齊子元件。例如，若子元件是垂直排列，`justifyContent` 控制它們如何垂直對齊。其功能類似於 CSS 中的 `justify-content`（預設值為 flex-start）。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/justify-content 以獲取更多資訊。
...
詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/max-width 以獲取更多資訊。
...
`overflow` 控制子元件的測量與顯示方式。`overflow: hidden` 會裁剪視圖，而 `overflow: scroll` 會讓視圖獨立於父元件主軸進行測量。其功能類似 CSS 中的 `overflow`（預設值為 visible）。詳見 https://developer.mozilla.org/en/docs/Web/CSS/overflow 以獲取更多資訊。
...
設定 `padding` 的效果等同於同時設定 `paddingTop`、`paddingBottom`、`paddingLeft` 和 `paddingRight`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/padding 以獲取更多資訊。
...
`paddingBottom` 的功能類似 CSS 中的 `padding-bottom`。詳見 https://developer.mozilla.org/en-US/docs/Web/CSS/padding-bottom 以獲取更多資訊。
...
`zIndex` 控制哪些元件顯示在其他元件之上。通常情況下不需要使用 `zIndex`，元件會依照文件樹中的順序渲染，因此後面的元件會覆蓋前面的元件。`zIndex` 在動畫或自訂模態介面等不希望遵循此行為的情境下可能有用。
...
在 iOS 上，`zIndex` 可能需要 `View` 元件彼此是兄弟節點才能正常運作。
