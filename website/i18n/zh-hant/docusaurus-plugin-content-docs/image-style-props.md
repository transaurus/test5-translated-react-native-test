...
當圖片具有圓角時，指定 overlayColor 會使角落剩餘空間以純色填充。這在以下 Android 圓角實作不支援的情況特別有用：

- Certain resize modes, such as `'contain'`
- Animated GIFs

常見使用方式是將圖片顯示在純色背景上，並將 `overlayColor` 設為與背景相同的顏色。

底層實作細節請參閱 [Fresco 文件](https://frescolib.org/docs/rounded-corners-and-circles.html)。
...
決定當畫框與原始圖片尺寸不符時如何調整圖片大小。預設為 `cover`。
