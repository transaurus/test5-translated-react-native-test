...
此範例創建了一個 `View`，其中包裹了兩個帶顏色的方塊和一個文字元件，並以行排列且具有內邊距。
...
無障礙操作允許輔助技術以程式化方式觸發元件的操作。`accessibilityActions` 屬性應包含一組操作物件，每個物件需包含名稱和標籤欄位。
...
當無障礙標籤無法明確表達操作結果時，無障礙提示可幫助使用者理解觸發該元素操作後的預期行為。
...
指示當色彩反轉功能啟用時，此視圖是否應被反轉。設為 `true` 表示即使啟用色彩反轉，該視圖仍保持原始色彩。
...
- `'combobox'` - 當元素代表組合框（允許使用者從多個選項中選擇）時使用。
- `'menu'` - 當組件是選單時使用。
...
參閱 [無障礙指南](accessibility.md#accessibilitystate-ios-android) 以獲取更多資訊。
...
參閱 [無障礙指南](accessibility.md#accessibilityvalue-ios-android) 以獲取更多資訊。
...
僅用於佈局子元件或不繪製任何內容的視圖可能會作為優化被自動從原生層級中移除。將此屬性設為 `false` 可禁用此優化，確保該 `View` 存在於原生視圖層級中。
...
參見 [Android `importantForAccessibility` 文檔](https://developer.android.com/reference/android/R.attr.html#importantForAccessibility) 以獲取參考。
...
離屏渲染以保留正確的 alpha 行為極為耗費資源且對非原生開發者難以除錯，因此預設未啟用。若您確實需要為動畫啟用此屬性，請考慮在視圖**內容**為靜態時（即不需要每幀重新繪製）與 renderToHardwareTextureAndroid 結合使用。若啟用該屬性，此視圖將離屏渲染一次，儲存為硬體紋理，然後每幀以 alpha 值合成到螢幕上，無需在 GPU 上切換渲染目標。
...
該視圖現在開始響應觸控事件。此時應高亮顯示並向用戶展示當前狀態。
...
這是 `RCTView` 暴露的保留性能屬性，對於包含許多子視圖（大多數位於畫面外）的滾動內容非常有用。要使此屬性生效，必須將其應用於包含許多超出其邊界的子視圖的視圖。子視圖還必須具有 `overflow: hidden`，包含視圖（或其父視圖之一）也應如此。
