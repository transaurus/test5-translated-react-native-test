...
設為 `false` 可移除為特定升部/降部預留的額外字體內邊距。某些字體使用此內邊距時，垂直置中的文字可能略微錯位。為達最佳效果，建議同時將 `textAlignVertical` 設為 `center`。
...
數值型屬性，控制文字元素內行間垂直間距。指定連續文字行基線之間的距離。
...
指定文字對齊方式。在 Android 上，'justify' 值僅支援 Oreo (8.0) 或更高版本 (API level >= 26)。較低版本將自動回退為 `left`。
