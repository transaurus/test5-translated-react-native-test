...
在將應用程式發布給用戶之前，實際裝置測試始終是個好主意。本文將引導您完成在裝置上運行 React Native 應用程式並為生產環境做準備的必要步驟。

:::info
若您使用 `create-expo-app` 建立專案，可透過掃描執行 `npm start` 時顯示的 QR Code，在 Expo Go 中於裝置上運行應用程式。詳情請參閱 Expo 的[在裝置上運行專案指南](https://docs.expo.dev/get-started/expo-go/)。
:::
