...
本指南的其餘部分假設你在應用中有一個本地的 Turbo Native Module，是根據網站上其他指南的指示建立的：特定平台的 Turbo Native Modules，或 [跨平台 Turbo Native Modules](./pure-cxx-modules)。但它也適用於元件和舊架構的模組及元件。你需要根據情況調整需要複製和更新的檔案。
...
查看 `example` 資料夾，其結構與使用 [`react-native-community/template`](https://github.com/react-native-community/template) 建立的 React Native 應用相同。
...
某些情況下，您可能希望直接將函式庫作為本地模組重複使用，無需發佈至 NPM。
...
使用 `create-react-native-library` 建立的函式庫也適用此情境。
...
若立即運行應用，Metro 將找不到需提供給應用的 JS 檔案。這是因為 Metro 會從 `App` 資料夾啟動，無法存取位於 `Library` 資料夾的 JS 檔案。請依下列方式更新 `metro.config.js` 檔案：
...
由於 `create-react-native-library` 已預先配置好發佈流程，因此所有發佈至 NPM 的設定都已就緒。

1. 在模組中安裝相依套件：執行 `yarn install`。
2. 建置函式庫：執行 `yarn prepare`。
3. 發佈函式庫：執行 `yarn release`。

稍等片刻後，您的函式庫就會出現在 NPM 上。若要確認是否已發佈成功，可執行以下指令：
