...
由於 JavaScript 不支援大於 2^53 的數字，C++ Turbo 原生模組目前尚未支援 `int64_t` 型別。我們可在 JS 端使用 `string` 型別表示大數，並在 C++ 端自動轉換為 `int64_t`。
...
接著我們可修改 JS 規格，新增使用此型別的方法。與往常相同，規格可使用 Flow 或 TypeScript 編寫。
...
首先需更新 `App.tsx` 檔案以使用 TurboModule 的新方法，接著可分別建置 Android 與 iOS 應用程式。
...
上述方法可通用於任何類型。對於結構化類型，React Native 提供了一些輔助函數，能更簡便地將其從 JS 橋接到 C++ 或反向操作。

假設我們要橋接一個自定義的 `Address` 類型，其屬性如下：
...
首先，我們在 JS 規格中定義新的自定義類型，讓 Codegen 能自動生成所有支援代碼。這樣就不需手動編寫代碼。
...
現在我們需要在 C++ 中實作 `validateAddress` 函數。首先在 `.h` 檔案中添加函數宣告，然後在 `.cpp` 檔案中實作。
...
在實作中，代表 `Address` 的物件是一個 `jsi::Object`。要從此物件提取值，需使用 `JSI` 提供的存取器：
...
:::note
若想深入了解 `JSI` 及其運作原理，可參考這場 [精彩演講](https://youtu.be/oLmGInjKU2U?feature=shared)（App.JS 2024）
:::
