...
有時需要直接修改元件而不透過 state/props 觸發整個子樹的重新渲染。例如在瀏覽器中使用 React 時，有時需要直接操作 DOM 節點，這在行動應用程式的視圖中同樣適用。`setNativeProps` 就是 React Native 中相當於直接設定 DOM 節點屬性的方法。

:::caution
請在頻繁重新渲染導致效能瓶頸時使用 `setNativeProps`！

直接操作不應是經常使用的手段。通常僅在建立連續動畫以避免元件層級渲染和大量視圖協調的開銷時使用。
...
你可以使用 [`clear`](../textinput#clear) 方法來清空 `TextInput`，該方法採用相同方式清除當前輸入文字。

## 避免與渲染函式衝突

若你更新的屬性同時被渲染函式管理，可能會導致不可預測且難以除錯的問題。因為每當元件重新渲染且該屬性變動時，先前透過 `setNativeProps` 設定的值都會被完全忽略並覆寫。