...
建立動畫的核心流程是：創建一個 `Animated.Value`，將其綁定至動畫元件的多個樣式屬性，再透過 `Animated.timing()` 驅動數值更新。

> 請勿直接修改動畫數值。可使用 [`useRef` Hook](https://reactjs.org/docs/hooks-reference.html#useref) 返回可變的 ref 物件，該物件的 `current` 屬性會作為初始參數並在元件生命週期內持續存在。
...
啟用原生驅動後，所有動畫參數會在開始前預先傳送至原生端，讓 UI 執行緒能直接處理動畫，無需每幀都透過橋接通訊。動畫啟動後，即使 JavaScript 執行緒阻塞也不會影響動畫表現。

在動畫配置中指定 `useNativeDriver: true` 即可啟用。詳見[動畫指南](animations#using-the-native-driver)。

### 可動畫化元件

僅有特殊封裝的「可動畫化元件」能接受動畫數值。這些元件會將動畫數值綁定至屬性，並針對性地更新原生層以避免每幀都觸發 React 渲染與協調過程，同時會自動處理卸載時的資源清理。

- [`createAnimatedComponent()`](animated#createanimatedcomponent) can be used to make a component animatable.

`Animated` 預先導出以下已封裝的元件：
...
若指定stiffness/damping/mass參數，`Animated.spring`將採用基於[阻尼諧振子](https://en.wikipedia.org/wiki/Harmonic_oscillator#Damped_harmonic_oscillator)運動方程的解析彈簧模型。此行為更精確且符合彈簧動力學的物理原理，並高度還原iOS中CASpringAnimation的實現方式。
...
建立一個新的動畫值，其值限制在兩個數值之間。它會根據前後值的差異來調整，因此即使當前值遠離邊界，當值開始接近時仍會觸發變化（`value = clamp(value + diff, min, max)`）。

這在處理滾動事件時非常有用，例如向上滾動時顯示導航欄，向下滾動時隱藏。
...
按順序執行一組動畫，等待前一個動畫完成後才開始下一個。如果當前運行的動畫被停止，後續動畫將不會啟動。
...
同時啟動一組動畫。預設情況下，如果其中一個動畫被停止，所有動畫都會停止。可以通過 `stopTogether` 標誌覆蓋此行為。
...
一組動畫可以並行運行（重疊），但會按順序以連續的延遲啟動。適合實現拖尾效果。
...
用於監聽通過 props 傳遞的動畫事件的高級命令式 API。它允許向現有的 `AnimatedEvent` 添加新的 JavaScript 監聽器。如果 `animatedEvent` 是一個 JavaScript 監聽器，它會將兩個監聽器合併為一個；如果 `animatedEvent` 為 null/undefined，則直接分配 JavaScript 監聽器。盡可能直接使用值。
...
通過調用動畫的 start() 方法來啟動動畫。start() 接受一個完成回調，該回調會在動畫完成或因為調用 stop() 而提前結束時被調用。
...
用於驅動動畫的標準數值類別。通常在類別元件中初始化為 `useAnimatedValue(0);` 或 `new Animated.Value(0);`。
