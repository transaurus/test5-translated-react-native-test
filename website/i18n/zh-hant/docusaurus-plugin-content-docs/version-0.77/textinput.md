...
`TextInput` 默認在其視圖底部有一個邊框。此邊框的填充由系統提供的背景圖像設置，無法更改。避免此問題的解決方案是不要明確設置高度（系統會負責在正確位置顯示邊框），或者通過將 `underlineColorAndroid` 設置為透明來不顯示邊框。
...
指定字體是否應縮放以尊重文本大小無障礙設置。默認為 `true`。
...
當設為 `false` 時，若文字輸入周圍可用空間較少（例如手機橫向模式），作業系統可能會讓使用者在全螢幕文字輸入模式中編輯文字。設為 `true` 時會停用此功能，使用者將始終直接在文字輸入框中編輯文字。預設值為 `false`。
...
決定返回鍵上顯示的文字。此屬性優先於 `returnKeyType`。
...
如果定義，提供的圖片資源將在左側渲染。圖片資源必須位於 `/android/app/src/main/res/drawable` 目錄下，並以如下方式引用：
...
:::note
請注意：在 iOS 上此設定會使文字對齊頂部，而在 Android 上會置中對齊。若要讓兩平台行為一致，請搭配設定 `textAlignVertical` 為 `top`。
:::
...
當按鍵被按下時觸發的回調函式。回傳物件中的 `keyValue` 會是 `'Enter'`、`'Backspace'` 等特殊鍵值，或輸入的字元（包含空格 `' '`）。此事件會在 `onChange` 回調之前觸發。注意：在 Android 上僅處理軟鍵盤輸入，不處理硬體鍵盤輸入。
...
若為 `true`，允許 TextInput 將觸控事件傳遞給父元件。這使得如 SwipeableListView 等元件在 iOS 上可以從 TextInput 滑動（在 Android 上預設即為如此）。若為 `false`，TextInput 總是要求處理輸入（除非被禁用）。預設值為 `true`。
...
若為 `true`，文字輸入會遮蔽輸入內容以保護敏感資訊（如密碼）。預設值為 `false`。不支援與 `multiline={true}` 同時使用。
...
設定選取控點的顏色。與 `selectionColor` 不同，此屬性可獨立自訂選取控點顏色，不受選取反白顏色影響。
...
若為 `false`，停用拼字檢查樣式（如紅色底線）。預設值繼承自 `autoCorrect`。
...
:::note
[`autoComplete`](#autocomplete) 提供相同功能且支援全平台。可透過 [`Platform.select`](/docs/next/platform#select) 實現跨平台差異行為。

避免同時使用 `textContentType` 與 `autoComplete`。為保持向後相容性，兩者同時設定時會優先採用 `textContentType`。
:::

可將 `textContentType` 設為 `username` 或 `password` 以啟用裝置鑰匙圈自動填入登入資訊功能。

`newPassword` 用於標記使用者可能想儲存至鑰匙圈的新密碼輸入欄位，`oneTimeCode` 則用於標記可透過簡訊驗證碼自動填入的欄位。

若要停用自動填寫功能，請將 `textContentType` 設為 `none`。
...
在 iOS 上使用 `textContentType` 設為 `newPassword` 時，我們可以讓作業系統知道密碼的最低要求，以便生成符合這些要求的密碼。要建立有效的 `PasswordRules` 字串，請參閱 [Apple 文件](https://developer.apple.com/password-rules/)。
...
請注意，並非所有文字樣式都受支援，以下是不支援的部分樣式清單：
...
文字輸入框顯示的值。`TextInput` 是一個受控元件，這意味著如果提供了此屬性，原生值將被強制與此值匹配。在大多數情況下，這運作良好，但在某些情況下可能會導致閃爍——一個常見的原因是通過保持相同的值來防止編輯。除了設定相同的值外，還可以設定 `editable={false}`，或設定/更新 `maxLength` 以防止不需要的編輯而不會閃爍。
...
若設為 `true`，則會停用鍵盤快捷鍵（復原/重做和複製按鈕）。預設值為 `false`。
...
若輸入元件目前擁有焦點則回傳 `true`；否則回傳 `false`。
