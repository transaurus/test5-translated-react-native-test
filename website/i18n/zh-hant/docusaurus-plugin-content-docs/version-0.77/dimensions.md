...
> 雖然尺寸資料可立即取得，但這些值可能會變動（例如因裝置旋轉、摺疊裝置等情況），因此任何依賴這些常數的渲染邏輯或樣式，都應該在每次渲染時呼叫此函式，而非快取值（例如使用行內樣式而非在 `StyleSheet` 中設定固定值）。

若您的目標裝置是摺疊式裝置或可能改變螢幕/應用視窗尺寸的裝置，可以使用 Dimensions 模組提供的事件監聽器，如下方範例所示。
...
- `change`：當 `Dimensions` 物件中的屬性變更時觸發。事件處理器的參數為 [`DimensionsValue`](#dimensionsvalue) 類型的物件。
