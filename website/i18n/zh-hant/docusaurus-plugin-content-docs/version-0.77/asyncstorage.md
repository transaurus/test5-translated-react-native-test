...
建議在輕量使用之外，使用基於 `AsyncStorage` 的抽象層而非直接操作 `AsyncStorage`，因其運作於全域範圍。
...
為 `key` 設定數值並在完成時呼叫回調函式。回傳一個 `Promise` 物件。
...
移除 `key` 對應的項目並在完成時呼叫回調函式。回傳一個 `Promise` 物件。
...
合併現有 `key` 值與輸入值，假設兩者均為 JSON 字串。回傳一個 `Promise` 物件。
...
清除 _所有_ 客戶端、函式庫等的 `AsyncStorage` 資料。通常不建議直接呼叫此方法，應改用 `removeItem` 或 `multiRemove` 僅清除應用程式自身的鍵值。回傳一個 `Promise` 物件。
...
使用此方法作為批次操作來儲存多個鍵值對。當操作完成時，您將收到一個包含任何錯誤的單一回調：
...
調用此方法以批次刪除 `keys` 陣列中的所有鍵。返回一個 `Promise` 物件。
...
批次操作以合併給定鍵集的現有值和新值。此操作假設這些值是經過 JSON 字串化的。返回一個 `Promise` 物件。
