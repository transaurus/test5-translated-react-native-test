...
為確保流程順暢，請先為整合專案建立新資料夾，然後**將現有 Android 專案**移至 `/android` 子資料夾中。
...
安裝程序會建立新的 `node_modules` 資料夾，此資料夾存放專案建置所需的所有 JavaScript 依賴套件。
...
接著開啟頂層的 `build.gradle` 檔案並加入這行（參照[社群範本建議](https://github.com/react-native-community/template/blob/0.77-stable/template/android/build.gradle)）：
...
請確保僅在 **debug** 版的清單檔案中添加此設定。

## 4. 撰寫 TypeScript 程式碼

現在我們將實際修改原生 Android 應用程式以整合 React Native。

我們要撰寫的第一段程式碼是新畫面的 React Native 程式碼，該畫面將被整合到我們的應用程式中。
...
每當你建立一個新的 Activity 時，你都需要將其添加到你的 `AndroidManifest.xml` 檔案中。你還需要將 `MyReactActivity` 的主題設置為 `Theme.AppCompat.Light.NoActionBar`（或任何非 ActionBar 的主題），否則你的應用程式將在 React Native 畫面上方渲染一個 ActionBar：
...
現在你的 Activity 已經準備好運行一些 JavaScript 程式碼了。

## 6. 測試你的整合

你已完成整合 React Native 到應用程式中的所有基本步驟。現在我們將啟動 [Metro 打包器](https://metrobundler.dev/)，將你的 TypeScript 應用程式代碼打包成一個 bundle。Metro 的 HTTP 伺服器會將 bundle 從開發環境的 `localhost` 分享到模擬器或裝置上，這使得[熱重載](https://reactnative.dev/blog/2016/03/24/introducing-hot-reloading)成為可能。
...
一旦配置文件就位，你就可以運行打包器。在專案的根目錄下運行以下命令：
