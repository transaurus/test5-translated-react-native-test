...
為 App Store 分發建置應用程式時，需在 Xcode 中使用 `Release` 方案。以 `Release` 建置的應用會自動停用應用內開發者選單，防止使用者在生產環境中誤觸。同時會將 JavaScript 程式碼本地打包，因此即使未連接電腦，也能在裝置上測試應用。
...
當應用套件體積增長時，您可能會在啟動畫面與根應用視圖之間看到空白閃爍畫面。若發生此情況，可將以下程式碼加入 `AppDelegate.m`，以在過渡期間保持啟動畫面顯示。
