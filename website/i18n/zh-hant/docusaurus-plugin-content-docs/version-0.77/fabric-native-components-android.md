...
接著，程式碼定義了一個輔助函式來實際發出事件。要發出事件，您必須：
...
當 React 從應用程式接收到渲染特定元件的指令時，React 使用已註冊的視圖管理器來建立視圖並傳遞所有必要的屬性。
...
接著，它覆寫了 `getName` 函式，該函式必須返回與 spec 中 `codegenNativeComponent` 函式呼叫所使用的相同名稱。
...
最後，若元件能觸發事件，您需透過覆寫 `getExportedCustomBubblingEventTypeConstants`（用於冒泡事件）或 `getExportedCustomDirectEventTypeConstants`（用於直接事件）來映射事件名稱。

### 4. 撰寫 `ReactWebViewPackage`

與原生模組相同，原生元件也需實作 `ReactPackage` 類別。此物件可用於在 React Native 執行環境中註冊元件。
...
最後，您需在應用程式中註冊 `ReactWebViewPackage`。修改 `MainApplication` 檔案，將 `ReactWebViewPackage` 加入 `getPackages` 函式返回的套件清單中。
