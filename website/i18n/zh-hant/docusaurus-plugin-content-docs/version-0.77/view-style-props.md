...
在 iOS 17+ 中，設置為`pointer`時，當指針（如觸控板、Apple Pencil 或 visionOS 的視線追蹤）懸停於視圖上時會顯示懸停效果。
...
使用 Android 底層的[高程 API](https://developer.android.com/training/material/shadows-clipping.html#Elevation)設置視圖的立體高度。這會為元素添加投射陰影並影響重疊視圖的 z 軸排序。僅支援 Android 5.0+，更早版本無效。
