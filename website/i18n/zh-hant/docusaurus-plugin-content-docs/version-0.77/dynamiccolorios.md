...
`DynamicColorIOS` 接收一個物件作為參數，該物件包含兩個必要鍵值：`dark` 和 `light`，以及兩個可選鍵值 `highContrastLight` 和 `highContrastDark`。這些鍵值分別對應 iOS 在「淺色模式」、「深色模式」以及啟用高對比度無障礙模式時所要使用的色彩。

在運行時，系統會根據當前外觀設定和無障礙設定自動選擇要顯示的色彩。動態色彩特別適用於品牌色彩或其他需要隨系統設定自動變更的應用程式特定色彩。
