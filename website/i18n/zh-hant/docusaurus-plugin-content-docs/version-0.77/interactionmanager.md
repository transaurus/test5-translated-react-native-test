...
InteractionManager 允許在互動/動畫完成後安排長時間運行的任務，這能確保 JavaScript 動畫流暢運行。
...
觸控處理系統將一或多個有效觸控視為「互動」，會延遲執行 `runAfterInteractions()` 回調，直到所有觸控結束或被取消。

InteractionManager 也允許應用程式在動畫開始時建立互動「handle」並在完成時清除，藉此註冊動畫：
...
預設情況下，排隊任務會在一個 `setImmediate` 批次中循環執行。若呼叫 `setDeadline` 並傳入正數，則任務僅執行至截止時間（依據 js 事件循環運行時間），屆時會透過 setTimeout 讓出執行權，允許觸控等事件開始互動並阻止排隊任務執行，提升應用程式響應速度。
...
傳入正數時，當 eventLoopRunningTime 達到截止值後會使用 setTimeout 安排任務，否則所有任務將在一個 setImmediate 批次中執行（預設行為）。