...
除了 `https` 之外，你可能也熟悉 `mailto` scheme。當你開啟一個帶有 mailto scheme 的連結時，作業系統會啟動已安裝的郵件應用程式。同樣地，也有用於撥打電話和發送簡訊的 scheme。更多關於[內建 URL](#built-in-url-schemes) scheme 的資訊請見下文。

就像使用 mailto scheme 一樣，也可以透過自訂 URL scheme 連結到其他應用程式。例如，當你收到 Slack 的 **Magic Link** 郵件時，**Launch Slack** 按鈕實際上是一個帶有類似 `slack://secret/magic-login/other-secret` 這樣 href 屬性的錨點標籤。與 Slack 類似，你可以告訴作業系統你希望處理某個自訂 scheme。當 Slack 應用程式開啟時，它會接收到用來開啟它的 URL。這通常被稱為深度連結 (deep linking)。更多關於如何[獲取深度連結](#get-the-deep-link)到你的應用程式的資訊請見下文。

自訂 URL scheme 並不是在行動裝置上開啟應用程式的唯一方式。例如，如果你想透過電子郵件發送一個在行動裝置上開啟的連結，使用自訂 URL scheme 並不理想，因為使用者可能在桌機上開啟郵件，而連結將無法運作。相反地，你應該使用標準的 `https` 連結，例如 `https://www.myapp.io/records/1234546`。在行動裝置上，這些連結可以配置為開啟你的應用程式。在 Android 上，此功能稱為 **Deep Links**，而在 iOS 上則稱為 **Universal Links**。
...
你可以使用 `Linking.getInitialURL()` 來處理這些事件——它會回傳一個解析為 URL 的 Promise（如果有的話）。
...
透過監聽 `url` 事件類型並提供處理函式，來為 Linking 的變化添加處理器。
...
此方法會回傳一個 `Promise` 物件。當系統判斷該 URL 能否被處理時，Promise 會解析並回傳第一個參數表示該 URL 是否能被打開。
...
> 當目標為 Android 11 (SDK 30) 時，必須在 `AndroidManifest.xml` 中聲明您要處理的 scheme 對應的意圖。常見意圖列表可參考 [此處](https://developer.android.com/guide/components/intents-common)。
>
> 例如要處理 `https` scheme，需在清單中添加以下內容：
>
> ```
> <manifest ...>
>     <queries>
...
若應用程式是由應用程式連結觸發啟動，則回傳該連結 URL，否則回傳 `null`。

> 關於 Android 深度連結支援，請參閱 https://developer.android.com/training/app-indexing/deep-linking.html#handling-intents

> 啟用遠端 JS 除錯時，getInitialURL 可能回傳 `null`。請停用除錯器以確保傳遞正常。
...
此方法會回傳一個 `Promise` 物件。若使用者確認開啟對話框或 URL 自動開啟，Promise 會解析。若使用者取消開啟對話框或沒有應用程式註冊處理該 URL，Promise 會拒絕。
...
> 若系統不知道如何開啟指定的 URL，此方法會失敗。若傳入非 http(s) 的 URL，建議先檢查 `canOpenURL()`。
