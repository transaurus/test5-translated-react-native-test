...
`AppRegistry` 是所有 React Native 應用程式的 JavaScript 入口點。應用程式的根元件應透過 `AppRegistry.registerComponent` 註冊自身，之後原生系統可以載入應用程式的套件包，並在準備就緒時透過呼叫 `AppRegistry.runApplication` 實際運行應用程式。
...
`AppRegistry` 應在 `require` 序列的早期引入，以確保 JavaScript 執行環境在其他模組被引入前就已設置完成。
